...
sentence splitting and match keys); all three phases executed in one
pass, each gated by the golden test and byte-identical output checks.

Final artifact shapes (supersedes the proposal below where they differ):

- Store: `output/_questions/{set_slug}.json` — `{_id: doc}`, sorted keys.
- Topic: `output/{slug}/questions_ref.json` — ordered
  `[{query_string, difficulties, min_year, mentions, tossups: [_id],
  bonuses: [_id]}]`; written by `questions_store.record_fetch` (run.py).
- Sweep: `set.json` rows carry `id` (bonus rows also `part`); text
  resolved by render_sweep.
- Unit: `questions.json` — `{answerline: [{id, part}]}`;
  `questions_data.js` is generated by build_overviews (gitignored).
  Agents inspect answerline text via
  `python lib/sweep/capture_questions.py <unit> --show "<answerline>"`.
- Raw API responses cache to gitignored `cache/topics/` etc. — a
  network-avoidance layer only.
- validate: `NO QUESTIONS REF` + `DANGLING QUESTION REF` checks.

Verification: all 773 questions.html byte-identical across the cutover;
sweep.html hash-identical; opera questions_data.js byte-identical when
regenerated from refs. Net: 1,736 committed cache files (~71 MB) →
20k-doc store (~50 MB) + small refs.
