...
**Demo tournament (July 30, 2026; reworked Aug 1):** `app/demo.html` — a
fully in-browser simulated tournament so prospective TDs can try the TD
hub, MODAQ, stats, buzzpoints, and categories without creating anything.
`?t=demo`, `?a=demo`, or `?b=demo` / `?b=demo-b` (slug reserved
server-side; bucket secrets are long random tokens so the demo names
can't collide) make `api.js` serve every `pub()` call from
`app/js/demo.js`, which emulates the `/pub`, `/b`, AND `/a` Worker routes
over a committed fixture (`app/demo/fixture.js`, generated by
`tools/demo_fixture.mjs` from 2022 ACF Winter packets 1-9 in
`tools/demo/`, exported by `dev/oneshots/export_vault_demo_packets.py`
with `<i>`→`<em>` because MODAQ's formatter throws on unknown tags) plus
localStorage uploads — the same frozen-data-behind-the-real-pages idea as
the archive, but live. Fixture: 4-team triple round robin (engine `rr3`,
9 rounds, ACF so no powers) of Stanford / Berkeley / UIUC / ASU; rounds
1-6 in plus round 7's Room B game, and Room A reads the round-7
Stanford–Berkeley rubber match (both 5-1); the upload completes round 7
and unlocks it on buzzpoints (gate pre-seeded in sessionStorage). The TD
hub runs against the emulated `/a` detail + blob reads (client-side
stats/report/.yft/zip all work), advance-round and visitor-upload delete
persist, other writes reject as 'not in the demo'; raw `${API}` download
hrefs in admin.js are the one known 404. In `read.bundle.js` demo.js
loads via a runtime non-literal `import()` so the 784 KB fixture never
rides in the bundle. Zero Worker traffic, enforced by a fetch-stub in the
unit suite. Reset clears `qbtdDemoUploads`, `qbtdDemoRound`, and the
reader's `qbtd{Meta,Game}:{demo,demo-b}:*` keys.
