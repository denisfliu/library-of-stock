...
**Demo tournament (July 30, 2026):** `app/demo.html` — a fully in-browser
simulated tournament so prospective TDs can try MODAQ, stats, buzzpoints,
and categories without creating anything. `?t=demo` / `?b=demo` (slug and
bucket secret reserved server-side) make `api.js` serve every `pub()` call
from `app/js/demo.js`, which emulates the `/pub` + `/b` Worker routes over
a committed fixture (`app/demo/fixture.js`, generated by
`tools/demo_fixture.mjs` from three hand-written 10/10 packets in
`tools/demo/`) plus localStorage uploads — the same
frozen-data-behind-the-real-pages idea as the archive, but live: the
fixture is a 4-team RR with rounds 1-2 played and round 3's Room B game
in; the visitor's Room A reads Riverside vs Summit (both 2-0), and the
upload completes round 3, which also unlocks it on the buzzpoints tab
(the buzz password gate is pre-seeded in sessionStorage). Reset clears
`qbtdDemoUploads` + the reader's `qbtd{Meta,Game}:demo:*` keys.
