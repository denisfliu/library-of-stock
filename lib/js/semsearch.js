...
function runSearch() {
  ...
  if (!auth) {
    $('results').innerHTML = '<div class="note">Semantic search runs on the '
      + 'sync backend and needs a (free) sign-in &mdash; it embeds your query '
      + 'server-side. <button type="button" class="linkbtn" id="signin">'
  ...
  fetch(CFG.syncBase + '/search', {
    method: 'POST',
    headers: Object.assign({ 'Content-Type': 'application/json' }, auth),
    body: JSON.stringify(Object.assign({ q }, readFilters())),
  }).then(r => {
    if (r.status === 401) throw new Error('Session expired — sign in again from the button above.');
...
function renderResults(results) {
  if (!results.length) {
    $('results').innerHTML = '<div class="note">No matches within these filters. '
      + 'Narrow filters shrink the scanned pool &mdash; try widening them.</div>';
    return;
  }
