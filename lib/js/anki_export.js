...
async function exportApkg(cards, deckName) {
    ...
    // Model IDs must be GLOBAL constants — same across all decks.
    // Using a deck-specific hash caused Anki to treat each import as a new
    // note type, multiplying "StockQB Basic+" entries in the card type list.
    const modelId = stableHash('StockQB Basic');
    const imageModelId = stableHash('StockQB Image');
    ...
    // Download images and audio, track all media files
    ...
    // Collect unique audio URLs (mp3 paths relative to the output dir)
    const audioUrls = [];
    for (const card of cards) {
        if (card.type === 'audio' && card.mp3 && !audioUrls.includes(card.mp3)) {
            audioUrls.push(card.mp3);
        }
    }
    ...
    const audioMediaMap = {};  // mp3 url -> bundled filename
    for (const audioUrl of audioUrls) {
        try {
            if (mediaIdx > 0) await new Promise(r => setTimeout(r, 200));
            const resp = await fetchWithRetry(audioUrl);
            if (resp) {
                const blob = await resp.blob();
                if (blob.size > 0) {
                    ...
                    if (Object.values(audioMediaMap).includes(fname)) {
                        fname = `${mediaIdx}_${fname}`;
                    }
                    audioMediaMap[audioUrl] = fname;
                    mediaFiles[String(mediaIdx)] = { name: fname, blob: new Uint8Array(await blob.arrayBuffer()) };
                    mediaIdx++;
                    console.log(`Downloaded audio: ${fname} (${blob.size} bytes)`);
                }
            } else {
                console.warn(`Failed to download audio: ${audioUrl}`);
            }
        } catch (e) {
            console.warn('Failed to process audio:', audioUrl, e);
        }
    }
    console.log(`Processed ${audioUrls.length} audio clips`);
    ...
    for (let i = 0; i < cards.length; i++) {
        ...
        const guid = stableGuid(deckName, card.front || '', card.back || '');
        noteId++;
        cardId++;
        ...
        // For audio cards, append [sound:filename] to the back field
        let backContent = card.back || '';
        if (card.type === 'audio' && card.mp3 && audioMediaMap[card.mp3]) {
            backContent += `\n[sound:${audioMediaMap[card.mp3]}]`;
        }
        ...
        if (hasFrontImg && !hasBackImg) {
            ...
            const flds = frontContent + '\x1f' + backContent + '\x1f' + (hasBackImg ? backImgTags : '');
            db.run(`INSERT INTO notes VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
...
function stableGuid(deck, front, back) {
    // Deterministic GUID based on deck+front+back so re-importing the same
    // topic updates existing notes rather than creating duplicates.
    const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!#$%&()*+,-./:;<=>?@[]^_`{|}~';
    const key = deck + '\x1f' + front + '\x1f' + back;
    let h = 5381;
    for (let i = 0; i < key.length; i++) {
        h = ((h << 5) + h) ^ key.charCodeAt(i);
        h = h >>> 0; // keep unsigned 32-bit
    }
    // Produce a 10-char string in Anki's allowed GUID charset
    let result = '';
    let n = h;
    for (let i = 0; i < 10; i++) {
        result += chars[n % chars.length];
        n = Math.floor(n / chars.length) || (h ^ (i * 2654435761)) >>> 0;
    }
    return result;
}
