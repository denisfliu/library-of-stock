...
function initSearchNav(containerSelector, options = {}) {
    ...
    let nextGuide = null;
    if (currentSlug) {
        const currentPath = 'output/' + currentSlug + '/stock.html';
    ...
    // Overflow menu toggle (mobile)
    const overflowBtn = document.querySelector('.nav-overflow-btn');
    const overflowWrap = document.querySelector('.nav-overflow-wrap');
    if (overflowBtn && overflowWrap) {
        overflowBtn.addEventListener('click', e => {
            e.stopPropagation();
            overflowWrap.classList.toggle('open');
        });
        document.addEventListener('click', () => overflowWrap.classList.remove('open'));
        overflowWrap.querySelector('.nav-secondary')?.addEventListener('click', e => e.stopPropagation());
    }
}
