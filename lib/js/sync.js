...
function showMergePrompt(token, p, count) {
  ...
  card.innerHTML =
    ...
    '<button class="btn' + (switching ? ' primary' : '') + '" id="mergefresh">Start fresh &mdash; discard this browser&rsquo;s local history</button>' +
...
function renderPanel(status) {
  ...
  if (!st.token) {
    body.innerHTML = '<button class="btn" id="synclogin">Sign in with GitHub</button>' +
      ...
      'Sync your stats and settings across devices. Sign-in required &mdash; nothing is stored without it.</div>';
