...
function loadEmbedder() {
  ...
  _pipe = import(TJS_URL).then(tjs => {
    const mk = device => tjs.pipeline('feature-extraction', EMBED_MODEL, {
      dtype: 'q8', device, progress_callback: modelProgress,
    });
    // WebGPU when the browser has it; a failed init falls back to WASM.
    return navigator.gpu ? mk('webgpu').catch(() => mk('wasm')) : mk('wasm');
