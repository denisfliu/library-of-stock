// clue_search.js — one clue-search call, two backends. CANONICAL HOME:
// github.com/qbsuite/qb-semantic-search. A vendored copy ships in
// library-of-stock (lib/js/clue_search.js) for the reader's clue panel
// + missed-clues report — any change here must be mirrored there (that
// repo's CI runs a parity test against the sync Worker's scan).
//
...
//
// The local scan is a port of the sync Worker's /search
// (library-of-stock sync/worker.js) and MUST stay in lockstep with it
// (tests/run_tests.js here + tests/semsearch in library-of-stock
// exercise the shared pieces against synthetic rows). The mode is
// per-device (localStorage, shared across pages) and deliberately NOT
// synced in prefs: the model download is a per-device commitment.
//
// Load order: after qdata.js (QDATA_BASE), before page scripts.
...
/* ---------- cloud backend ---------- */
// defaultMode is the page's fallback when the device has no stored
// choice: 'local' on qb-semantic-search (no sign-in needed), unset
// (-> cloud) on the library-of-stock reader.
let cfg = { syncBase: null, getAuth: null, defaultMode: null };
...
function mode() {
  ...
  return cfg.defaultMode === 'local' ? 'local' : 'cloud';
}
function setMode(m) {
