...
function renderQText() {
  ...
  html += readHtml;
  el.innerHTML = html;
  if (typeof qpanePin === 'function') qpanePin();   // guard: test slices stub around this
}
...
function wireUp() {
  ...
  $('fsize').oninput = e => {
    ...
    qpanePin();   // a reflow moves the pin spot
  };
...
function showView(v) {
  ...
  syncQPane();   // leaving the play view mid-question must hide the follow pill
}
...
/* Live-question follow: while a question is live on a phone the page itself
 * scrolls chat-style — each reveal keeps the newest words pinned just above
 * the fixed bottom bar. Any manual scroll away from that spot unpins: up to
 * reread earlier clues, or down into the #qspacer run-out to park the live
 * text at the top of the screen and watch it grow downward. The ↓ Latest
 * pill repins. Nothing is scroll-locked — the header scrolls away naturally.
 * Voice mode is exempt — its text is hidden until done anyway. */
let qpanePinned = true, qpaneProg = false, qpaneQid = null;

function qpaneIn(phases) {
  return !!(window.losLayout && losLayout() === 'mobile' && cur && !cur.voice
    && phases.includes(phase) && $('view-play').style.display !== 'none');
}
function qpaneActive() { return qpaneIn(['reading', 'paused']); }
// The spacer holds through buzz/grading so the page doesn't jump under the
// answer input; it leaves with the phases where scroll position matters.
function qpaneLive() { return qpaneIn(['reading', 'paused', 'buzzed', 'selfgrade']); }

// The pin scroll position: the card's bottom edge riding just above the
// bottom bar. 0 while the text still fits above the bar.
function qpinTarget() {
  const b = $('qcard').getBoundingClientRect().bottom + window.scrollY;
  return Math.max(0, Math.round(b - $('mbar').getBoundingClientRect().top + 10));
}

// Called on phase changes and layout flips: reset to a pinned, top-of-page
// view when a new question starts; otherwise re-assert the pin + pill.
function syncQPane() {
  const pill = $('qjump'); if (!pill) return;
  $('qspacer').hidden = !qpaneLive();
  if (!qpaneActive()) { pill.hidden = true; return; }
  if (qpaneQid !== cur.q.id) {
    qpaneQid = cur.q.id;
    qpanePinned = true;
    qpaneProg = true;
    window.scrollTo({ top: 0 });
    requestAnimationFrame(() => { qpaneProg = false; });
  }
  qpanePin();
  syncQJump();
}

// Keep the newest words just above the bottom bar. No-op once the user has
// scrolled away; renderQText calls this on every reveal tick.
function qpanePin() {
  if (!qpaneActive() || !qpanePinned) return;
  const t = qpinTarget();
  if (Math.abs(window.scrollY - t) < 2) return;
  qpaneProg = true;
...
function syncQJump() {
  const pill = $('qjump');
  const show = qpaneActive() && !qpanePinned;
  pill.hidden = !show;
  if (show) pill.style.bottom =
    (window.innerHeight - $('mbar').getBoundingClientRect().top + 12) + 'px';
}
...
function initMobile() {
  ...
  // Follow scroll: a manual scroll away from the pin spot (in either
  // direction) unpins the view; scrolling back near it (or the pill) repins.
  window.addEventListener('scroll', () => {
    if (!qpaneActive() || qpaneProg) return;
    qpanePinned = Math.abs(window.scrollY - qpinTarget()) <= 50;
    syncQJump();
  }, { passive: true });
  $('qjump').onclick = () => { qpanePinned = true; qpanePin(); syncQJump(); };
  window.addEventListener('resize', syncQPane);
