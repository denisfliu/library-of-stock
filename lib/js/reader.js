...
/* ---------- missed clues report (view-missed) ----------
   ...
   first (dead, then by buzz depth, deepest first). Each row's query is
   the sentence BEFORE the last-heard one — the clue that should have
   converted — re-derived from the LOG row alone: rec.sent reconstructs
...
function missedQueryFor(rec, doc) {
  ...
  const qi = Math.max(0, lastHeard - 1);
  return { query: sents[qi], qi, lastHeard, n: sents.length };
}
...
function missedCardHtml(rec, i) {
  ...
  return '<div class="mc" id="mc' + i + '">'
    ...
    + '<div class="mc-query"><span class="lbl">missed clue</span><span class="mq">loading&hellip;</span></div>'
    + '<div class="mc-sim"><div class="cluenote">queued</div></div>'
    + '</div>';
}
...
async function missedOne(rec, i, canSearch) {
  ...
  if (!card) return;
  const qEl = card.querySelector('.mc-query .mq'), simEl = card.querySelector('.mc-sim');
  ...
  try {
    ...
    if (!entry) {
      ...
      entry = { query: mq.query };
      missedCache.set(key, entry);
    }
    qEl.textContent = entry.query;
    if (!canSearch) {
      simEl.innerHTML = '<div class="cluenote">Sign in from the Sync panel, '
        + 'or switch clue search to In browser.</div>';
      return;
    }
    if (!entry.rows) {
      simEl.innerHTML = '<div class="cluenote">searching&hellip;</div>';
      const res = await losClueSearch.search(entry.query, {});
      entry.rows = await resolveClueRows(res.results || [], rec.id, MISSED_TOPN);
    }
    if (!entry.rows.length) {
      simEl.innerHTML = '<div class="cluenote">No similar clues found.</div>';
      return;
    }
    simEl.innerHTML = '<button class="simtoggle">' + (i === 0 ? '▾' : '▸') + ' '
      + entry.rows.length + ' similar clue' + (entry.rows.length > 1 ? 's' : '') + '</button>'
      + '<div class="simlist' + (i === 0 ? ' open' : '') + '">'
      + entry.rows.map(clueItemHtml).join('') + '</div>';
  } catch (e) {
    if (qEl && qEl.textContent === 'loading…') qEl.textContent = '—';
    if (simEl) simEl.innerHTML = '<div class="cluenote">'
      + (e && e.signin ? 'Sign in from the Sync panel, or switch clue search to In browser.'
