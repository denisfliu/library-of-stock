...
// pin slot, never by rank, so a re-sort never repaints a series.
const SERIES_COLS = ['#6b9eff', '#e0655f', '#b98adf', '#5dbb7a'];
const METRIC_COL = { acc: '#e8b04a', cel: '#6b9eff', neg: '#e0655f' };
const METRIC_LBL = { acc: 'Accuracy', cel: 'Celerity', neg: 'Neg rate' };
const PROG_MAX_PINS = 4;
const progState = { x: 'idx', metrics: new Set(['acc', 'cel']), W: 50, wmax: false, pins: [] };
...
function progressBlock(data) {
  ...
  h += '<div class="ctlrow" id="progctl">'
    ...
    + '<div class="ctlgrp"><span class="ctllbl">show</span>' + mbtn('acc') + mbtn('cel') + mbtn('neg') + '</div>'
...
function progSeries(data) {
  ...
  for (const m of ['acc', 'cel', 'neg']) {
    ...
    out.push({
      ...
      pts: rollup(data, win, m), band: progState.pins.length === 0,
    });
  }
  progState.pins.forEach((pin, i) => {
    ...
    out.push({
      ...
      pts: rollup(sub, subWin, 'acc'), band: false, dash: true,
    });
  });
  return out;
}
...
function drawProgress(data) {
  ...
  for (const gy of [0, 0.25, 0.5, 0.75, 1]) {
    ctx.strokeStyle = cBorder; ctx.globalAlpha = gy === 0 ? 0.6 : 0.26;
    ...
    ctx.fillText(Math.round(gy * 100) + '%', px0 - 6, Y(gy) + 3);
  }
  ...
  if (leg) leg.innerHTML = series.length < 2 ? '' : series.map(s =>
    '<span><span class="sw' + (s.dash ? ' dash' : '') + '" style="'
    + (s.dash ? 'border-color:' + s.col : 'background:' + s.col) + '"></span>'
    + esc(s.label) + (s.dash ? ' acc' : '') + '</span>').join('');
  const note = $('prognote');
  if (note) note.textContent = (progState.wmax
    ...
    + (progState.pins.length ? ' Dashed: pinned breakdown rows.' : ' Band: 95% interval.')
    + ' Faint bars: how much you played.';
}
...
function showProgTip(cv) {
  ...
  tip.innerHTML = '<div class="tth">'
    ...
    + at.map(o => '<div class="ttr"><i style="color:' + o.s.col + '">' + esc(o.s.label)
      + '</i><span>' + fmtPct(o.p.v) + '</span></div>').join('');
