...
function showView(v) {
  ...
  syncQPane();   // leaving the play view mid-question must release the scroll lock
}
...
/* Live-question window: while a question is live on a phone, the qcard
 * becomes a fixed pane between the meta strip and the bottom bar — the text
 * scrolls INSIDE it, pinned to the newest words chat-style, so the reveal
 * never runs under the bar and the buzz target never moves. Scrolling up
 * unpins (to reread earlier clues) and shows the ↓ Latest pill; the card
 * relaxes back to a normal growing card once the question resolves. Voice
 * mode is exempt — its text is hidden until done anyway. */
let qpanePinned = true, qpaneProg = false, qpaneQid = null;

function qpaneActive() {
...
// Size the live card to fill from its top edge down to the bottom bar (which
// the keyboard inset may have lifted — recomputed from its live rect). The
// answer input and prompt judge live below the text inside the card, so the
// flex column absorbs their appearance without remeasuring.
function syncQPane() {
  const card = $('qcard'), qt = $('qtext'), pill = $('qjump');
  if (!card || !pill) return;
  if (!qpaneActive()) {
    if (card.classList.contains('live')) {
      card.classList.remove('live');
      card.style.height = '';
      qt.classList.remove('scrolled');
      document.body.classList.remove('qlock');
      pill.hidden = true;
    }
    return;
  }
  if (!card.classList.contains('live') || qpaneQid !== cur.q.id) {
    qpaneQid = cur.q.id;
    card.classList.add('live');
    document.body.classList.add('qlock');
    qpanePinned = true;
    window.scrollTo({ top: 0 });
    // qlock slides the top chrome closed (CSS), moving the card's top edge.
    // transitionend remeasures; the timeout covers prefers-reduced-motion,
    // where no transition runs and no event fires.
    setTimeout(syncQPane, 300);
  }
  const h = $('mbar').getBoundingClientRect().top - card.getBoundingClientRect().top - 12;
  card.style.height = Math.max(160, Math.round(h)) + 'px';
  qpanePin();
  syncQJump();
}

// Keep the newest words in view. No-op once the user has scrolled up;
// renderQText calls this on every reveal tick.
function qpanePin() {
  const card = $('qcard'), qt = $('qtext');
  if (!card || !card.classList.contains('live')) return;
  if (qpanePinned) {
    qpaneProg = true;
    qt.scrollTop = qt.scrollHeight;
    requestAnimationFrame(() => { qpaneProg = false; });
  }
  qt.classList.toggle('scrolled', qt.scrollTop > 4);
}

function syncQJump() {
  $('qjump').hidden = qpanePinned || !(phase === 'reading' || phase === 'paused');
}
...
function initKbInset() {
  ...
  const onVV = () => {
    ...
    syncQPane();   // the live pane shrinks with the bar so the input stays visible
  };
...
function initMobile() {
  ...
  // Live-pane scroll: a manual scroll away from the bottom unpins the view;
  // scrolling back to the bottom (or the pill) repins it.
  $('qtext').addEventListener('scroll', () => {
    const qt = $('qtext');
    if (!$('qcard').classList.contains('live')) return;
    qt.classList.toggle('scrolled', qt.scrollTop > 4);
    if (qpaneProg) return;
    qpanePinned = qt.scrollHeight - qt.scrollTop - qt.clientHeight <= 50;
    syncQJump();
  });
  $('qjump').onclick = () => { qpanePinned = true; qpanePin(); syncQJump(); };
  window.addEventListener('resize', syncQPane);

  // The top chrome's qlock slide moves the card's top edge in both
  // directions — remeasure the pane when it lands.
  document.querySelector('header').addEventListener('transitionend', syncQPane);
  document.querySelector('.mchrome').addEventListener('transitionend', syncQPane);
