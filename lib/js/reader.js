...
function wireUp() {
  ...
  // Question text size. Per-device (like the clue-search backend): a scale
  // that reads well on a phone is wrong on a monitor, so it never syncs.
  const applyFsize = pct => {
    document.documentElement.style.setProperty('--qscale', pct / 100);
    $('fsize').value = pct;
    $('fsizeval').textContent = pct + '%';
  };
  let fsize = Number(localStorage.getItem(FSIZE)) || 100;
  applyFsize(fsize);
  $('fsize').oninput = e => {
