 ...
 * the fixed bottom bar, and a manual scroll up unpins so earlier clues can
 * be reread (the ↓ Latest pill repins). Nothing is scroll-locked: the header
 * scrolls away naturally and the question is readable from the top. Voice
...
// Keep the newest words just above the bottom bar (the wrap's bottom padding
// clears it). No-op once the user has scrolled up, and while the text still
// fits the viewport; renderQText calls this on every reveal tick.
function qpanePin() {
...
function initMobile() {
  ...
  // Follow scroll: a manual scroll away from the bottom unpins the view;
  // scrolling back to the bottom (or the pill) repins it.
  window.addEventListener('scroll', () => {
    if (!qpaneActive() || qpaneProg) return;
    const doc = document.scrollingElement;
    qpanePinned = doc.scrollHeight - doc.scrollTop - window.innerHeight <= 50;
    syncQJump();
  }, { passive: true });
