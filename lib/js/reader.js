...
const settings = { wpm: 380, sent: 0, drill: false, focus: 55,  // focus = raw 0..100 (broad..targeted)
  dist: { ...DEFAULT_DIST }, useDist: true, voice: false, vrate: 0.95 };
let queue = [], qpos = -1;
let cur = null;
...
function audioUrl(qid) { return AUDIO_BASE + '/tossups/' + qid.slice(0, 2) + '/' + qid + '.opus'; }
...
function savePrefs() {
  try {
    ...
    localStorage.setItem(PREFS, JSON.stringify({
      ...
      voice: settings.voice, vrate: settings.vrate,
