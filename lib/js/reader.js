...
// A spelled-out accidental written as its OWN word ("G natural", "E flat") —
// it belongs to the preceding note, so it must extend a run, not break it.
const ACCIDENTAL = /^(?:double[-\s]?)?(?:sharps?|flats?|naturals?)[”")»\],.;:!?]*$/i;
...
const NOTE_GLUE = /^(?:and|then|to|or|a|an|back|up|down|again|from|by|via)[,.;:]?$/i;
...
function slowSpans(words) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    else if (runStart >= 0 && ACCIDENTAL.test(w)) { /* belongs to prior note: extend run, don't count */ }
