// reader.js — the question reader (reader.html).
//
// Runtime data comes entirely from the R2 data plane via qdata.js:
//   catalog.json  — columnar index of every tossup (set/packet/taxonomy/difficulty)
//   topics.json   — wiki overlay: per-topic metadata + backing question ids
//   sets/{slug}.json — full question docs, fetched lazily per set and cached
//
// Everything else (reveal engine, answer checking, stats) runs client-side;
// history lives in localStorage. Tossups only for now — bonuses later.
(function () {
'use strict';

// Focus (tags) and Era facets are off for now — they only cover
// wiki-linked questions, so the Group (overview-section) facet supersedes
// them. Flip to true to re-enable the facets and their practice-tag
// shortcuts (wiki-box tag chips, stats "practice ▸" links).
const FACETS_ON = false;

const $ = id => document.getElementById(id);
const esc = s => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
// answerlines may contain only these formatting tags; escape everything else
const ANS_OK = /<\/?(b|u|i|em|strong)>/;
function answerHTML(a) { return a.split(/(<\/?(?:b|u|i|em|strong)>)/g).map(p => ANS_OK.test(p) ? p : esc(p)).join(''); }

/* ---------- era buckets (subsubcategory facet, derived from topic year) ---------- */
function eraOf(y) {
  if (y == null) return null;
  if (y < 1500) return 'Pre-1500';
  if (y < 1600) return '1500s';
  if (y < 1700) return '1600s';
  if (y < 1800) return '1700s';
  if (y < 1900) return '1800s';
  if (y < 1946) return '1900–1945';
  return 'Post-1945';
}
const ERA_ORDER = ['Pre-1500', '1500s', '1600s', '1700s', '1800s', '1900–1945', 'Post-1945'];

/* ---------- sentence splitting (for the last-n-sentences mode) ---------- */
/* Known abbreviations, plus single-letter initial(ism)s ("W.", "v.",
   "O.B.", "U.S."). MUST stay in lockstep with lib/pipeline/parse.py
   split_sentences — the sentence-embedding index keys on these boundaries. */
const ABBREV = /(?:\b(?:Mr|Mrs|Ms|Dr|St|Sts|Mt|vs|etc|Jr|Sr|Prof|Gen|Col|Fr|ca|e\.g|i\.e|No|Op|Nos)|(?:^|[^A-Za-z])(?:[A-Za-z]\.)*[A-Za-z])\.$/;
function splitSentences(text) {
  const parts = []; let buf = '';
  const tokens = text.split(/(\s+)/);
  for (let i = 0; i < tokens.length; i++) {
    buf += tokens[i];
    const t = tokens[i];
    if (/[.!?][”")\]]*$/.test(t) && !ABBREV.test(t.replace(/[”")\]]+$/, ''))) {
      const rest = tokens.slice(i + 1).join('');
      if (/^\s*[“A-Z0-9”"(]/.test(rest) || !rest.trim()) { parts.push(buf); buf = ''; }
    }
  }
  if (buf.trim()) parts.push(buf);
  return parts.map(s => s.trim()).filter(Boolean);
}

/* ---------- score-clue pacing ----------
   Reveal-unit splitting + slow note-run spans live in the SHARED
   reveal_units.js (canonical copy: qbsuite/qb-scorekeeper
   app/vendor/reveal_units.js, vendored into lib/js/ by build.py —
   edit it THERE, never here). Loaded as a classic script before this
   file (build_reader.py); unit indexes are the buzz-position/power
   contract shared with the scorekeeper. */
const { buildUnits, slowSpans, SLOW_FACTOR } = globalThis.qbRevealUnits;

/* ---------- answer checking ----------
   Grading is qbreader's own checker, vendored verbatim in answer_checker.js
   (window.qbCheckAnswer): full accept/prompt/reject clause parsing, directed
   prompts, abbreviation + underlining logic, typo tolerance. The verdict
   stays a suggestion — the reader always lets you override it. The helpers
   below only derive the drill/stats answerline KEY, not the verdict. */
function normAns(s) {
  return s.normalize('NFD').replace(/[̀-ͯ]/g, '')
    .toLowerCase().replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim()
    .replace(/^(the|a|an) /, '');
}
// underlined (<u>…</u>) spans of the raw answer = the required core(s);
// fall back to bold, then to the whole string when there's no markup.
function requiredCores(raw) {
  const grab = re => { const out = []; let m; while ((m = re.exec(raw))) out.push(m[1].replace(/<[^>]+>/g, '')); return out; };
  const u = grab(/<u>(.*?)<\/u>/gis);
  return u.length ? u : grab(/<b>(.*?)<\/b>/gis);
}
// The drill/stats answerline key: the bold-underline CORE (not the full
// accept/prompt clause), so the same answer unifies across questions that
// phrase their accept clauses differently. Falls back to the pre-bracket
// main answer when there's no markup. Mirrors publish.py answer_key so the
// catalog's answerline_values line up with each LOG entry's key.
function answerKey(raw, san) {
  const cores = requiredCores(raw || '').filter(s => s.trim());
  const base = cores.length ? cores[0]
    : (san || (raw || '').replace(/<[^>]+>/g, '')).split(/[\[(]/)[0];
  return normAns(base);
}

/* ---------- data plane ---------- */
let CAT = null;          // catalog.json
let TOPICS = null;       // topics.json .topics
let ID2SLUG = null;      // tossup id -> topic slug
let ID2ROW = null;       // tossup id -> catalog row index
let AKEYS = null;        // distinct answerline id -> normalized key (catalog)
const shardDocs = new Map();   // set slug -> Map(tossup id -> doc)

// Normalized answerline for a catalog row, keyed to match each LOG entry
// (LOG stores normAns(answer_sanitized); the catalog's answerline_values are
// the deduped sanitized answers, re-normalized here so the JS normAns is
// authoritative on both sides). '' when the column isn't published yet.
function rowAns(r) {
  const T = CAT.tossups;
  if (!AKEYS || !T.answerline) return '';
  const id = T.answerline[r];
  return id >= 0 ? (AKEYS[id] || '') : '';
}

function shardFor(setIdx) {
  const slug = CAT.sets[setIdx].slug;
  if (shardDocs.has(slug)) return Promise.resolve(shardDocs.get(slug));
  return qdataFetch('sets/' + slug + '.json').then(shard => {
    const m = new Map();
    for (const pk of shard.packets) for (const tu of pk.tossups) m.set(tu._id, tu);
    shardDocs.set(slug, m);
    return m;
  });
}

/* ---------- state ---------- */
const filters = { cats: new Set(), subs: new Set(), subsubs: new Set(), tags: new Set(), eras: new Set(), sections: new Set(), diffs: new Set([7]) };
// Subcategories whose alternate_subcategory (subtype) genuinely partitions
// them — Other Science, Other Fine Arts, Social Science, the Literature
// forms. Computed once from the corpus: real partitions sit at ~100%
// coverage, incidental mis-tags (a few VFA/AFA/Mythology rows) at ~0-2%, so
// a 50% floor cleanly separates them. Only these feed the subtype facet.
let SUBTYPED_SUBS = null;
// Static taxonomy maps (from the corpus): which category each subcategory
// belongs to, and which subtypes occur under each subcategory / category.
// These drive the hierarchical filter semantics in rowInScope.
let CAT_OF_SUB = null;    // sub name -> its category name
let ALTS_OF_SUB = null;   // sub name -> Set of subtype names occurring in it
let CATS_OF_ALT = null;   // subtype name -> Set of category names it occurs in
// A unit needs a real presence in the scope (GROUP_UNIT_FLOOR rows) to get
// a Group-facet row — the corpus has stray mis-sectioned rows (a "Math"
// question sectioned under religion) that must not surface foreign units.
const GROUP_UNIT_FLOOR = 5;
// Manual open/close overrides for the Group accordion, per unit slug.
// Session-only, and written ONLY on user toggles — auto-open decisions
// (small scope, picked sections) must stay live, not freeze as overrides.
const groupOpen = new Map();

// Pure model for the Group facet (sliced by tests/reader_facets): which
// units and section ids to offer, given the scope's per-section counts.
// units = [unit, ids] pairs, row-count order; ids count-sorted; a section
// needs >=3 rows (or an active pick) so a couple of stray mis-sectioned
// rows can't surface chips; a unit below GROUP_UNIT_FLOOR shows only if
// one of its sections is picked. show requires SOME taxonomy narrowing —
// the unfiltered corpus spans every unit and would just be noise.
function groupFacetModel(secCount, SEC) {
  const secIds = Object.keys(secCount).map(Number);
  const sectioned = secIds.filter(id => id >= 0)
    .filter(id => secCount[id] >= 3 || filters.sections.has(id))
    .sort((a, b) => secCount[b] - secCount[a]);
  const unitRows = {};
  for (const id of secIds) if (id >= 0 && SEC[id])
    unitRows[SEC[id][0]] = (unitRows[SEC[id][0]] || 0) + secCount[id];
  const byUnit = new Map();
  for (const id of sectioned) {
    const u = SEC[id] ? SEC[id][0] : null;
    if (!u) continue;
    if (!byUnit.has(u)) byUnit.set(u, []);
    byUnit.get(u).push(id);
  }
  const units = [...byUnit.entries()]
    .filter(([u, ids]) => unitRows[u] >= GROUP_UNIT_FLOOR || ids.some(id => filters.sections.has(id)))
    .sort((a, b) => unitRows[b[0]] - unitRows[a[0]]);
  const narrowed = !!(filters.cats.size || filters.subs.size || filters.subsubs.size);
  return { show: narrowed && units.length >= 1 && sectioned.length > 1, units, unitRows };
}
function computeSubtypedSubs() {
  const T = CAT.tossups, n = T.id.length, tot = {}, have = {}, pair = {};
  CAT_OF_SUB = {}; ALTS_OF_SUB = {}; CATS_OF_ALT = {};
  for (let r = 0; r < n; r++) {
    const s = subName(T.subcategory[r]); if (!s) continue;
    tot[s] = (tot[s] || 0) + 1;
    CAT_OF_SUB[s] = catName(T.category[r]);
    if (T.alternate_subcategory[r] >= 0) {
      have[s] = (have[s] || 0) + 1;
      const k = s + '\x00' + altName(T.alternate_subcategory[r]);
      pair[k] = (pair[k] || 0) + 1;
    }
  }
  SUBTYPED_SUBS = new Set();
  for (const s in tot) if ((have[s] || 0) >= 30 && have[s] / tot[s] >= 0.5) SUBTYPED_SUBS.add(s);
  // taxonomy maps only from real partitions: subtype-bearing subs and
  // (sub, subtype) pairs with a genuine presence — a lone mis-tagged row
  // (a Mythology question marked "Math") must not teach the hierarchy
  // that Math lives under Mythology.
  for (const k in pair) {
    const [s, a] = k.split('\x00');
    if (!SUBTYPED_SUBS.has(s) || pair[k] < 10) continue;
    (ALTS_OF_SUB[s] = ALTS_OF_SUB[s] || new Set()).add(a);
    (CATS_OF_ALT[a] = CATS_OF_ALT[a] || new Set()).add(CAT_OF_SUB[s]);
  }
}

// Hierarchical filter semantics: a sub/subtype pick narrows only its own
// category — a category selected with no descendants picked stays whole, so
// "CS + Math" can sit next to a whole Mythology (or next to Opera under Fine
// Arts) instead of a subtype pick strangling every row that lacks one.
// Derived from the current filters; refreshed before every scope walk.
let DESC_CATS = new Set();        // categories with a selected sub/subtype
let CONSTRAINED_SUBS = new Set(); // subs with >=1 of their subtypes selected
function refreshBranchScopes() {
  if (!CAT) return;
  if (!SUBTYPED_SUBS) computeSubtypedSubs();
  DESC_CATS = new Set([...filters.subs].map(s => CAT_OF_SUB[s]).filter(Boolean));
  for (const a of filters.subsubs) for (const c of (CATS_OF_ALT[a] || [])) DESC_CATS.add(c);
  CONSTRAINED_SUBS = new Set();
  for (const s in ALTS_OF_SUB)
    if ([...filters.subsubs].some(a => ALTS_OF_SUB[s].has(a))) CONSTRAINED_SUBS.add(s);
}
// Taxonomy membership under those semantics; c/s/a are the row's names.
function taxPass(c, s, a) {
  if (filters.cats.size && !filters.cats.has(c)) return false;
  if (!DESC_CATS.has(c)) return true;   // no descendant picks in this category
  if (a != null && filters.subsubs.has(a)) return true;
  return filters.subs.has(s) && !CONSTRAINED_SUBS.has(s);
}
// Deselecting a category orphans its sub/subtype picks — drop them so they
// can't keep filtering the queue from a hidden facet.
function pruneOrphanedPicks() {
  if (!CAT) return;
  if (!SUBTYPED_SUBS) computeSubtypedSubs();
  if (filters.cats.size) {
    for (const s of [...filters.subs]) if (!filters.cats.has(CAT_OF_SUB[s])) filters.subs.delete(s);
    for (const a of [...filters.subsubs])
      if (![...(CATS_OF_ALT[a] || [])].some(c => filters.cats.has(c))) filters.subsubs.delete(a);
  } else { filters.subs.clear(); filters.subsubs.clear(); }
}
// Standard quizbowl category distribution (relative weights, ~per 20-tossup
// packet) — what qbreader samples by, instead of the raw corpus frequency.
// Pop Culture is 0 by default (excluded from the academic distribution); the
// user can raise it. Keys must match catalog category values.
const DEFAULT_DIST = {
  'Literature': 4, 'History': 4, 'Science': 4, 'Fine Arts': 3,
  'Mythology': 1, 'Religion': 1, 'Philosophy': 1, 'Social Science': 1,
  'Geography': 1, 'Current Events': 0.5, 'Other Academic': 0.5, 'Pop Culture': 0,
};
// distSub: sparse nested splits under a category / subcategory, keyed by the
// parent's name (category and subcategory names are globally unique) —
// distSub['Science'] = { Biology: 2, ... } splits Science's share across its
// subcategories; distSub['Other Science'] splits that sub across its
// subtypes. A missing group or child means weight 1 (even split).
const settings = { wpm: 380, sent: 0, drill: false, focus: 55,  // focus = raw 0..100 (broad..targeted)
  dist: { ...DEFAULT_DIST }, distSub: {}, useDist: true, voice: false, vrate: 0.95, multibuzz: false };
let queue = [], qpos = -1;
let cur = null;
let phase = 'idle';      // idle|loading|reading|paused|buzzed|selfgrade|done
let wordIdx = 0, tick = null, graceTimer = null, graceAnim = null;
let buzzAt = null;
let history = [], reviewIdx = -1, liveSnap = null;   // previous-question review

const STORE = 'losReaderStatsV1';
let LOG = [];
try { LOG = JSON.parse(localStorage.getItem(STORE) || '[]'); } catch (e) {}
// Migrate v1 entries: old records stored `pct` (buzz depth 0–100) instead of
// `bf` (buzz fraction 0–1); section/diff/ans/pow are simply absent (treated
// as unknown by the aggregators). Pre-sync records also lack `ts` — they get
// small synthetic ordinals so cross-device dedup (keyed id+ts) stays unique
// while preserving their order.
let _synthTs = 0;
for (const r of LOG) {
  if (r.bf === undefined) r.bf = (r.pct != null) ? r.pct / 100 : null;
  if (typeof r.ts !== 'number') r.ts = ++_synthTs;
}
function saveLog() { try { localStorage.setItem(STORE, JSON.stringify(LOG)); } catch (e) {} }
// LOG rows recorded since this page load — the missed-clues report scope.
const sessionRows = [];

/* ---------- cross-device sync surface (lib/js/sync.js) ----------
   sync.js is pure transport: it pushes LOG records to the backend and feeds
   remote records back through mergeLog. Records carry a private `_p` flag
   ("pushed") that the stats/drill aggregators ignore. */
window.losReaderHook = {
  getLog: () => LOG,
  save: saveLog,
  onRecord: null,   // sync.js assigns; fired after each attempt is recorded
  // Merge remote records into LOG: dedupe by (id, ts), keep chronological
  // order so the SRS replay stays deterministic across devices.
  mergeLog(entries) {
    const seen = new Set(LOG.map(r => r.id + '/' + r.ts));
    let added = 0;
    for (const r of entries) {
      if (r.bf === undefined) r.bf = (r.pct != null) ? r.pct / 100 : null;
      const k = r.id + '/' + r.ts;
      if (seen.has(k)) continue;
      seen.add(k); LOG.push(r); added++;
    }
    if (added) {
      LOG.sort((a, b) => (a.ts || 0) - (b.ts || 0));
      saveLog();
      if (settings.drill && CAT) rebuildQueue();
      if ($('view-stats').style.display !== 'none') renderStats();
    }
    return added;
  },
};

/* ---------- clue-search engine wiring (lib/js/clue_search.js) ----------
   Providers are functions: sync.js loads after this file and assigns
   hook.searchBase/searchAuth, so they must be read at call time. */
losClueSearch.configure({
  syncBase: () => window.losReaderHook.searchBase,
  getAuth: () => window.losReaderHook.searchAuth ? window.losReaderHook.searchAuth() : null,
});
function syncCsUI() {
  const m = losClueSearch.mode();
  for (const b of $('csmode').querySelectorAll('button'))
    b.classList.toggle('on', b.dataset.m === m);
}

/* ---------- filtering over the catalog ---------- */
const catName = i => i >= 0 ? CAT.category_values[i] : null;
const subName = i => i >= 0 ? CAT.subcategory_values[i] : null;
const altName = i => i >= 0 ? CAT.alternate_subcategory_values[i] : null;

function rowInScope(r, upTo) {
  const T = CAT.tossups;
  // Read-aloud restricts the whole corpus to questions that have audio, so
  // every facet count and the queue reflect only playable questions.
  if (audioMode() && !hasAudio(T.id[r])) return false;
  const c = catName(T.category[r]);
  if (filters.cats.size && !filters.cats.has(c)) return false;
  if (upTo === 'cat') return true;
  if (!taxPass(c, subName(T.subcategory[r]), altName(T.alternate_subcategory[r]))) return false;
  if (filters.diffs.size && !filters.diffs.has(T.difficulty[r])) return false;
  if (upTo === 'sub') return true;
  // Overview-section filter: T.section is a catalog column, so it applies
  // to every question, wiki-linked or not (unlike tags/eras below).
  if (filters.sections.size && T.section && !filters.sections.has(T.section[r])) return false;
  if (filters.tags.size || filters.eras.size) {
    const slug = ID2SLUG.get(T.id[r]);
    if (!slug) return false;
    const t = TOPICS[slug];
    if (filters.tags.size && !(t.tags || []).some(x => filters.tags.has(x))) return false;
    if (filters.eras.size && !filters.eras.has(eraOf(t.year))) return false;
  }
  return true;
}

function chip(label, count, on) {
  const b = document.createElement('button');
  b.className = 'chip' + (on ? ' on' : '');
  b.innerHTML = esc(label) + (count != null ? ' <span class="n">' + count + '</span>' : '');
  return b;
}
function renderChipSet(el, entries, set, onChange) {
  el.innerHTML = '';
  for (const [label, count] of entries) {
    const c = chip(label, count, set.has(label));
    c.onclick = () => { set.has(label) ? set.delete(label) : set.add(label); onChange(); };
    el.appendChild(c);
  }
}

function renderFilters() {
  if (!CAT) return;   // clicks before the catalog loads
  refreshBranchScopes();
  const T = CAT.tossups, n = T.id.length;
  const SEC = CAT.section_values || [];
  // Dynamic faceted counts: each facet's chip number = how many questions
  // it would represent GIVEN the other active filters, so counts react as
  // you pick a difficulty, category, etc. Each facet is restricted by its
  // ancestors plus the orthogonal difficulty filter, but NOT by its own
  // selection (so you can still see and multi-select siblings). Taxonomy
  // membership is hierarchical (taxPass): sub/subtype picks narrow only
  // their own category, so cross-branch mixes (CS + Math next to Opera or a
  // whole Mythology) compose instead of strangling each other. Difficulty
  // is restricted by the whole taxonomy above it, so it reacts symmetrically.
  const catCount = {}, subCount = {}, subsubCount = {}, secCount = {}, diffCount = {};
  const hasSec = filters.sections.size, hasDiff = filters.diffs.size;
  for (let r = 0; r < n; r++) {
    const c = catName(T.category[r]);
    if (!c) continue;
    // Read-aloud: every facet count reflects only audio-backed questions, so
    // the chip numbers stay consistent with the audio-restricted scope total.
    if (audioMode() && !hasAudio(T.id[r])) continue;
    const s = subName(T.subcategory[r]);
    const a = altName(T.alternate_subcategory[r]);
    const sec = T.section ? T.section[r] : -1;
    const d = T.difficulty[r];
    const pCat = !filters.cats.size || filters.cats.has(c);
    const pTax = taxPass(c, s, a);   // full hierarchical taxonomy membership
    const pSec = !hasSec || filters.sections.has(sec);
    const pDiff = !hasDiff || filters.diffs.has(d);
    if (pDiff) catCount[c] = (catCount[c] || 0) + 1;
    if (pCat && pDiff && s) subCount[s] = (subCount[s] || 0) + 1;
    // Only subtype-bearing subcategories feed the subtype facet, so a sibling
    // without subtypes (e.g. Biology next to Other Science) can't dilute it.
    // Gated by category only (not sub/subtype picks): subtype chips stay
    // offered for the whole category scope so picks compose across branches.
    if (pCat && pDiff && a && SUBTYPED_SUBS.has(s))
      subsubCount[a] = (subsubCount[a] || 0) + 1;
    if (T.section && pTax && pDiff)
      secCount[sec] = (secCount[sec] || 0) + 1;
    if (pTax && pSec && d > 0)
      diffCount[d] = (diffCount[d] || 0) + 1;
  }
  // tag/era facets exist only for wiki-linked questions; count via topics
  const tagCount = {}, eraCount = {};
  for (const slug in TOPICS) {
    const t = TOPICS[slug];
    let m = 0;
    for (const id of (t.tossups || [])) {
      const r = ID2ROW.get(id);
      if (r != null && rowInScope(r, 'sub')) m++;
    }
    if (!m) continue;
    for (const tag of (t.tags || [])) tagCount[tag] = (tagCount[tag] || 0) + m;
    const e = eraOf(t.year); if (e) eraCount[e] = (eraCount[e] || 0) + m;
  }
  const bySize = o => Object.entries(o).sort((a, b) => b[1] - a[1]);
  renderChipSet($('f-cats'), bySize(catCount), filters.cats, onFilter);

  const subs = bySize(subCount).filter(e => e[1] >= 10 && !filters.cats.has(e[0]));
  $('subs-head').style.display = filters.cats.size && subs.length > 1 ? '' : 'none';
  renderChipSet($('f-subs'), filters.cats.size ? subs : [], filters.subs, onFilter);

  // Subtype (alternate_subcategory) facet. subsubCount already holds only
  // rows from subtype-bearing subcategories (SUBTYPED_SUBS — Other Science,
  // Other Fine Arts, Social Science, the Literature forms), so the gate is
  // just "a category is chosen and there are >=2 real subtypes in scope".
  // This shows the subtypes for a subtype-bearing subcategory even when it's
  // selected alongside siblings that have none (the old aggregate coverage
  // ratio hid the facet in that case).
  const subsubEntries = bySize(subsubCount).filter(e => e[1] >= 5);
  const showSubsub = filters.cats.size > 0 && subsubEntries.length >= 2;
  if (showSubsub) {
    // keep an active selection visible even if it dips below the threshold
    for (const s of filters.subsubs)
      if (!subsubEntries.some(e => e[0] === s)) subsubEntries.push([s, subsubCount[s] || 0]);
    renderChipSet($('f-subsub'), subsubEntries, filters.subsubs, onFilter);
  } else if (filters.subsubs.size) {
    // scope no longer supports the full facet, but active picks still filter
    // the queue — keep them visible and removable, never wipe them silently
    renderChipSet($('f-subsub'), [...filters.subsubs].map(a2 => [a2, subsubCount[a2] || 0]),
      filters.subsubs, onFilter);
  } else {
    $('f-subsub').innerHTML = '';
  }
  $('subsub-head').style.display = (showSubsub || filters.subsubs.size) ? '' : 'none';

  // Focus (tags) and Era facets — gated by the module-level FACETS_ON.
  if (FACETS_ON) {
    const tags = bySize(tagCount).filter(e => e[1] >= 6).slice(0, 22);
    for (const t of filters.tags) if (!tags.some(e => e[0] === t)) tags.push([t, tagCount[t] || 0]);
    $('tags-head').style.display = tags.length ? '' : 'none';
    renderChipSet($('f-tags'), tags, filters.tags, onFilter);

    const eras = ERA_ORDER.filter(e => eraCount[e] >= 6).map(e => [e, eraCount[e]]);
    $('eras-head').style.display = eras.length > 1 ? '' : 'none';
    renderChipSet($('f-eras'), eras, filters.eras, onFilter);
  } else {
    filters.tags.clear(); filters.eras.clear();
    $('tags-head').style.display = 'none'; $('f-tags').innerHTML = '';
    $('eras-head').style.display = 'none'; $('f-eras').innerHTML = '';
  }

  // Group (overview section) facet — one accordion row per overview unit,
  // so a multi-unit scope stays coherent at any size (the old flat chip row
  // needed a 4-unit cap that HID the whole facet past it — Denis's bug).
  // A single-unit scope renders flat chips, no header noise. Shown once the
  // taxonomy is narrowed at all; the unfiltered whole-corpus scope spans
  // ~40 units and stays hidden. Auto-open: everything at <=2 units, any
  // unit with picks; manual open/close sticks for the session (groupOpen).
  const gm = groupFacetModel(secCount, SEC);
  const gEl = $('f-groups');
  gEl.innerHTML = '';
  const secChip = (parent, id, label, count) => {
    const c = chip(label, count, filters.sections.has(id));
    if (id === -1) c.title = 'Rare / uncategorized answerlines';
    c.onclick = () => { filters.sections.has(id) ? filters.sections.delete(id) : filters.sections.add(id); onFilter(); };
    parent.appendChild(c);
  };
  const unitName = u => u.replace(/_/g, ' ').replace(/\b[a-z]/g, ch => ch.toUpperCase());
  if (gm.show && gm.units.length === 1) {
    gEl.className = 'chips';
    for (const id of gm.units[0][1]) secChip(gEl, id, SEC[id] ? SEC[id][1] : '?', secCount[id]);
    if (secCount[-1]) secChip(gEl, -1, 'Unsectioned', secCount[-1]);
  } else if (gm.show) {
    gEl.className = 'groupacc';
    for (const [u, ids] of gm.units) {
      const picked = ids.filter(id => filters.sections.has(id)).length;
      const det = document.createElement('details');
      det.open = groupOpen.has(u) ? groupOpen.get(u) : (gm.units.length <= 2 || picked > 0);
      const sum = document.createElement('summary');
      sum.innerHTML = esc(unitName(u))
        + (picked ? ' <span class="picked">' + picked + ' picked</span>' : '')
        + ' <span class="n">' + gm.unitRows[u] + '</span>';
      // record only USER toggles (a details `toggle` listener would also
      // capture the auto-open state above and freeze it as an override)
      sum.addEventListener('click', () => { groupOpen.set(u, !det.open); });
      det.appendChild(sum);
      const row = document.createElement('div');
      row.className = 'chips';
      for (const id of ids) secChip(row, id, SEC[id] ? SEC[id][1] : '?', secCount[id]);
      det.appendChild(row);
      gEl.appendChild(det);
    }
    if (secCount[-1]) {
      const extra = document.createElement('div');
      extra.className = 'chips groupextra';
      secChip(extra, -1, 'Unsectioned', secCount[-1]);
      gEl.appendChild(extra);
    }
  } else if (filters.sections.size) {
    // facet hidden (nothing narrowed), but active picks still filter the
    // queue — keep them visible and removable, never wipe them silently
    gEl.className = 'chips';
    for (const id of [...filters.sections].sort((a, b) => (secCount[b] || 0) - (secCount[a] || 0)))
      secChip(gEl, id, id === -1 ? 'Unsectioned' : (SEC[id] ? SEC[id][1] : '?'), secCount[id] || 0);
  }
  $('groups-head').style.display = (gm.show || filters.sections.size) ? '' : 'none';

  const diffs = Object.keys(diffCount).map(Number).sort((a, b) => a - b)
    .filter(d => d > 0).map(d => [String(d), diffCount[d]]);
  $('f-diffs').innerHTML = '';
  for (const [d, cnt] of diffs) {
    const c = chip(d, cnt, filters.diffs.has(Number(d)));
    c.onclick = () => { const v = Number(d); filters.diffs.has(v) ? filters.diffs.delete(v) : filters.diffs.add(v); onFilter(); };
    $('f-diffs').appendChild(c);
  }
  $('scopecount').innerHTML = '<b>' + queueSize().toLocaleString() + '</b> questions';
  updateScopeBadge();
}

function candidateRows() {
  const T = CAT.tossups;
  refreshBranchScopes();
  // facet filters restrict to wiki-linked questions; walk those ids directly
  if (filters.tags.size || filters.eras.size) {
    const rows = [];
    for (const slug in TOPICS) {
      const t = TOPICS[slug];
      if (filters.tags.size && !(t.tags || []).some(x => filters.tags.has(x))) continue;
      if (filters.eras.size && !filters.eras.has(eraOf(t.year))) continue;
      for (const id of (t.tossups || [])) {
        const r = ID2ROW.get(id);
        if (r != null && rowInScope(r, 'all')) rows.push(r);
      }
    }
    return rows;
  }
  const rows = [];
  for (let r = 0; r < T.id.length; r++) if (rowInScope(r, 'all')) rows.push(r);
  return rows;
}
function queueSize() { return candidateRows().length; }

function onFilter() { pruneOrphanedPicks(); renderFilters(); rebuildQueue(); savePrefs(); }

/* ---------- drill: buzz-graded spaced repetition ----------
   Anki without a wall clock: "time" is measured in attempts played, not
   days (LOG.length is the logical clock). Each ANSWERLINE carries an
   FSRS-lite state {stab, diff, reps, lapses, last} derived by replaying
   your whole history in order:

     - stab (stability) = the review interval, in attempts, at which
       retrievability decays to TARGET_R. It multiplies up on every pass
       (bigger jumps for early/quick buzzes) and collapses on a miss, so a
       well-known answer drifts to long intervals and a shaky one stays
       close — the schedule reviews strengths occasionally and weaknesses
       constantly from one mechanism.
     - diff (intrinsic difficulty) eases on quick correct buzzes, rises on
       misses (a neg is worse than a dead — you had a misconception).

   The grade is read straight off the buzz: celerity IS the review quality
   (power/early -> Easy -> big stability jump; giveaway -> Hard -> small).

   A candidate question's weight blends three drives:
     - DUE:  how overdue its answerline is (retrievability below TARGET_R).
     - WEAK: its answerline's difficulty and lapse count.
     - NEW:  unseen answerlines, steered toward your weak areas via a
       hierarchical section/subcategory weakness prior (cold-start).
   The focus slider morphs the blend (broad = schedule-driven, revisits
   strengths on time; targeted = hammer the worst items) and doubles as a
   sampling temperature. Sampling is Efraimidis–Spirakis (no replacement). */
const GAMMA = 0.5, PRIOR_M = 5, EXPLORE = 0.4;
const STAB0 = 2, STAB_MIN = 1, STAB_MAX = 500, DIFF0 = 5;
const FACTOR = 9, TARGET_R = 0.9, MIN_GAP = 8;
// per-attempt AREA weakness (drives cold-start priors): miss = 1.0, a
// correct buzz = GAMMA x buzz-fraction (a late correct is partially weak).
function _attemptWeakness(r) {
  return r.res === 'C' ? GAMMA * (r.bf == null ? 0.5 : r.bf) : 1.0;
}
const _clamp = (x, lo, hi) => Math.max(lo, Math.min(hi, x));
// Fold one attempt into an answerline's FSRS-lite state.
function _applyReview(st, r) {
  if (r.res === 'C') {
    const q = _clamp(1 - (r.bf == null ? 0.5 : r.bf), 0, 1);  // celerity = grade
    st.diff = _clamp(st.diff - 0.15 * (q - 0.4), 1, 10);
    const growth = 1 + (11 - st.diff) / 10 * (0.4 + q);       // ~1.1 .. ~2.4
    st.stab = Math.min(STAB_MAX, st.stab * growth);
  } else {
    st.diff = _clamp(st.diff + (r.res === 'W' ? 1.1 : 0.8), 1, 10);  // neg worse
    st.stab = Math.max(STAB_MIN, st.stab * 0.35);
    st.lapses++;
  }
  st.reps++;
}
function masteryModel() {
  const C = LOG.length;
  const item = new Map();                 // answerline -> FSRS-lite state
  let sumAll = 0, nAll = 0;
  const sec = new Map(), sub = new Map();
  const addP = (m, k, w) => { if (k == null) return; let a = m.get(k); if (!a) { a = { s: 0, n: 0 }; m.set(k, a); } a.s += w; a.n++; };
  for (let i = 0; i < LOG.length; i++) {
    const r = LOG[i];
    const w = _attemptWeakness(r); sumAll += w; nAll++;
    addP(sec, r.section, w); addP(sub, r.sub, w);
    const key = r.ans;
    if (key) {
      let st = item.get(key);
      if (!st) { st = { stab: STAB0, diff: DIFF0, reps: 0, lapses: 0, last: i }; item.set(key, st); }
      _applyReview(st, r);
      st.last = i;
    }
  }
  const W0 = nAll ? sumAll / nAll : 0.5;
  const prior = (m, k) => { const a = m.get(k); return a ? (a.s + PRIOR_M * W0) / (a.n + PRIOR_M) : W0; };
  return { C, W0, item, secWk: k => prior(sec, k), subWk: k => prior(sub, k) };
}
function drillWeight(row, M, mix) {
  const T = CAT.tossups;
  const sid = T.section[row];
  const S = sid >= 0 && CAT.section_values[sid] ? CAT.section_values[sid][1] : null;
  const subN = subName(T.subcategory[row]);
  const key = rowAns(row);
  const st = key ? M.item.get(key) : null;
  if (!st) {                               // unseen answerline: explore weak areas
    const prior = (S ? M.secWk(S) : M.W0) * (subN ? M.subWk(subN) : M.W0);
    return mix.wNew * (0.5 + prior) + EXPLORE;
  }
  const elapsed = M.C - st.last;
  if (elapsed < MIN_GAP) return 1e-3;      // just saw this answerline — hold off
  const R = 1 / (1 + elapsed / (FACTOR * st.stab));
  const due = Math.max(0, TARGET_R - R);            // overdue amount (0..0.9)
  const weak = (st.diff / 10) * (1 + 0.15 * st.lapses);
  return Math.max(mix.wDue * due + mix.wWeak * weak + 1e-3, 1e-4);
}

// Pure model for distribution sampling (sliced by tests/reader_dist): the
// per-row sampling weight under the nested weight chain
//   w(cat) * w(sub | cat) * w(alt | sub),
// each level normalized over the cells present in `cells`, so absent or
// filtered-out branches redistribute to their in-scope siblings (same as the
// old category-only behavior). cells[i] = { c, s, a } — s/a are '' when the
// row has none; the alt level only exists for subtype-bearing subs
// (`subtyped`). Weights come from `dist` (flat category weights) and
// `distSub` (sparse per-parent child splits; a missing group or child means
// weight 1 = even split). A zero weight excludes the branch unless it is
// explicitly picked in the scope filters (`picked`) — the same escape hatch
// the category level always had. Returns one weight per cell (null =
// excluded), already divided by the cell's row count so every row of a cell
// gets an equal slice of the cell's share.
function distWeights(cells, { dist, distSub, subtyped, picked }) {
  const catW = c => {
    let w = Math.max(0, dist[c] || 0);
    if (picked.cats.has(c) && w === 0) w = 1;
    return w;
  };
  const splitW = (parent, name, isPicked) => {
    const g = distSub[parent];
    let w = g && typeof g[name] === 'number' ? Math.max(0, g[name]) : 1;
    if (isPicked && w === 0) w = 1;
    return w;
  };
  const subW = (c, s) => s ? splitW(c, s, picked.subs.has(s)) : 1;
  const altW = (s, a) => a ? splitW(s, a, picked.subsubs.has(a)) : 1;
  const key = (c, s, a) => c + '\x00' + s + '\x00' + (subtyped.has(s) ? a : '');
  const catSeen = {}, subSeen = {}, altSeen = {}, cellCnt = {};
  for (const { c, s, a } of cells) {
    catSeen[c] = 1;
    (subSeen[c] = subSeen[c] || {})[s] = 1;
    if (subtyped.has(s)) (altSeen[s] = altSeen[s] || {})[a] = 1;
    const k = key(c, s, a);
    cellCnt[k] = (cellCnt[k] || 0) + 1;
  }
  let catDen = 0; for (const c in catSeen) catDen += catW(c);
  const subDen = {}, altDen = {};
  for (const c in subSeen) { let t = 0; for (const s in subSeen[c]) t += subW(c, s); subDen[c] = t; }
  for (const s in altSeen) { let t = 0; for (const a in altSeen[s]) t += altW(s, a); altDen[s] = t; }
  return cells.map(({ c, s, a }) => {
    let w = catDen > 0 ? catW(c) : 1;   // every category zeroed -> uniform shares
    if (w <= 0) return null;
    const sw = subW(c, s);
    if (subDen[c] > 0) { if (sw <= 0) return null; w *= sw / subDen[c]; }
    if (subtyped.has(s)) {
      const aw = altW(s, a);
      if (altDen[s] > 0) { if (aw <= 0) return null; w *= aw / altDen[s]; }
    }
    return w / cellCnt[key(c, s, a)];
  });
}

function rebuildQueue() {
  const rows = candidateRows();
  const T = CAT.tossups;
  if (settings.drill && LOG.length) {
    // focus 0 (broad) .. 1 (targeted): morphs the drive blend and the
    // sampling temperature together.
    const M = masteryModel();
    const f = settings.focus / 100;
    const tau = 2.5 - f * 2.1;
    const mix = { wDue: 1.0 - 0.6 * f, wWeak: 0.3 + 1.2 * f, wNew: 0.2 + 0.8 * (1 - f) };
    queue = rows.map(r => {
      const w = Math.pow(drillWeight(r, M, mix), 1 / tau);
      return [Math.pow(Math.random(), 1 / w), r];
    }).sort((a, b) => b[0] - a[0]).map(x => x[1]);
  } else if (settings.useDist) {
    // qbreader-style distribution sampling: category shares follow
    // settings.dist (default = standard quizbowl mix), and each category's
    // share splits across its subcategories — and their subtypes, for
    // subtype-bearing subs — by settings.distSub (untouched levels split
    // evenly). distWeights normalizes over what's actually in scope and
    // keeps explicitly picked branches alive at zero weight; a mild
    // unseen-first preference on top.
    if (!SUBTYPED_SUBS) computeSubtypedSubs();
    const seen = new Set(LOG.map(rec => rec.id));
    const cells = [], cellRow = [];
    for (const r of rows) {
      const c = catName(T.category[r]); if (!c) continue;
      cells.push({ c, s: subName(T.subcategory[r]) || '', a: altName(T.alternate_subcategory[r]) || '' });
      cellRow.push(r);
    }
    const ws = distWeights(cells, {
      dist: settings.dist, distSub: settings.distSub, subtyped: SUBTYPED_SUBS,
      picked: { cats: filters.cats, subs: filters.subs, subsubs: filters.subsubs },
    });
    const scored = [];
    for (let i = 0; i < ws.length; i++) {
      if (ws[i] == null) continue;
      const w = ws[i] * (seen.has(T.id[cellRow[i]]) ? 0.5 : 1);
      scored.push([Math.pow(Math.random(), 1 / w), cellRow[i]]);
    }
    queue = scored.sort((a, b) => b[0] - a[0]).map(x => x[1]);
  } else {
    const seen = new Set(LOG.map(rec => rec.id));
    const shuffle = a => { for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [a[i], a[j]] = [a[j], a[i]]; } return a; };
    const unseen = [], done = [];
    for (const r of rows) (seen.has(T.id[r]) ? done : unseen).push(r);
    queue = shuffle(unseen).concat(shuffle(done));
  }
  qpos = -1;
}

/* ---------- reading engine ---------- */
function setStatus(cls, text) { const s = $('m-status'); s.className = 'status ' + cls; s.textContent = text; }
function msPerWord() { return 60000 / settings.wpm; }

function loadNext() {
  if (!CAT) return;   // Start pressed before the catalog loads
  if (reviewIdx >= 0) {   // leaving review: snap back to the live question first
    cur = liveSnap.cur; buzzAt = liveSnap.buzzAt; wordIdx = liveSnap.wordIdx; phase = liveSnap.phase;
    liveSnap = null; reviewIdx = -1; $('reviewbar').style.display = 'none';
  }
  clearTimers();
  pushHistory();          // stash the question we're leaving for later review
  qpos++;
  if (qpos >= queue.length) {
    $('qtext').innerHTML = '<div class="empty">No more questions in this scope. Widen the filters, or clear your history in My stats to replay.</div>';
    phase = 'idle'; updateButtons(); return;
  }
  const r = queue[qpos];
  const T = CAT.tossups;
  phase = 'loading';
  $('qtext').innerHTML = '<div class="empty">Loading&hellip;</div>';
  $('answerrow').classList.remove('show');
  $('judge').classList.remove('show');
  $('wikibox').classList.remove('show');
  $('selfgrade').style.display = 'none';
  $('timerfill').style.width = '0%';
  setStatus('', '');
  updateButtons();
  const myPos = qpos;
  shardFor(T.set[r]).then(docs => {
    if (qpos !== myPos || phase !== 'loading') return;  // user moved on
    const doc = docs.get(T.id[r]);
    if (!doc) { loadNext(); return; }
    startQuestion(r, doc);
    prefetchNext();
  }).catch(err => {
    if (qpos !== myPos) return;
    phase = 'idle'; updateButtons();
    $('qtext').innerHTML = qdataErrorHtml(err);
  });
}

function prefetchNext() {
  const nxt = queue[qpos + 1];
  if (nxt == null) return;
  shardFor(CAT.tossups.set[nxt]).catch(() => {});
  if (audioMode()) prefetchAudio(CAT.tossups.id[nxt]);   // warm next audio while this one plays
}

function startQuestion(r, doc) {
  const T = CAT.tossups;
  const set = CAT.sets[T.set[r]];
  const q = {
    id: doc._id,
    text: doc.question_sanitized || doc.question,
    a: doc.answer || doc.answer_sanitized,
    as: doc.answer_sanitized || doc.answer,
    cat: doc.category || catName(T.category[r]) || '',
    sub: doc.subcategory || subName(T.subcategory[r]) || '',
    subsub: doc.alternate_subcategory || altName(T.alternate_subcategory[r]) || '',
    d: T.difficulty[r], setName: set.name, year: set.year,
    slug: ID2SLUG.get(doc._id) || null,
    sectionId: (T.section && T.section[r] >= 0) ? T.section[r] : -1,
    section: (T.section && T.section[r] >= 0 && CAT.section_values[T.section[r]])
      ? CAT.section_values[T.section[r]][1] : null,
  };
  const sents = splitSentences(q.text);
  const nSent = settings.sent;
  const startSent = (nSent > 0 && sents.length > nSent) ? sents.length - nSent : 0;
  const skippedText = sents.slice(0, startSent).join(' ');
  const readText = sents.slice(startSent).join(' ');
  const skipWords = skippedText ? skippedText.split(/\s+/).length : 0;
  const units = buildUnits(readText.split(/\s+/));
  // dispWords spans the WHOLE question (skipped sentences first) because the
  // audio does too — the sidecar map is expressed in those indexes.
  const dispWords = (skippedText ? skippedText.split(/\s+/) : []).concat(readText.split(/\s+/));
  cur = { q, units, skipWords, startSent, sents, slow: slowSpans(units.map(u => u.t)), showSkipped: false, prompted: false, userRaw: null,
    firstBuzzAt: null, negged: false, lateCorrect: false,
    dispWords, unitStarts: wordUnitStarts(units), audioMap: null, mapReady: null, audioStart: 0,
    voice: settings.voice && voiceAvailable() };
  wordIdx = 0; buzzAt = null;
  if (cur.voice) cur.mapReady = attachSidecar();
  $('m-set').textContent = q.setName;
  $('m-diff').textContent = 'diff ' + q.d;
  $('m-cat').textContent = q.cat + (q.sub && q.sub !== q.cat ? ' · ' + q.sub : '')
    + (q.subsub ? ' · ' + q.subsub : '');
  phase = 'reading';
  renderQText();
  setStatus('reading', cur.voice ? 'Reading aloud' : 'Reading');
  updateButtons();
  if (cur.voice) playAudio(); else scheduleStep();
}

function scheduleStep() {
  const f = (cur.slow.has(wordIdx) || cur.slow.has(wordIdx + 1)) ? SLOW_FACTOR : 1;
  tick = setTimeout(step, msPerWord() * f);
}
function step() {
  tick = null;
  wordIdx++;
  renderQText();
  if (wordIdx >= cur.units.length) { startGrace(); return; }
  scheduleStep();
}

function startGrace() {
  setStatus('reading', 'Buzz window');
  const ms = 5000; const t0 = performance.now();
  graceAnim = setInterval(() => { $('timerfill').style.width = Math.min(100, (performance.now() - t0) / ms * 100) + '%'; }, 100);
  graceTimer = setTimeout(goDead, ms);
}
function goDead() {
  clearTimers();
  const res = cur.negged ? 'W' : 'D';   // an unrecovered multibuzz neg is still a neg
  cur.res = res;
  record(res);
  phase = 'done';
  setStatus('dead', cur.negged ? 'Dead, neg recorded' : 'Dead, no buzz');
  showJudge(cur.negged ? 'W' : null);
  updateButtons();
}

function renderQText() {
  if (!cur) return;
  const el = $('qtext');
  // Voice mode hides the text until the question resolves — hearing it and
  // reading it at once would defeat the point.
  if (cur.voice && phase !== 'done') {
    el.innerHTML = '<div class="empty">&#128266; Reading aloud. The text appears when the question ends.</div>';
    return;
  }
  let html = '';
  const skiptoggleHtml = () => {
    const n = cur.startSent;
    return '<button class="skiptoggle" id="skiptoggle">'
      + (cur.showSkipped ? '▾ hide ' : '▸ show ') + n + ' earlier sentence' + (n > 1 ? 's' : '')
      + '</button><br>';
  };
  if (phase === 'done') {
    // Question over (also every review screen — those render with phase
    // 'done'): each sentence becomes a clue-lookup tap target. Sentence
    // indices match the corpus search index (same splitter both sides).
    const clueBody = t => esc(t).replace(/\(\*\)/g, '<span class="powermark">(*)</span>');
    if (cur.startSent > 0) {
      html += skiptoggleHtml();
      if (cur.showSkipped) html += '<span class="skipped">'
        + cur.sents.slice(0, cur.startSent).map((s, i) =>
          '<span class="clue-s" data-si="' + i + '" title="tap to look up this clue">'
          + clueBody(s) + '</span>').join(' ')
        + '</span> ';
    }
    // Buzz position as a char offset into the read text — the units
    // reconstruct it exactly, so the mark lands mid-sentence where it fell.
    let buzzChar = null;
    if (buzzAt != null) {
      let s = '';
      for (let i = 0; i < buzzAt; i++) s += (i ? cur.units[i].sep : '') + cur.units[i].t;
      buzzChar = s.length;
    }
    if (buzzAt === 0) html += '<span class="buzzmark">●</span> ';
    let off = 0;
    cur.sents.slice(cur.startSent).forEach((sent, j) => {
      const start = off, end = off + sent.length;
      let body;
      if (buzzChar != null && buzzChar > start && buzzChar <= end) {
        body = clueBody(sent.slice(0, buzzChar - start))
          + ' <span class="buzzmark">●</span>' + clueBody(sent.slice(buzzChar - start));
      } else body = clueBody(sent);
      html += (j ? ' ' : '') + '<span class="clue-s" data-si="' + (cur.startSent + j)
        + '" title="tap to look up this clue">' + body + '</span>';
      off = end + 1;
    });
    el.innerHTML = html;
    return;
  }
  if (cur.startSent > 0) {
    html += skiptoggleHtml();
    if (cur.showSkipped) html += '<span class="skipped">' + esc(cur.sents.slice(0, cur.startSent).join(' ')) + '</span> ';
  }
  const U = cur.units;
  const upto = wordIdx;
  const mark = t => t === '(*)' ? '<span class="powermark">(*)</span>' : esc(t);
  let readHtml = buzzAt === 0 ? '<span class="buzzmark">●</span> ' : '';
  for (let i = 0; i < upto; i++) {
    readHtml += (i ? U[i].sep : '') + mark(U[i].t);
    if (buzzAt != null && i === buzzAt - 1) readHtml += ' <span class="buzzmark">●</span>';
  }
  html += readHtml;
  el.innerHTML = html;
}

// Timers only — buzz() uses this so a multibuzz resume can continue the
// audio from the buzz point instead of restarting a torn-down element.
function clearTicks() {
  if (tick) { clearTimeout(tick); tick = null; }
  if (graceTimer) { clearTimeout(graceTimer); graceTimer = null; }
  if (graceAnim) { clearInterval(graceAnim); graceAnim = null; }
}
function clearTimers() {
  clearTicks();
  stopAudio();
}

/* ---------- voice reading (pre-generated Chatterbox audio) ----------
   When "Read aloud" is on, the question plays from a pre-generated Opus file
   on the audio host instead of being shown — the text stays hidden until the
   question resolves (guard in renderQText), like a moderator reading to you.
   The queue is RESTRICTED to questions that have audio (audioMode gate in
   rowInScope), so a question that plays always has a file. Buzz depth comes
   from the file's {qid}.json sidecar (see the mapping block below) — the exact
   word the voice had reached — falling back to currentTime/duration only when
   the sidecar is unavailable. The <audio> element's native pause/resume is
   reliable (unlike Web Speech), so pause just pauses. */
const AUDIO_BASE = 'https://huggingface.co/datasets/uild42/qb-audio/resolve/main';
let AUDIO_HAVE = null;      // Set of qids with audio (from audio_index.json); null = not loaded
let audioLoad = null;       // in-flight manifest load promise
const audioEl = typeof Audio !== 'undefined' ? new Audio() : null;
if (audioEl) audioEl.preload = 'auto';
// Prefetched next-question audio as object URLs (qid -> blobURL, or null while
// in flight). Each opus is ~130 KB; prefetching the next question during the
// current one's ~40 s of playback hides HF's ~0.5 s resolve→CDN latency, so
// every question after the first starts instantly. LRU-evicted + revoked.
const audioCache = new Map();
const AUDIO_CACHE_MAX = 6;

function voiceAvailable() { return !!audioEl; }
function hasAudio(qid) { return !!(AUDIO_HAVE && AUDIO_HAVE.has(qid)); }
// Audio mode is only "active" (queue-filtering) once the manifest has loaded;
// until then we don't want to filter the whole queue down to nothing.
function audioMode() { return !!(settings.voice && AUDIO_HAVE); }

// Load the manifest of audio-backed qids once (cached). Fetched from the audio
// host, not the R2 data plane. Idempotent; returns the in-flight promise.
function loadAudioIndex() {
  if (AUDIO_HAVE) return Promise.resolve(AUDIO_HAVE);
  if (audioLoad) return audioLoad;
  audioLoad = fetch(AUDIO_BASE + '/audio_index.json', { cache: 'no-cache' })
    .then(r => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
    .then(idx => { AUDIO_HAVE = new Set(idx.tossups || []); return AUDIO_HAVE; })
    .catch(err => { audioLoad = null; throw err; });
  return audioLoad;
}

function voiceRate() { return Math.min(2, Math.max(0.5, settings.vrate || 0.95)); }
// Shard = LAST two hex chars of the qid (ObjectId counter, uniform) — the
// timestamp prefix put every qid in one folder and hit HF's 10k/dir cap.
function audioUrl(qid) { return AUDIO_BASE + '/tossups/' + qid.slice(-2) + '/' + qid + '.opus'; }

// Warm the next question's audio into an object URL so its playback is
// instant. Dedupes in-flight, tolerates failure (falls back to network URL).
function prefetchAudio(qid) {
  if (!qid || !hasAudio(qid)) return;
  loadSidecar(qid);            // tiny; warms the audio-time -> text map too
  if (audioCache.has(qid)) return;
  audioCache.set(qid, null);   // in-flight marker (also dedupes)
  fetch(audioUrl(qid)).then(r => r.ok ? r.blob() : Promise.reject())
    .then(blob => {
      audioCache.set(qid, URL.createObjectURL(blob));
      while (audioCache.size > AUDIO_CACHE_MAX) {
        const k = audioCache.keys().next().value;
        const u = audioCache.get(k); if (typeof u === 'string') URL.revokeObjectURL(u);
        audioCache.delete(k);
      }
    })
    .catch(() => audioCache.delete(qid));
}
// The already-prefetched blob URL if ready, else the network URL.
function audioSrcFor(qid) {
  const c = audioCache.get(qid);
  return typeof c === 'string' ? c : audioUrl(qid);
}

/* ---------- sidecars: exact audio time -> text position ----------
   Every .opus has a {qid}.json sidecar: chunk [start,end] audio offsets plus
   the chunk texts (synthesis was chunk-by-chunk, so the offsets are exact),
   and after the post-run forced-alignment pass (dev/tts/align_tts.py) also
   per-word offsets ("v":2). Using it, buzz depth is the word the voice had
   actually reached rather than a currentTime/duration guess — which matters
   most for power (the (*) unit index) and for last-n-sentence mode, where the
   audio reads the WHOLE question but the reader only reveals the last n
   sentences (so the proportional estimate was systematically wrong there).

   Two coordinate systems have to meet:
     SPOKEN words — the sidecar's text: the CLEANED question, with
       pronunciation guides and moderator notes stripped, "Mrs." spoken as
       "Missus", "(*)" silent (lib/audio/ttsclean.py);
     DISPLAY words — the question as shown and counted, whose indexes drive the
       reveal-unit contract shared with the scorekeeper.
   alignSpokenToDisplay walks both monotonically: each spoken word claims the
   next display word that matches within a short lookahead (a stripped guide or
   note is a handful of display words), and when the cleaner substituted a word
   outright it just takes the cursor position. Everything in this block is pure
   (no DOM, no fetch, no globals) so tests/reader_audio exercises it directly. */
const NORM_W = s => String(s == null ? '' : s).toLowerCase().replace(/[^a-z0-9]+/g, '');
const SIDE_LOOKAHEAD = 12;   // display words a spoken word may skip over (stripped guide/note)

function alignSpokenToDisplay(spoken, disp) {
  const out = new Array(spoken.length);
  let c = 0;
  for (let i = 0; i < spoken.length; i++) {
    const sw = NORM_W(spoken[i]);
    let hit = -1;
    const stop = Math.min(disp.length, c + SIDE_LOOKAHEAD);
    for (let j = c; j < stop; j++) {
      if (sw && NORM_W(disp[j]) === sw) { hit = j; break; }
    }
    if (hit < 0) hit = Math.min(c, disp.length - 1);   // substitution: Mrs. -> Missus
    out[i] = hit;
    c = hit + 1;
  }
  return out;
}

// The spoken words with their audio times: straight from a v2 sidecar's word
// offsets, or interpolated evenly across each chunk's span for a v1 sidecar.
function spokenTimeline(side) {
  const sp = [];
  if (!side) return sp;
  if (Array.isArray(side.words) && side.words.length) {
    for (const w of side.words) {
      if (w && w.length >= 3) sp.push({ w: w[0], s: +w[1], e: +w[2] });
    }
    return sp;
  }
  const chunks = Array.isArray(side.chunks) ? side.chunks : [];
  const texts = Array.isArray(side.texts) ? side.texts : [];
  for (let k = 0; k < chunks.length; k++) {
    const ws = String(texts[k] == null ? '' : texts[k]).split(/\s+/).filter(Boolean);
    const a = +chunks[k][0], b = +chunks[k][1];
    const step = ws.length ? (b - a) / ws.length : 0;
    ws.forEach((w, n) => sp.push({ w, s: a + n * step, e: a + (n + 1) * step }));
  }
  return sp;
}

// {s, disp} for a question: spoken-word start times and the display-word index
// each spoken word lands on. null when the sidecar carries nothing usable.
function buildAudioMap(side, dispWords) {
  const sp = spokenTimeline(side);
  if (!sp.length || !dispWords || !dispWords.length) return null;
  return { s: sp.map(x => x.s), disp: alignSpokenToDisplay(sp.map(x => x.w), dispWords) };
}

// How many DISPLAY words have been heard at audio time t (counting from the
// start of the whole question, skipped sentences included).
function dispWordsHeard(map, t) {
  const s = map.s;
  let lo = 0, hi = s.length - 1, i = -1;
  while (lo <= hi) {                       // last spoken word that has started
    const mid = (lo + hi) >> 1;
    if (s[mid] <= t) { i = mid; lo = mid + 1; } else hi = mid - 1;
  }
  return i < 0 ? 0 : map.disp[i] + 1;
}

// When the voice reaches display word w — the seek target for last-n-sentence
// mode. null if no spoken word maps that far (nothing to seek to).
function audioTimeForDispWord(map, w) {
  for (let i = 0; i < map.disp.length; i++) if (map.disp[i] >= w) return map.s[i];
  return null;
}

// First reveal-unit index of each display word. buildUnits splits dash-joined
// note runs into several units and marks continuations with sep '', so a unit
// with sep ' ' is where a new display word starts.
function wordUnitStarts(units) {
  const starts = [];
  for (let i = 0; i < units.length; i++) if (i === 0 || units[i].sep === ' ') starts.push(i);
  return starts;
}

// Reveal-unit index for "heard display words", in the read portion's frame
// (skipWords display words precede it in last-n-sentence mode).
function unitIdxForDispWord(starts, unitCount, skipWords, heard) {
  const wr = heard - skipWords;
  if (wr <= 0) return 0;
  return wr >= starts.length ? unitCount : starts[wr];
}
/* ---------- end sidecar mapping ---------- */

function sidecarUrl(qid) { return AUDIO_BASE + '/tossups/' + qid.slice(-2) + '/' + qid + '.json'; }
// qid -> sidecar object, null (absent/failed), or the in-flight promise. A
// missing sidecar is cached as null so one 404 isn't refetched all session.
const sideCache = new Map();
const SIDE_CACHE_MAX = 8;
function loadSidecar(qid) {
  if (sideCache.has(qid)) {
    const v = sideCache.get(qid);
    return (v && typeof v.then === 'function') ? v : Promise.resolve(v);
  }
  const p = fetch(sidecarUrl(qid)).then(r => (r.ok ? r.json() : null)).catch(() => null)
    .then(side => {
      sideCache.set(qid, side);
      while (sideCache.size > SIDE_CACHE_MAX) sideCache.delete(sideCache.keys().next().value);
      return side;
    });
  sideCache.set(qid, p);
  return p;
}

// Attach the live question's audio map once its sidecar lands. Playback does
// not wait on this (buzz depth uses the proportional fallback until it does);
// only the last-n seek needs it, and prefetch makes it instant there.
function attachSidecar() {
  const mine = cur;
  return loadSidecar(cur.q.id).then(side => {
    if (cur !== mine) return null;
    cur.audioMap = buildAudioMap(side, cur.dispWords);
    return cur.audioMap;
  }).catch(() => null);
}

function stopAudio() {
  if (!audioEl) return;
  audioEl.onended = audioEl.ontimeupdate = audioEl.onerror = audioEl.onloadedmetadata = null;
  try { audioEl.pause(); } catch (e) {}
  audioEl.removeAttribute('src');
  try { audioEl.load(); } catch (e) {}
}

// Map the audio clock to a reveal-unit index so buzz depth/power/stats work.
// Exact via the sidecar; proportional when there is none.
function audioWordIdx() {
  const t = audioEl.currentTime;
  if (cur.audioMap) {
    return unitIdxForDispWord(cur.unitStarts, cur.units.length, cur.skipWords,
                              dispWordsHeard(cur.audioMap, t));
  }
  const d = audioEl.duration;
  if (!d || !isFinite(d)) return wordIdx;
  return Math.min(cur.units.length, Math.round(t / d * cur.units.length));
}

function playAudio() {
  stopAudio();
  const mine = cur;
  audioEl.src = audioSrcFor(cur.q.id);
  audioEl.playbackRate = voiceRate();
  try { audioEl.preservesPitch = true; } catch (e) {}
  const degrade = () => {   // file missing/failed/blocked: fall back to text reveal
    if (phase === 'reading' && cur && cur.voice) { cur.voice = false; renderQText(); updateButtons(); scheduleStep(); }
  };
  audioEl.ontimeupdate = () => {
    if (phase !== 'reading') return;
    wordIdx = audioWordIdx();
    const d = audioEl.duration;
    // The bar tracks the part being read: in last-n mode playback starts partway
    // in, so measure from the seek point, not from 0.
    if (d && isFinite(d) && d > cur.audioStart) {
      $('timerfill').style.width = Math.min(100,
        (audioEl.currentTime - cur.audioStart) / (d - cur.audioStart) * 100) + '%';
    }
  };
  audioEl.onended = () => {
    if (phase !== 'reading') return;
    wordIdx = cur.units.length; $('timerfill').style.width = '0%'; startGrace();
  };
  audioEl.onerror = degrade;
  const start = () => {
    if (cur !== mine || phase !== 'reading') return;
    // Last-n-sentence mode: the file reads the whole question, so start at the
    // first word of the read portion instead of the top.
    if (cur.startSent > 0 && cur.audioMap) {
      const t0 = audioTimeForDispWord(cur.audioMap, cur.skipWords);
      if (t0 != null && t0 > 0) {
        cur.audioStart = t0;
        const seek = () => { try { audioEl.currentTime = t0; } catch (e) {} };
        audioEl.onloadedmetadata = seek;
        if (audioEl.readyState >= 1) seek();
      }
    }
    audioEl.play().catch(degrade);
  };
  // Only the seek needs the sidecar, so only that path waits for it (the
  // sidecar is prefetched with the audio, so the wait is nil after question 1).
  if (cur.startSent > 0 && cur.mapReady) cur.mapReady.then(start, start);
  else start();
}

// Turn on audio mode: load the audio manifest, then rebuild the queue with
// the audio restriction applied. On failure, revert the toggle cleanly.
function enterAudioMode() {
  loadAudioIndex().then(() => { onFilter(); }).catch(() => {
    settings.voice = false;
    if ($('voice')) $('voice').checked = false;
    $('voicerow').style.display = 'none';
    savePrefs();
    onFilter();
    alert('Could not load the audio index. Read-aloud is unavailable.');
  });
}

/* ---------- buzz + judge ---------- */
function buzz() {
  if (phase !== 'reading' && phase !== 'paused') return;
  if (cur.voice && audioEl && audioEl.src) wordIdx = audioWordIdx();
  // Pause, don't tear down — a multibuzz neg resumes from here. Finalization
  // (judged/goDead/skip/loadNext) does the teardown.
  clearTicks();
  if (cur.voice && audioEl) { try { audioEl.pause(); } catch (e) {} }
  buzzAt = wordIdx;
  if (cur.firstBuzzAt == null) cur.firstBuzzAt = wordIdx;   // stats credit the first buzz
  phase = 'buzzed';
  setStatus('buzzed', 'Buzzed. Answer?');
  renderQText();
  $('answerrow').classList.add('show');
  $('answerinput').value = '';
  $('answerinput').focus();
  updateButtons();
}

function submitAnswer() {
  const raw = $('answerinput').value.trim();
  $('answerrow').classList.remove('show');
  const { directive, directedPrompt } = qbCheckAnswer(cur.q.a || cur.q.as || '', raw);
  if (directive === 'accept') { judged('C', raw); return; }
  // qbreader-style prompting: re-prompt as long as the checker says prompt,
  // relaying the answerline's directed-prompt text when it has one. An empty
  // resubmission concedes (reject -> wrong).
  if (directive === 'prompt') {
    cur.prompted = true;
    const hint = directedPrompt ? 'Prompt: ' + directedPrompt : 'Prompt: be more specific';
    setStatus('buzzed', hint);
    $('verdict').className = 'verdict p';
    $('verdict').textContent = 'Prompt on “' + raw + '”' + (directedPrompt ? ': ' + directedPrompt : '') + '. Try again:';
    $('judge').classList.add('show');
    $('answerline').innerHTML = '';
    $('selfgrade').style.display = 'none';
    $('answerrow').classList.add('show');
    $('answerinput').value = ''; $('answerinput').focus();
    return;
  }
  if (settings.multibuzz) { negContinue(raw); return; }
  judged('W', raw);
}

// Multibuzz: a wrong answer doesn't end the question — note the neg (no
// answerline reveal), then resume reading from the buzz point so the player
// can buzz again until correct or dead.
function negContinue(raw) {
  cur.negged = true;
  cur.userRaw = raw;
  $('judge').classList.remove('show');   // the prompt loop may have opened it
  setStatus('done-w', 'No, keep listening');
  // A short beat before reading resumes; phase stays 'buzzed' so Space/tap
  // are inert, and Skip's clearTimers() cancels the beat via tick.
  tick = setTimeout(resumeAfterNeg, 700);
}
function resumeAfterNeg() {
  tick = null;
  phase = 'reading';
  setStatus('reading', cur.voice ? 'Reading aloud' : 'Reading');
  renderQText();
  updateButtons();
  // A played-out audio element restarts from the top on play() — route the
  // ended case (buzz during the grace window) to a fresh grace instead.
  if (cur.voice && audioEl && audioEl.src && !audioEl.ended) audioEl.play().catch(() => {});
  else if (wordIdx >= cur.units.length) startGrace();
  else scheduleStep();
}

// auto-verdict, then let the reader override it (checking isn't perfect)
function judged(res, raw) {
  stopAudio();
  phase = 'done';
  cur.userRaw = raw;
  cur.lateCorrect = res === 'C' && cur.negged;
  const finalRes = cur.lateCorrect ? 'W' : res;   // a neg counts even if recovered
  cur.res = finalRes;
  record(finalRes);
  renderVerdict(finalRes);
  showJudge(finalRes);
  updateButtons();
}
function regrade(newRes) {
  const last = LOG[LOG.length - 1];
  if (last && last.res !== newRes) {
    last.res = newRes;
    last.pow = newRes === 'C' && poweredBuzz();
    saveLog();
  }
  if (cur) cur.res = newRes;
  renderVerdict(newRes);
  showJudge(newRes);
}

/* ---------- previous-question review ----------
   Step back through recently seen questions ('k' or the Prev button) to
   re-read them fully with answer + wiki, without touching the queue or your
   stats. Return with 'j' or "Return to current". Entry is only allowed once
   the live question is finished (done/idle), so restoring the live view is a
   plain re-render. */
const HISTORY_MAX = 50;
function reviewing() { return reviewIdx >= 0; }
function pushHistory() {
  if (!cur) return;
  history.push({ snap: cur, buzzAt, res: cur.res != null ? cur.res : null });
  if (history.length > HISTORY_MAX) history.shift();
}
function openPrev() {
  if (reviewing()) { if (reviewIdx > 0) { reviewIdx--; showReviewAt(); } return; }
  if (!history.length || !(phase === 'done' || phase === 'idle')) return;
  clearTimers();
  liveSnap = { cur, buzzAt, wordIdx, phase };
  reviewIdx = history.length - 1;
  showReviewAt();
}
function reviewForward() {
  if (!reviewing()) return;
  if (reviewIdx < history.length - 1) { reviewIdx++; showReviewAt(); }
  else exitReview();
}
/* ---------- clue lookup (semantic search over the whole corpus) ----------
   Tap a sentence on a finished or reviewed question -> losClueSearch
   (cloud: the sign-in-gated Worker /search; in-browser: local embedding
   + client-side index scan, no sign-in) -> resolve the returned (id,
   kind, sentence/part, set) rows to text via the published
   sets/{slug}.json shards. Sentence indices line up because
   splitSentences here and split_sentences in the pipeline are ports of
   each other. */
const CLUE_TOPN = 8;
const clueSetCache = new Map(); // set slug -> {tossups: Map, bonuses: Map}

function clueSetFor(slug) {
  if (clueSetCache.has(slug)) return Promise.resolve(clueSetCache.get(slug));
  return qdataFetch('sets/' + slug + '.json').then(shard => {
    const t = new Map(), b = new Map();
    for (const pk of shard.packets) {
      for (const tu of pk.tossups || []) t.set(tu._id, tu);
      for (const bn of pk.bonuses || []) b.set(bn._id, bn);
    }
    const entry = { tossups: t, bonuses: b };
    clueSetCache.set(slug, entry);
    return entry;
  });
}

function closeCluePanel() { $('cluepanel').classList.remove('show'); }

function openCluePanel(si) {
  if (!cur || !cur.sents[si]) return;
  const sent = cur.sents[si];
  $('cluepanel').classList.add('show');
  $('cluequery').textContent = sent;
  $('cluebody').innerHTML = '<div class="cluenote">Searching the corpus&hellip;</div>';
  losClueSearch.search(sent, {})
    .then(res => renderClueResults(res.results || []))
    .catch(e => {
      $('cluebody').innerHTML = '<div class="cluenote">' + (e && e.signin
        ? 'Sign in from the Sync panel, or switch clue search to In browser.'
        : 'Lookup failed (' + esc(String(e.message || e)) + ').') + '</div>';
    });
}

// Resolve /search result rows to display rows ({r, text, ans, pos}) via
// the set shards, dropping the excluded question and cross-set recycles.
// Shared by the clue panel and the missed-clues report.
function resolveClueRows(results, excludeId, topn) {
  const picks = results.filter(r => r.id !== excludeId && r.set).slice(0, topn);
  return Promise.all(picks.map(r => clueSetFor(r.set).then(shard => {
    if (r.kind === 0) {
      const doc = shard.tossups.get(r.id);
      if (!doc) return null;
      const sents = splitSentences(doc.question_sanitized || doc.question || '');
      return { r, text: sents[r.s] || '', ans: doc.answer_sanitized || doc.answer || '',
               pos: 'clue ' + (r.s + 1) + '/' + sents.length };
    }
    const doc = shard.bonuses.get(r.id);
    if (!doc) return null;
    const leadin = r.s === 255;
    const answers = doc.answers_sanitized || doc.answers || [];
    return {
      r,
      text: leadin ? (doc.leadin_sanitized || doc.leadin || '')
                   : ((doc.parts_sanitized || doc.parts || [])[r.s] || ''),
      // a leadin has no answer of its own — show its first part's answer
      ans: (leadin ? answers[0] : answers[r.s]) || '',
      pos: leadin ? 'bonus leadin' : 'bonus part',
    };
  }).catch(() => null))).then(rows => {
    const seen = new Set(), out = [];
    for (const row of rows) {
      if (!row || !row.text) continue;
      const key = row.text.slice(0, 80);
      if (seen.has(key)) continue; // same question recycled across sets
      seen.add(key);
      out.push(row);
    }
    return out;
  });
}

function clueItemHtml(row) {
  const slug = ID2SLUG ? ID2SLUG.get(row.r.id) : null;
  const ansHead = (row.ans || '').split('[')[0].split('(')[0].trim();
  const pos = row.pos + (row.r.set ? ' · ' + row.r.set.replace(/_/g, ' ') : '');
  return '<div class="clueitem">'
    + '<div class="clueans">' + esc(ansHead || '—')
    + (slug ? ' <a href="output/' + slug + '/stock.html" target="_blank" rel="noopener">wiki ↗</a>' : '')
    + '<span class="cluepos">' + esc(pos) + '</span></div>'
    + '<div class="cluetext">' + esc(row.text) + '</div>'
    + '</div>';
}

function renderClueResults(results) {
  const mine = cur ? cur.q.id : null;
  resolveClueRows(results, mine, CLUE_TOPN).then(rows => {
    $('cluebody').innerHTML = rows.map(clueItemHtml).join('')
      || '<div class="cluenote">No similar clues found.</div>';
  });
}

/* ---------- missed clues report (view-missed) ----------
   Session scope: every attempt recorded since this page load, worst buzz
   first (dead, then by buzz depth, deepest first). Each row queries the
   up-to-3 sentences BEFORE the last-heard one — the clues that should
   have converted — re-derived from the LOG row alone: rec.sent reconstructs
   the trim, bf*total recovers the buzz unit (bf is rounded to 3 decimals,
   exact for any question under ~500 units), and the unit's char offset
   lands in a sentence. Nothing extra is stored in LOG. Searches run
   through losClueSearch (cloud or in-browser) and cache per attempt. */
const MISSED_TOPN = 5;
const missedCache = new Map();   // id/ts -> {query, rows?}
let missedSeq = 0;

function missedQueryFor(rec, doc) {
  const text = doc.question_sanitized || doc.question || '';
  const sents = splitSentences(text);
  if (!sents.length) return null;
  let lastHeard = sents.length - 1;          // no buzz: heard the giveaway
  if (rec.bf != null) {
    const nSent = rec.sent || 0;
    const startSent = (nSent > 0 && sents.length > nSent) ? sents.length - nSent : 0;
    const skipped = sents.slice(0, startSent).join(' ');
    const skipWords = skipped ? skipped.split(/\s+/).length : 0;
    const units = buildUnits(sents.slice(startSent).join(' ').split(/\s+/));
    const total = skipWords + units.length;
    const fb = Math.max(0, Math.min(units.length, Math.round(rec.bf * total) - skipWords));
    let s = '';
    for (let i = 0; i < fb; i++) s += (i ? units[i].sep : '') + units[i].t;
    const buzzChar = s.length;
    // Last sentence with any text before the buzz point. Offsets index the
    // read text (sentences joined by single spaces), same as renderQText.
    lastHeard = startSent;
    let start = 0;
    for (let i = startSent; i < sents.length; i++) {
      if (buzzChar > start) lastHeard = i; else break;
      start += sents[i].length + 1;
    }
  }
  // Query the up-to-3 sentences before the last-heard one — a late buzz
  // means the clues that should have converted sit a couple back, not
  // just one. Buzzing inside sentence 0 falls back to sentence 0 itself.
  const queries = [];
  for (let k = 1; k <= 3; k++) {
    const qi = lastHeard - k;
    if (qi < 0) break;
    queries.push({ qi, text: sents[qi] });
  }
  if (!queries.length) queries.push({ qi: lastHeard, text: sents[lastHeard] });
  return { queries, lastHeard, n: sents.length };
}

function missedOrder() {
  return sessionRows.slice().sort((a, b) =>
    (b.bf == null ? 1.5 : b.bf) - (a.bf == null ? 1.5 : a.bf));
}

function missedCardHtml(rec, i) {
  const res = rec.res === 'C' ? ['ok', rec.pow ? 'power' : 'correct']
    : rec.res === 'W' ? ['neg', 'neg'] : ['dead', 'dead'];
  const pct = rec.bf != null ? Math.round(rec.bf * 100) : null;
  const meta = [rec.cat, rec.diff != null ? 'diff ' + rec.diff : null].filter(Boolean).join(' · ');
  return '<div class="mc" id="mc' + i + '">'
    + '<div class="mc-head">'
    + '<span class="mc-depth">' + (pct != null ? pct + '<small>%</small>' : '—') + '</span>'
    + '<span class="mc-res ' + res[0] + '">' + res[1] + '</span>'
    + '<span class="mc-ans">' + esc(rec.aDisp || rec.ans || '—')
    + (rec.t ? ' <a href="output/' + esc(rec.t) + '/stock.html" target="_blank" rel="noopener">wiki ↗</a>' : '')
    + '</span>'
    + '<span class="mc-meta">' + esc(meta) + '</span></div>'
    + '<div class="mc-bar"><i class="' + res[0] + '" style="width:' + (pct != null ? pct : 100) + '%"></i></div>'
    + '<div class="mc-queries"><div class="cluenote">loading&hellip;</div></div>'
    + '</div>';
}

function missedNote(rows, searching) {
  const n = k => rows.filter(r => r.res === k).length;
  $('missednote').textContent = 'This session · ' + rows.length + ' played · '
    + n('D') + ' dead · ' + n('W') + ' negs · ' + n('C') + ' correct · worst buzz first'
    + (searching ? ' · searching ' + searching : '');
}

function renderMissed() {
  const rows = missedOrder();
  if (!rows.length) {
    $('missednote').textContent = '';
    $('missedlist').innerHTML = '<div class="empty">Nothing this session yet. Play some questions first.</div>';
    return;
  }
  missedNote(rows, '');
  $('missedlist').innerHTML = rows.map((rec, i) => missedCardHtml(rec, i)).join('');
  const seq = ++missedSeq;
  const canSearch = losClueSearch.mode() === 'local'
    || !!(window.losReaderHook.searchAuth && window.losReaderHook.searchAuth());
  let done = 0;
  const items = rows.map((rec, i) => ({ rec, i }));
  const lane = async slice => {
    for (const it of slice) {
      if (seq !== missedSeq) return;
      await missedOne(it.rec, it.i, canSearch);
      done++;
      if (seq === missedSeq && canSearch && done < rows.length)
        missedNote(rows, done + ' of ' + rows.length);
    }
  };
  const lanes = [[], []];
  items.forEach((it, ix) => lanes[ix % 2].push(it));
  Promise.all(lanes.map(lane)).then(() => { if (seq === missedSeq) missedNote(rows, ''); });
}

async function missedOne(rec, i, canSearch) {
  const card = document.getElementById('mc' + i);
  if (!card) return;
  const wrap = card.querySelector('.mc-queries');
  const key = rec.id + '/' + rec.ts;
  const SIGNIN = 'Sign in from the Sync panel, or switch clue search to In browser.';
  try {
    let entry = missedCache.get(key);
    if (!entry) {
      const r = ID2ROW.get(rec.id);
      if (r == null) throw new Error('not in the published catalog');
      const docs = await shardFor(CAT.tossups.set[r]);
      const doc = docs.get(rec.id);
      if (!doc) throw new Error('question text unavailable');
      const mq = missedQueryFor(rec, doc);
      if (!mq) throw new Error('no sentences');
      entry = { n: mq.n, queries: mq.queries };   // q.rows cache per sentence
      missedCache.set(key, entry);
    }
    wrap.innerHTML = entry.queries.map(q =>
      '<div class="mc-q">'
      + '<div class="mc-query"><span class="lbl">clue ' + (q.qi + 1) + '/' + entry.n + '</span>'
      + esc(q.text) + '</div>'
      + '<div class="mc-sim"><div class="cluenote">'
      + (canSearch ? 'queued' : SIGNIN) + '</div></div>'
      + '</div>').join('');
    if (!canSearch) return;
    for (let k = 0; k < entry.queries.length; k++) {
      const q = entry.queries[k];
      const simEl = wrap.children[k].querySelector('.mc-sim');
      if (!q.rows) {
        simEl.innerHTML = '<div class="cluenote">searching&hellip;</div>';
        const res = await losClueSearch.search(q.text, {});
        q.rows = await resolveClueRows(res.results || [], rec.id, MISSED_TOPN);
      }
      if (!q.rows.length) {
        simEl.innerHTML = '<div class="cluenote">No similar clues found.</div>';
        continue;
      }
      const open = i === 0 && k === 0;
      simEl.innerHTML = '<button class="simtoggle">' + (open ? '▾' : '▸') + ' '
        + q.rows.length + ' similar clue' + (q.rows.length > 1 ? 's' : '') + '</button>'
        + '<div class="simlist' + (open ? ' open' : '') + '">'
        + q.rows.map(clueItemHtml).join('') + '</div>';
    }
  } catch (e) {
    wrap.innerHTML = '<div class="cluenote">'
      + (e && e.signin ? SIGNIN
                       : 'Lookup failed (' + esc(String(e.message || e)) + ').')
      + '</div>';
  }
}

function exitReview() {
  if (!reviewing()) return;
  const s = liveSnap; liveSnap = null; reviewIdx = -1;
  cur = s.cur; buzzAt = s.buzzAt; wordIdx = s.wordIdx; phase = s.phase;
  $('reviewbar').style.display = 'none';
  if (!cur) {
    $('qtext').innerHTML = '<div class="empty">' + queueSize().toLocaleString()
      + ' questions in scope. Press <b>Start</b> (or tap here).</div>';
    $('judge').classList.remove('show'); $('wikibox').classList.remove('show');
  } else {
    renderQText();
    if (phase === 'done') showJudge(cur.res != null ? cur.res : null);
  }
  updateButtons();
}
function showReviewAt() {
  const h = history[reviewIdx];
  cur = h.snap; buzzAt = h.buzzAt; wordIdx = cur.units.length; phase = 'done';
  $('answerrow').classList.remove('show');
  renderQText();
  showJudge(h.res);   // verdict + answerline + wiki (selfgrade suppressed in review)
  if (h.res) renderVerdict(h.res); else setStatus('', 'Reviewing');
  renderReviewBar();
  updateButtons();
}
function renderReviewBar() {
  const bar = $('reviewbar');
  const back = history.length - reviewIdx;   // 1 = most recent
  bar.style.display = '';
  bar.innerHTML = '<span class="rlbl">Reviewing</span> ' + back + ' question' + (back > 1 ? 's' : '') + ' back'
    + '<span class="rspacer"></span>'
    + '<button id="rv-older"' + (reviewIdx === 0 ? ' disabled' : '') + '>&#9664; Older</button>'
    + '<button id="rv-newer">Newer &#9654;</button>'
    + '<button id="rv-return">Return to current</button>';
  $('rv-older').onclick = openPrev;
  $('rv-newer').onclick = reviewForward;
  $('rv-return').onclick = exitReview;
}
function renderVerdict(res) {
  const pct = buzzPct();   // last buzz — what the player just did
  const word = res === 'C' ? (poweredBuzz() ? 'Power!' : 'Correct')
    : (res === 'W' && cur.lateCorrect) ? 'Correct, but negged' : 'Wrong';
  setStatus(res === 'C' ? 'done-c' : 'done-w', word + (pct != null ? ' · ' + pct + '%' : ''));
}
function buzzPct() {
  if (buzzAt == null) return null;
  const total = cur.skipWords + cur.units.length;
  return Math.round((cur.skipWords + buzzAt) / total * 100);
}
function poweredBuzz() {
  const pi = cur.units.findIndex(u => u.t === '(*)');
  const at = cur.firstBuzzAt != null ? cur.firstBuzzAt : buzzAt;   // power = first buzz
  return pi >= 0 && at != null && at <= pi;
}
function showJudge(res) {
  renderQText();
  $('judge').classList.add('show');
  const overridable = res === 'C' || res === 'W';   // you actually answered
  const said = cur.userRaw != null && cur.userRaw !== ''
    ? ' <span style="color:var(--faint)">(you said “' + esc(cur.userRaw) + '”)</span>' : '';
  if (res === 'C') { $('verdict').className = 'verdict c'; $('verdict').innerHTML = (poweredBuzz() ? 'Correct, in power' : 'Correct') + said; }
  else if (res === 'W' && cur.lateCorrect) { $('verdict').className = 'verdict w'; $('verdict').innerHTML = 'Correct on a later buzz. The first wrong buzz counts as a neg' + said; }
  else if (res === 'W') { $('verdict').className = 'verdict w'; $('verdict').innerHTML = 'Incorrect' + said; }
  else { $('verdict').className = 'verdict'; $('verdict').textContent = 'No buzz. The answer was:'; }
  $('answerline').innerHTML = answerHTML(cur.q.a);
  // override row — the auto-check is a suggestion; let the reader correct it
  // (but not while reviewing a past question — that would regrade the wrong
  // log entry)
  const sg = $('selfgrade');
  sg.style.display = (overridable && reviewIdx < 0) ? 'flex' : 'none';
  $('sg-right').classList.toggle('on', res === 'C');
  $('sg-wrong').classList.toggle('on', res === 'W');
  showWiki();
}

// Scope the reader to a taxonomy slice and jump straight into it — powers
// the "Practice more" chips (e.g. drill the rest of a Romantic-composer group
// after a miss).
function practiceScope(opts) {
  for (const k of ['cats', 'subs', 'subsubs', 'sections', 'tags', 'eras']) filters[k].clear();
  if (opts.cat) filters.cats.add(opts.cat);
  if (opts.sub) filters.subs.add(opts.sub);
  if (opts.subsub) filters.subsubs.add(opts.subsub);
  if (opts.section != null && opts.section >= 0) filters.sections.add(opts.section);
  onFilter();
  loadNext();
  window.scrollTo({ top: 0 });
}

function showWiki() {
  const q = cur.q;
  const slug = q.slug;
  let t = slug ? TOPICS[slug] : null;
  // Trust the wiki topic only when its category matches the question's — a
  // "Mendelssohn" music tossup must not resolve to a philosophy page.
  if (t && q.cat && t.category && t.category !== q.cat) t = null;

  const topicBlock = $('w-topicblock');
  if (t) {
    topicBlock.style.display = '';
    $('w-topic').textContent = t.name;
    $('w-topic').href = 'output/' + slug + '/stock.html';
    const bits = [];
    if (t.year != null) bits.push(t.year < 0 ? Math.abs(t.year) + ' BCE' : String(t.year));
    if (t.country) bits.push(t.country);
    bits.push(t.category + (t.subcategory && t.subcategory !== t.category ? ' · ' + t.subcategory : ''));
    $('w-meta').textContent = bits.join(' · ');
    const tagEl = $('w-tags'); tagEl.innerHTML = '';
    for (const tag of (t.tags || [])) {
      const c = chip(tag, null, false);
      if (FACETS_ON) { c.title = 'Practice this: add to scope'; c.onclick = () => { filters.tags.add(tag); onFilter(); window.scrollTo({ top: 0 }); }; }
      tagEl.appendChild(c);
    }
    // Related — restricted to the SAME category, so a composer suggests other
    // composers (Schumann), not cross-domain co-mentions (Spinoza/Lessing).
    const relEl = $('w-rel'); relEl.innerHTML = '';
    const rel = (t.related || [])
      .filter(x => { const rt = TOPICS[x.slug]; return rt && (!q.cat || rt.category === q.cat); })
      .slice(0, 5);
    if (rel.length) {
      const lbl = document.createElement('span');
      lbl.className = 'subhead'; lbl.style.margin = '0.1rem 0.4rem 0 0'; lbl.textContent = 'Related:';
      relEl.appendChild(lbl);
      for (const x of rel) {
        const a = document.createElement('a');
        a.className = 'chip'; a.style.color = 'var(--wiki)';
        a.textContent = x.topic; a.href = 'output/' + x.slug + '/stock.html';
        relEl.appendChild(a);
      }
    }
  } else {
    topicBlock.style.display = 'none';
  }

  // "Practice more" — always available, driven by the question's own taxonomy
  // (works for every category, wiki-linked or not). Group chip drills the
  // overview section (e.g. Romanticism); subcategory chip broadens.
  const pEl = $('w-practice'); pEl.innerHTML = '';
  const addChip = (label, title, fn) => {
    const c = chip(label, null, false);
    c.title = title; c.style.color = 'var(--wiki)';
    c.onclick = fn; pEl.appendChild(c);
  };
  if (q.section && q.sectionId >= 0)
    addChip('▸ ' + q.section, 'Drill this group',
      () => practiceScope({ cat: q.cat, sub: q.sub, subsub: q.subsub, section: q.sectionId }));
  if (q.sub)
    addChip('▸ ' + q.sub, 'Drill this subcategory',
      () => practiceScope({ cat: q.cat, sub: q.sub === q.cat ? '' : q.sub }));
  $('w-practiceblock').style.display = pEl.children.length ? '' : 'none';

  if (t || pEl.children.length) $('wikibox').classList.add('show');
  else $('wikibox').classList.remove('show');
}

/* ---------- stats ----------
   Each attempt: res C(correct)/W(neg — buzzed wrong)/D(dead — no buzz);
   bf = buzz fraction into the FULL question (0=start, 1=end), null if no
   buzz; celerity = mean(1-bf) over correct buzzes. Exactly one row per
   question even in multibuzz mode: res is 'W' if ANY buzz was wrong, and
   bf/pow are taken from the FIRST buzz. */
function record(res) {
  const total = cur.skipWords + cur.units.length;
  const fb = cur.firstBuzzAt != null ? cur.firstBuzzAt : buzzAt;
  const bf = fb == null ? null : (cur.skipWords + fb) / total;
  const q = cur.q;
  LOG.push({
    id: q.id, t: q.slug || null, ans: answerKey(q.a, q.as) || null,
    aDisp: (q.as || '').split('[')[0].split('(')[0].trim() || q.as || '',
    cat: q.cat, sub: q.sub, section: q.section || null, diff: q.d,
    res, bf: bf == null ? null : Math.round(bf * 1000) / 1000,
    pow: res === 'C' && poweredBuzz(), sent: settings.sent, ts: Date.now(),
  });
  sessionRows.push(LOG[LOG.length - 1]);
  saveLog();
  if (window.losReaderHook.onRecord) try { window.losReaderHook.onRecord(); } catch (e) {}
}

// --- stats state ---
let statsDims = new Set(['cat']);         // breakdown dimensions (composite pivot)
let statsSort = { col: 'acc', dir: 1 };   // 1 asc (weakest first), -1 desc
// Scope filters the WHOLE stats view; each facet is a multi-select set,
// empty = all. The breakdown then groups rows by every active dimension —
// scope Literature+History at diff 2-3, pivot Subcategory×Difficulty to
// compare, e.g., celerity per subcategory per level.
const statsScope = { cat: new Set(), sub: new Set(), section: new Set(), diff: new Set() };
const DIM_ORDER = ['cat', 'sub', 'section', 'diff'];
const DIM_LABEL = { cat: 'Category', sub: 'Subcategory', section: 'Group', diff: 'Difficulty' };
const DIMS = { cat: r => r.cat, sub: r => r.sub, section: r => r.section, diff: r => r.diff != null ? 'Diff ' + r.diff : null };

function scopedLog() {
  return LOG.filter(r =>
    (!statsScope.cat.size || statsScope.cat.has(r.cat)) &&
    (!statsScope.sub.size || statsScope.sub.has(r.sub)) &&
    (!statsScope.section.size || statsScope.section.has(r.section)) &&
    (!statsScope.diff.size || statsScope.diff.has(String(r.diff))));
}
// values offered for one facet: distinct log values under the OTHER facets,
// so each chip row only lists options compatible with the rest of the scope.
function facetVals(facet) {
  const s = new Set();
  for (const r of LOG) {
    let ok = true;
    for (const f of DIM_ORDER) {
      if (f === facet) continue;
      if (statsScope[f].size && !statsScope[f].has(String(r[f]))) { ok = false; break; }
    }
    if (!ok) continue;
    const v = r[facet];
    if (v != null && v !== '') s.add(String(v));
  }
  return [...s].sort(facet === 'diff' ? (a, b) => a - b : undefined);
}
// a dropped category/subcategory takes its orphaned finer picks along
function pruneScope() {
  const keep = (field, pred) => {
    const ok = new Set();
    for (const r of LOG) if (pred(r)) ok.add(String(r[field]));
    for (const x of [...statsScope[field]]) if (!ok.has(x)) statsScope[field].delete(x);
  };
  keep('sub', r => !statsScope.cat.size || statsScope.cat.has(r.cat));
  keep('section', r => (!statsScope.cat.size || statsScope.cat.has(r.cat))
    && (!statsScope.sub.size || statsScope.sub.has(r.sub)));
}
// composite breakdown key: active dimension values joined on NUL (a char
// no taxonomy value contains; statTable name() splits it back apart);
// a null dimension value drops the row
const KEY_SEP = '\u0000';
function compositeKey(r) {
  const parts = [];
  for (const d of DIM_ORDER) {
    if (!statsDims.has(d)) continue;
    const v = DIMS[d](r);
    if (v == null || v === '') return null;
    parts.push(v);
  }
  return parts.join(KEY_SEP);
}

function aggBy(keyFn, data) {
  const m = new Map();
  for (const r of (data || LOG)) {
    const keys = keyFn(r); if (keys == null) continue;
    for (const k of (Array.isArray(keys) ? keys : [keys])) {
      if (k == null || k === '') continue;
      let a = m.get(k);
      if (!a) { a = { n: 0, c: 0, neg: 0, dead: 0, celSum: 0, celN: 0, pow: 0 }; m.set(k, a); }
      a.n++;
      if (r.res === 'C') { a.c++; if (r.pow) a.pow++; if (r.bf != null) { a.celSum += 1 - r.bf; a.celN++; } }
      else if (r.res === 'W') a.neg++;
      else if (r.res === 'D') a.dead++;
    }
  }
  return [...m.entries()].map(([k, a]) => ({
    k, n: a.n, acc: a.c / a.n, neg: a.neg / a.n, dead: a.dead / a.n,
    cel: a.celN ? a.celSum / a.celN : null,
  }));
}

const pctCol = v => v < 0.5 ? 'var(--bad)' : v < 0.75 ? 'var(--accent)' : 'var(--good)';
const fmtPct = v => v == null ? '—' : Math.round(v * 100) + '%';

/* ---------- progress over time ----------
   A rolling average of the scoped log in play order. Each point is the
   trailing window ENDING at that attempt, so the line reads "where I was
   by then" at every x. The window is user-set (10 .. everything in scope);
   at its maximum every window starts at attempt 0, which makes the line a
   running career average converging on the headline tile.

   Accuracy/neg get a Wilson interval, celerity a normal SE on the mean of
   (1-bf) over correct buzzes only — so its point count differs from the
   window size. The band is the honest part: on a thin window it is wide,
   and the eye correctly refuses to read a trend into it.

   Prefix sums make this O(n) at any window size, so dragging the window
   control repaints without lag even when the window is the whole history. */
const WIN_MIN = 10;      // below this a rolling rate is noise, not signal
const MIN_WIN_PTS = 12;  // don't emit points before a window can mean anything

// Wilson score interval — the honest band for a rate out of n trials.
function wilson(k, n) {
  if (!n) return [0, 1];
  const z = 1.96, p = k / n, den = 1 + z * z / n;
  const c = p + z * z / (2 * n);
  const w = z * Math.sqrt(p * (1 - p) / n + z * z / (4 * n * n));
  return [Math.max(0, (c - w) / den), Math.min(1, (c + w) / den)];
}

// rows must be in play order. metric: 'acc' | 'neg' | 'cel'.
function rollup(rows, W, metric) {
  const n = rows.length;
  W = Math.max(2, Math.min(W, n));
  // prefix[i] = total over rows[0..i-1]
  const cC = new Float64Array(n + 1), cW = new Float64Array(n + 1);
  const cN = new Float64Array(n + 1), cS = new Float64Array(n + 1), cQ = new Float64Array(n + 1);
  const cP = new Float64Array(n + 1), cPQ = new Float64Array(n + 1);
  for (let i = 0; i < n; i++) {
    const r = rows[i];
    const hasCel = r.res === 'C' && r.bf != null;
    const cel = hasCel ? 1 - r.bf : 0;
    const pt = r.res === 'C' ? (r.pow ? 15 : 10) : r.res === 'W' ? -5 : 0;
    cP[i + 1] = cP[i] + pt;
    cPQ[i + 1] = cPQ[i] + pt * pt;
    cC[i + 1] = cC[i] + (r.res === 'C' ? 1 : 0);
    cW[i + 1] = cW[i] + (r.res === 'W' ? 1 : 0);
    cN[i + 1] = cN[i] + (hasCel ? 1 : 0);
    cS[i + 1] = cS[i] + cel;
    cQ[i + 1] = cQ[i] + cel * cel;
  }
  const out = [];
  for (let i = 0; i < n; i++) {
    if (i + 1 < Math.min(W, MIN_WIN_PTS)) continue;
    const a = Math.max(0, i - W + 1), b = i + 1;   // half-open [a, b)
    let v, lo, hi, cnt;
    if (metric === 'acc' || metric === 'neg') {
      cnt = b - a;
      const k = metric === 'acc' ? cC[b] - cC[a] : cW[b] - cW[a];
      v = k / cnt;
      [lo, hi] = wilson(k, cnt);
    } else if (metric === 'ppq') {
      cnt = b - a;
      const S = cP[b] - cP[a], Q = cPQ[b] - cPQ[a];
      v = S / cnt;
      const varr = cnt > 1 ? Math.max(0, (Q - S * S / cnt) / (cnt - 1)) : 0;
      const se = Math.sqrt(varr / cnt);
      lo = v - 1.96 * se; hi = v + 1.96 * se;
    } else {
      cnt = cN[b] - cN[a];
      if (cnt < 5) continue;                        // a mean of <5 buzzes is noise
      const S = cS[b] - cS[a], Q = cQ[b] - cQ[a];
      v = S / cnt;
      const varr = cnt > 1 ? Math.max(0, (Q - S * S / cnt) / (cnt - 1)) : 0;
      const se = Math.sqrt(varr / cnt);
      lo = Math.max(0, v - 1.96 * se); hi = Math.min(1, v + 1.96 * se);
    }
    out.push({ i, ts: rows[i].ts, v, lo, hi, n: cnt });
  }
  return out;
}

// Pinned breakdown rows are drawn as extra series. Colors are assigned by
// slot, never by rank, so a re-sort never repaints a series.
const SERIES_COLS = ['#6b9eff', '#e0655f', '#b98adf', '#5dbb7a'];
// Split lines get their own six, ordered so no adjacent pair collapses under
// red-green or blue-yellow color blindness (checked, not eyeballed).
const SPLIT_COLS = ['#e8b04a', '#6b9eff', '#e0655f', '#5ec8b8', '#b98adf', '#5dbb7a'];
const SPLIT_LABEL = { none: 'None', cat: 'Category', sub: 'Subcategory',
                      section: 'Group', diff: 'Difficulty' };
const SPLIT_ORDER = ['none', 'cat', 'sub', 'section', 'diff'];
const SPLIT_MAX = 6;      // more lines than this is unreadable, whatever the palette
const SPLIT_MIN_N = 30;   // a line under this is noise pretending to be a trend
const METRIC_COL = { acc: '#e8b04a', cel: '#6b9eff', neg: '#e0655f', ppq: '#5ec8b8' };
const METRIC_LBL = { acc: 'Accuracy', cel: 'Celerity', neg: 'Neg rate', ppq: 'Points' };
const PROG_MAX_PINS = 4;
const progState = { x: 'idx', metrics: new Set(['acc', 'cel']), W: 50, wmax: false,
                     base: 'raw', split: 'none', hidden: new Set(), pins: [] };

/* Which single metric a split chart draws. Several lines times several
   metrics is unreadable, so splitting collapses to one: the first enabled
   metric the mode supports. */
function splitMetric() {
  const order = progState.base === 'adj' ? ['acc', 'ppq', 'neg'] : ['acc', 'cel', 'neg', 'ppq'];
  for (const m of order) if (progState.metrics.has(m)) return m;
  return 'acc';
}

/* The groups a split draws, largest first, capped. Color is assigned from a
   stable alphabetical order over the WHOLE log rather than by rank, so
   narrowing the scope or reordering never repaints a surviving line. */
function splitGroups(data) {
  const dim = progState.split;
  if (dim === 'none') return null;
  const keyOf = DIMS[dim];
  const groups = new Map();
  data.forEach((r, i) => {
    const k = keyOf(r);
    if (k == null || k === '') return;
    let a = groups.get(k);
    if (!a) { a = []; groups.set(k, a); }
    a.push(i);
  });
  const stable = [...new Set(LOG.map(keyOf).filter(k => k != null && k !== ''))]
    .sort(dim === 'diff' ? undefined : (a, b) => a.localeCompare(b));
  const eligible = [...groups.entries()].filter(([, v]) => v.length >= SPLIT_MIN_N);
  const shown = eligible.sort((a, b) => b[1].length - a[1].length).slice(0, SPLIT_MAX)
    .sort((a, b) => stable.indexOf(a[0]) - stable.indexOf(b[0]));
  return { shown, dropped: eligible.length - shown.length,
           thin: groups.size - eligible.length };
}

function progWindow(n) {
  return progState.wmax ? Math.max(WIN_MIN, n) : Math.min(progState.W, Math.max(WIN_MIN, n));
}
// Keep the window legal for the current scope; Max follows the scope down.
function clampWindow(n) {
  const hi = Math.max(WIN_MIN, n);
  if (progState.wmax) progState.W = hi;
  progState.W = Math.max(WIN_MIN, Math.min(progState.W, hi));
  return progState.W;
}

const PROG_MIN_ROWS = 25;
// an answerline needs a few sightings before a miss rate means anything
const ANS_MIN = 6;
function progressBlock(data) {
  let h = '<h2 class="sechead">Progress</h2>';
  if (data.length < PROG_MIN_ROWS)
    return h + '<div class="statnote">A trend needs about ' + PROG_MIN_ROWS
      + ' questions; this scope has ' + data.length + '.</div>';
  const n = clampWindow(data.length);
  const mbtn = m => '<button class="dimbtn' + (progState.metrics.has(m) ? ' on' : '')
    + '" data-pm="' + m + '">' + METRIC_LBL[m] + '</button>';
  h += '<div class="ctlrow" id="progctl">'
    + '<div class="ctlgrp"><span class="ctllbl">x</span>'
    + '<button class="dimbtn' + (progState.x === 'idx' ? ' on' : '') + '" data-px="idx">Attempts</button>'
    + '<button class="dimbtn' + (progState.x === 'date' ? ' on' : '') + '" data-px="date">Calendar</button></div>'
    + '<div class="ctlgrp"><span class="ctllbl">show</span>' + mbtn('acc') + mbtn('cel') + mbtn('neg') + mbtn('ppq') + '</div>'
    + '<div class="ctlgrp"><span class="ctllbl">split by</span>'
    + SPLIT_ORDER.map(s => '<button class="dimbtn' + (progState.split === s ? ' on' : '')
        + '" data-ps="' + s + '">' + SPLIT_LABEL[s] + '</button>').join('') + '</div>'
    + '<div class="ctlgrp"><span class="ctllbl">baseline</span>'
    + '<button class="dimbtn' + (progState.base === 'raw' ? ' on' : '') + '" data-pb="raw">Raw</button>'
    + '<button class="dimbtn' + (progState.base === 'adj' ? ' on' : '') + '" data-pb="adj">Vs my average</button></div>'
    + '<div class="ctlgrp"><span class="ctllbl">window</span>'
    + '<input type="range" id="progwin" class="wslider" min="' + WIN_MIN + '" max="' + Math.max(WIN_MIN, data.length)
    + '" step="1" value="' + n + '"' + (progState.wmax ? ' disabled' : '') + '>'
    + '<input type="number" id="progwinn" class="wnum" min="' + WIN_MIN + '" max="' + Math.max(WIN_MIN, data.length)
    + '" step="1" value="' + n + '">'
    + '<button class="dimbtn' + (progState.wmax ? ' on' : '') + '" id="progmax">Max</button></div>'
    + '</div>'
    + '<div class="progwrap"><canvas id="progcanvas" class="progcv"></canvas><div class="progtip" id="progtip"></div></div>'
    + '<div class="proglegend" id="proglegend"></div>'
    + '<div class="statnote" id="prognote"></div>';
  return h;
}

// hover x in canvas px, or null. Kept outside the redraw so a repaint
// during a drag doesn't drop the crosshair.
let progHover = null, progGeom = null;

/* Series for the chart. Three shapes:
   - split off, raw:  one line per enabled metric, plus any pinned rows
   - split on:        one line per group of the split dimension, one metric
   - baseline "vs my average": the line is the rolling gap between what a
     group returned and what the difficulty-and-date model expected of an
     average you. Overall metric lines are omitted there, because their
     residual is zero by construction and says nothing. */
function progSeries(data) {
  const win = progWindow(data.length);
  const adj = progState.base === 'adj';
  const out = [];

  const split = splitGroups(data);
  if (split) {
    const metric = splitMetric();
    const mdl = adj ? strengthModel(data, STRENGTH_METRIC[metric] ? metric : 'acc', progState.split) : null;
    if (adj && !mdl) return out;
    const M = adj ? STRENGTH_METRIC[metric] : null;
    split.shown.forEach(([name, idxs], i) => {
      const col = SPLIT_COLS[i % SPLIT_COLS.length];
      const label = progState.split === 'diff' ? String(name) : name;
      if (progState.hidden.has(label)) { out.push({ label, col, pts: [], off: true, metric }); return; }
      const subWin = Math.max(WIN_MIN, Math.round(win * idxs.length / data.length));
      let pts;
      if (adj) {
        pts = rollMean(idxs.map(i2 => ({ ts: data[i2].ts, i: i2, y: M.val(data[i2]) - mdl.expect[i2] })), subWin);
      } else {
        pts = rollup(idxs.map(i2 => data[i2]), subWin, metric);
        // rollup indexes within the subset; remap onto the full x axis
        pts = pts.map(p => ({ ...p, i: idxs[p.i] }));
      }
      out.push({ label, col, pts, band: false, adj, metric });
    });
    return out;
  }

  if (adj) {
    const metric = splitMetric();
    const mdl = strengthModel(data, metric);
    if (!mdl) return out;
    const M = STRENGTH_METRIC[metric];
    progState.pins.forEach((pin, i) => {
      const sub = [];
      data.forEach((r, idx) => {
        if (compositeKey(r) === pin) sub.push({ ts: r.ts, i: idx, y: M.val(r) - mdl.expect[idx] });
      });
      if (sub.length < 20) return;
      const subWin = Math.max(WIN_MIN, Math.round(win * sub.length / data.length));
      out.push({
        label: pin.split(KEY_SEP).join(' · '), col: SERIES_COLS[i % SERIES_COLS.length],
        pts: rollMean(sub, subWin), band: false, adj: true, metric,
      });
    });
    return out;
  }

  for (const m of ['acc', 'cel', 'neg', 'ppq']) {
    if (!progState.metrics.has(m)) continue;
    out.push({
      label: METRIC_LBL[m], col: METRIC_COL[m],
      pts: rollup(data, win, m), band: progState.pins.length === 0, metric: m,
    });
  }
  progState.pins.forEach((pin, i) => {
    const sub = data.filter(r => compositeKey(r) === pin);
    if (sub.length < 20) return;
    // Scale the window to the subset so a pinned row is smoothed over the
    // same stretch of TIME as the overall line, not the same question count.
    const subWin = Math.max(WIN_MIN, Math.round(win * sub.length / data.length));
    out.push({
      label: pin.split(KEY_SEP).join(' · '), col: SERIES_COLS[i % SERIES_COLS.length],
      pts: rollup(sub, subWin, 'acc'), band: false, dash: true, metric: 'acc',
    });
  });
  return out;
}

// trailing rolling mean of arbitrary values (the residual series), shaped
// like rollup's output so the drawing code does not care which it got
function rollMean(vals, W) {
  const n = vals.length;
  W = Math.max(2, Math.min(W, n));
  const c = new Float64Array(n + 1);
  for (let i = 0; i < n; i++) c[i + 1] = c[i] + vals[i].y;
  const out = [];
  for (let i = 0; i < n; i++) {
    if (i + 1 < Math.min(W, MIN_WIN_PTS)) continue;
    const a = Math.max(0, i - W + 1), b = i + 1, cnt = b - a;
    const v = (c[b] - c[a]) / cnt;
    out.push({ i: vals[i].i, ts: vals[i].ts, v, lo: v, hi: v, n: cnt });
  }
  return out;
}

function nearestPt(pts, xv, xof) {
  let best = null, bd = Infinity;
  for (const p of pts) { const d = Math.abs(xof(p) - xv); if (d < bd) { bd = d; best = p; } }
  return best;
}
const fmtDay = ts => new Date(ts).toLocaleDateString(undefined, { month: 'short', day: 'numeric' });

function drawProgress(data) {
  const cv = $('progcanvas'); if (!cv || !cv.getContext) return;
  if (data.length < PROG_MIN_ROWS) return;
  const ctx = cv.getContext('2d');
  const dpr = window.devicePixelRatio || 1;
  const W = cv.clientWidth, H = cv.clientHeight;
  if (!W || !H) return;
  cv.width = Math.round(W * dpr); cv.height = Math.round(H * dpr);
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  ctx.clearRect(0, 0, W, H);
  const L = 44, R = 16, T = 14, B = 40;
  const px0 = L, px1 = W - R, py0 = T, py1 = H - B;
  const cBorder = '#3a3f47', cFaint = '#808790';
  const byDate = progState.x === 'date';
  const xmin = byDate ? data[0].ts : 0;
  const xmax = byDate ? data[data.length - 1].ts : data.length - 1;
  const X = v => px0 + (xmax === xmin ? 0.5 : (v - xmin) / (xmax - xmin)) * (px1 - px0);
  const xof = p => byDate ? p.ts : p.i;
  // series first: in adjusted mode the y range comes from the residuals,
  // which are a delta around zero rather than a 0-1 rate
  const series = progSeries(data);
  const adj = progState.base === 'adj';
  const adjM = adj && series.length ? STRENGTH_METRIC[series[0].metric] : null;
  const vals = series.flatMap(s => s.pts.map(p => p.v));
  let ymin = 0, ymax = 1;
  if (adj) {
    const mx = Math.max(0.02 / (adjM ? adjM.scale / 100 : 1), ...vals.map(Math.abs)) * 1.3;
    ymin = -mx; ymax = mx;
  } else if (series.length && series.every(s => s.metric === 'ppq') && vals.length) {
    // points per question is not a 0-1 rate; give it its own room
    ymin = Math.min(0, ...vals) - 1; ymax = Math.max(...vals) + 1;
  }
  const Y = v => py1 - (v - ymin) / (ymax - ymin) * (py1 - py0);

  // how much you played, own scale, recessive
  const NB = Math.min(48, Math.max(12, Math.round((px1 - px0) / 22)));
  const bins = new Array(NB).fill(0);
  for (let i = 0; i < data.length; i++) {
    const f = byDate ? (xmax === xmin ? 0 : (data[i].ts - xmin) / (xmax - xmin)) : i / data.length;
    bins[Math.min(NB - 1, Math.floor(f * NB))]++;
  }
  const maxB = Math.max(...bins, 1), bw = (px1 - px0) / NB;
  ctx.fillStyle = 'rgba(128,135,144,0.13)';
  for (let i = 0; i < NB; i++) {
    const bh = (bins[i] / maxB) * (py1 - py0) * 0.26;
    ctx.fillRect(px0 + i * bw + 1, py1 - bh, Math.max(1, bw - 2), bh);
  }

  ctx.font = '11px -apple-system,Segoe UI,sans-serif'; ctx.lineWidth = 1;
  for (let g = 0; g <= 4; g++) {
    const gy = ymin + (ymax - ymin) * g / 4;
    ctx.strokeStyle = cBorder; ctx.globalAlpha = (!adj && g === 0) ? 0.6 : 0.26;
    ctx.beginPath(); ctx.moveTo(px0, Y(gy)); ctx.lineTo(px1, Y(gy)); ctx.stroke();
    ctx.globalAlpha = 1; ctx.fillStyle = cFaint; ctx.textAlign = 'right';
    ctx.fillText(adjM ? fmtDelta(gy, adjM)
      : (ymax > 1.5 || ymin < 0) ? gy.toFixed(1) : Math.round(gy * 100) + '%', px0 - 6, Y(gy) + 3);
  }
  if (adj) {   // the reference the whole view is a distance from
    ctx.strokeStyle = '#c8ccd1'; ctx.globalAlpha = 0.5; ctx.lineWidth = 1.5;
    ctx.setLineDash([4, 3]);
    ctx.beginPath(); ctx.moveTo(px0, Y(0)); ctx.lineTo(px1, Y(0)); ctx.stroke();
    ctx.setLineDash([]); ctx.globalAlpha = 1; ctx.lineWidth = 1;
    ctx.fillStyle = cFaint; ctx.textAlign = 'left';
    ctx.fillText('my average', px0 + 6, Y(0) - 5);
  }
  ctx.textAlign = 'center'; ctx.fillStyle = cFaint;
  for (let i = 0; i <= 4; i++) {
    const v = xmin + (xmax - xmin) * i / 4;
    ctx.fillText(byDate ? fmtDay(v) : String(Math.round(v)), X(v), py1 + 16);
  }
  ctx.fillText(byDate ? 'date' : 'questions played', (px0 + px1) / 2, py1 + 32);
  if (byDate) {   // one tick per session
    let last = null;
    ctx.strokeStyle = cBorder; ctx.globalAlpha = 0.3;
    for (const r of data) {
      const d = new Date(r.ts).toDateString();
      if (d === last) continue;
      last = d;
      ctx.beginPath(); ctx.moveTo(X(r.ts), py1); ctx.lineTo(X(r.ts), py1 - 5); ctx.stroke();
    }
    ctx.globalAlpha = 1;
  }

  for (const s of series) {
    if (!s.pts.length || s.off) continue;
    if (s.band) {
      ctx.fillStyle = s.col + '22'; ctx.beginPath();
      s.pts.forEach((p, i) => { ctx[i ? 'lineTo' : 'moveTo'](X(xof(p)), Y(p.hi)); });
      for (let i = s.pts.length - 1; i >= 0; i--) ctx.lineTo(X(xof(s.pts[i])), Y(s.pts[i].lo));
      ctx.closePath(); ctx.fill();
    }
    ctx.strokeStyle = s.col; ctx.lineWidth = s.dash ? 1.8 : 2.2;
    ctx.setLineDash(s.dash ? [5, 4] : []); ctx.lineJoin = 'round';
    ctx.beginPath();
    s.pts.forEach((p, i) => { ctx[i ? 'lineTo' : 'moveTo'](X(xof(p)), Y(p.v)); });
    ctx.stroke(); ctx.setLineDash([]);
    const e = s.pts[s.pts.length - 1];
    ctx.beginPath(); ctx.arc(X(xof(e)), Y(e.v), 4, 0, 6.2832);
    ctx.fillStyle = s.col; ctx.fill();
    ctx.lineWidth = 2; ctx.strokeStyle = '#1a1f25'; ctx.stroke();
  }
  progGeom = { X, Y, xof, series, px0, px1, byDate, xmin, xmax };

  if (progHover != null && progHover >= px0 && progHover <= px1) {
    const xv = xmin + (progHover - px0) / (px1 - px0) * (xmax - xmin);
    ctx.strokeStyle = cFaint; ctx.globalAlpha = 0.5; ctx.setLineDash([3, 3]);
    ctx.beginPath(); ctx.moveTo(X(xv), py0); ctx.lineTo(X(xv), py1); ctx.stroke();
    ctx.setLineDash([]); ctx.globalAlpha = 1;
    for (const s of series) {
      if (s.off) continue;
      const p = nearestPt(s.pts, xv, xof); if (!p) continue;
      ctx.beginPath(); ctx.arc(X(xof(p)), Y(p.v), 4.5, 0, 6.2832);
      ctx.fillStyle = s.col; ctx.fill();
      ctx.lineWidth = 2; ctx.strokeStyle = '#1a1f25'; ctx.stroke();
    }
  }
  ctx.strokeStyle = cBorder; ctx.globalAlpha = 0.6; ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(px0, py0); ctx.lineTo(px0, py1); ctx.lineTo(px1, py1); ctx.stroke();
  ctx.globalAlpha = 1;

  const leg = $('proglegend');
  if (leg) leg.innerHTML = series.length < 2 ? '' : series.map(s =>
    '<button class="legitem' + (s.off ? ' off' : '') + '" data-leg="'
    + esc(String(s.label)).replace(/"/g, '&quot;') + '">'
    + '<span class="sw' + (s.dash ? ' dash' : '') + '" style="'
    + (s.dash ? 'border-color:' + s.col : 'background:' + s.col) + '"></span>'
    + esc(String(s.label)) + (s.dash ? ' acc' : '') + '</button>').join('');
  const note = $('prognote');
  if (!note) return;
  const split = progState.split !== 'none';
  const parts = [];
  if (split) {
    const s = splitGroups(data);
    parts.push('One line per ' + SPLIT_LABEL[progState.split].toLowerCase() + ', '
      + METRIC_LBL[splitMetric()].toLowerCase() + ', rolled over each line’s own share of '
      + progWindow(data.length) + ' questions.');
    if (s && s.dropped) parts.push(s.dropped + ' more past the ' + SPLIT_MAX + '-line cap are not drawn.');
    if (s && s.thin) parts.push(s.thin + ' under ' + SPLIT_MIN_N + ' questions are too thin to plot.');
    if (!series.length) parts.push('Nothing in scope has ' + SPLIT_MIN_N + '+ questions at this split.');
    parts.push('Click the legend to hide a line.');
  } else if (adj) {
    parts.push(!series.length
      ? 'Pin rows in the Breakdown to compare them against your average.'
      : 'Each line is a pinned row against what its difficulty and date predicted.');
  } else {
    parts.push(progState.wmax
      ? 'Running average over the whole scope (' + data.length + ' questions).'
      : 'Rolling average over the last ' + progWindow(data.length) + ' questions in scope.');
    parts.push(progState.pins.length ? 'Dashed: pinned breakdown rows.' : 'Band: 95% interval.');
  }
  if (adj) parts.push('Zero is your own average, once difficulty and date are accounted for.');
  if (!split) parts.push('Faint bars: how much you played.');
  note.textContent = parts.join(' ');
}

// One sparkline per breakdown row: the same rolling accuracy, sized to the
// row's own history so short rows aren't drawn as a flat line.
function drawSpark(el, rows) {
  if (!el || !el.getContext) return;
  const dpr = window.devicePixelRatio || 1;
  const W = el.clientWidth || 96, H = el.clientHeight || 22;
  if (!W || !H) return;
  el.width = Math.round(W * dpr); el.height = Math.round(H * dpr);
  const ctx = el.getContext('2d');
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  ctx.clearRect(0, 0, W, H);
  const pts = rollup(rows, Math.min(30, Math.max(10, Math.floor(rows.length / 4))), 'acc');
  if (pts.length < 3) return;
  const x0 = pts[0].i, x1 = pts[pts.length - 1].i;
  const X = i => 2 + (i - x0) / Math.max(1, x1 - x0) * (W - 8);
  const Y = v => (H - 3) - v * (H - 7);
  ctx.strokeStyle = '#3a3f47'; ctx.globalAlpha = 0.6; ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(2, Y(0.5)); ctx.lineTo(W - 4, Y(0.5)); ctx.stroke();
  ctx.globalAlpha = 1;
  const pin = progState.pins.indexOf(el.dataset.k);
  const col = pin >= 0 ? SERIES_COLS[pin % SERIES_COLS.length] : '#9aa0a7';
  ctx.strokeStyle = col; ctx.lineWidth = 1.6; ctx.lineJoin = 'round';
  ctx.beginPath();
  pts.forEach((p, i) => { ctx[i ? 'lineTo' : 'moveTo'](X(p.i), Y(p.v)); });
  ctx.stroke();
  const e = pts[pts.length - 1];
  ctx.beginPath(); ctx.arc(X(e.i), Y(e.v), 2.4, 0, 6.2832);
  ctx.fillStyle = col; ctx.fill();
}

/* Controls repaint the canvas in place rather than re-rendering the stats
   body — a full re-render would rebuild the table and steal focus from the
   window field mid-keystroke. Only pinning, which changes the table, calls
   renderStats. */
let progResizeBound = false;
function wireProgress(data, spark) {
  const cv = $('progcanvas');
  if (cv) {
    drawProgress(data);
    const at = clientX => {
      const r = cv.getBoundingClientRect();
      progHover = clientX - r.left;
      drawProgress(data);
      showProgTip(cv);
    };
    cv.addEventListener('mousemove', e => at(e.clientX));
    cv.addEventListener('touchmove', e => { if (e.touches[0]) at(e.touches[0].clientX); });
    const off = () => {
      progHover = null;
      const t = $('progtip'); if (t) t.style.opacity = 0;
      drawProgress(data);
    };
    cv.addEventListener('mouseleave', off);
    cv.addEventListener('touchend', off);
    if (!progResizeBound) {
      progResizeBound = true;
      window.addEventListener('resize', () => {
        if ($('view-stats') && $('view-stats').style.display !== 'none') drawProgress(scopedLog());
      });
    }
  }
  const leg = $('proglegend');
  if (leg) leg.addEventListener('click', e => {
    const b = e.target.closest('.legitem');
    if (!b || !b.dataset.leg) return;
    const k = b.dataset.leg;
    if (progState.hidden.has(k)) progState.hidden.delete(k);
    else progState.hidden.add(k);
    drawProgress(data);
  });
  const ctl = $('progctl');
  if (ctl) {
    ctl.addEventListener('click', e => {
      const b = e.target.closest('.dimbtn'); if (!b) return;
      if (b.dataset.px) {
        progState.x = b.dataset.px;
        for (const o of ctl.querySelectorAll('[data-px]')) o.classList.toggle('on', o === b);
      } else if (b.dataset.pm) {
        const m = b.dataset.pm;
        if (progState.metrics.has(m)) { if (progState.metrics.size > 1) progState.metrics.delete(m); }
        else progState.metrics.add(m);
        for (const o of ctl.querySelectorAll('[data-pm]'))
          o.classList.toggle('on', progState.metrics.has(o.dataset.pm));
      } else if (b.dataset.ps) {
        progState.split = b.dataset.ps;
        progState.hidden.clear();
        for (const o of ctl.querySelectorAll('[data-ps]')) o.classList.toggle('on', o === b);
      } else if (b.dataset.pb) {
        progState.base = b.dataset.pb;
        for (const o of ctl.querySelectorAll('[data-pb]')) o.classList.toggle('on', o === b);
      } else if (b.id === 'progmax') {
        progState.wmax = !progState.wmax;
        clampWindow(data.length);
        b.classList.toggle('on', progState.wmax);
        $('progwin').disabled = progState.wmax;
        $('progwin').value = progState.W;
        $('progwinn').value = progState.W;
      } else return;
      drawProgress(data);
    });
    const setWin = (v, skipField) => {
      progState.wmax = false;
      progState.W = Math.round(v) || WIN_MIN;
      clampWindow(data.length);
      $('progwin').value = progState.W;
      $('progwin').disabled = false;
      if (!skipField) $('progwinn').value = progState.W;
      $('progmax').classList.remove('on');
      drawProgress(data);
    };
    $('progwin').addEventListener('input', function () { setWin(+this.value); });
    // while typing, leave the field alone until the value is usable
    $('progwinn').addEventListener('input', function () {
      const v = +this.value;
      if (this.value === '' || isNaN(v) || v < WIN_MIN) return;
      setWin(v, true);
    });
    $('progwinn').addEventListener('change', function () { setWin(+this.value); });
  }
  if (!spark) return;
  for (const el of document.querySelectorAll('canvas.spark')) {
    const k = spark.keys[+el.dataset.si];
    const rows = spark.map.get(k);
    if (!rows) continue;
    el.dataset.k = k;
    drawSpark(el, rows);
    el.onclick = () => {
      const i = progState.pins.indexOf(k);
      if (i >= 0) progState.pins.splice(i, 1);
      else if (progState.pins.length < PROG_MAX_PINS) progState.pins.push(k);
      else return;
      renderStats();
    };
  }
}

function showProgTip(cv) {
  const tip = $('progtip');
  if (!tip || !progGeom || progHover == null || progHover < progGeom.px0 || progHover > progGeom.px1) {
    if (tip) tip.style.opacity = 0;
    return;
  }
  const g = progGeom;
  const xv = g.xmin + (progHover - g.px0) / (g.px1 - g.px0) * (g.xmax - g.xmin);
  const at = [];
  for (const s of g.series) {
    if (s.off) continue;
    const p = nearestPt(s.pts, xv, g.xof);
    if (p) at.push({ s, p });
  }
  if (!at.length) { tip.style.opacity = 0; return; }
  tip.innerHTML = '<div class="tth">'
    + (g.byDate ? fmtDay(at[0].p.ts) : 'question ' + (at[0].p.i + 1)) + '</div>'
    + at.map(o => '<div class="ttr"><i style="color:' + o.s.col + '">' + esc(o.s.label)
      + '</i><span>' + (o.s.adj ? fmtDelta(o.p.v, STRENGTH_METRIC[o.s.metric]) : fmtPct(o.p.v))
      + '</span></div>').join('');
  const w = tip.offsetWidth || 150;
  tip.style.left = Math.min(cv.clientWidth - w - 6, Math.max(6, progHover + 16)) + 'px';
  tip.style.top = '16px';
  tip.style.opacity = 1;
}

/* ---------- difficulty-adjusted baseline ----------
   Raw accuracy per category ranks your categories by what you PLAYED, not
   by how you played it. Three confounds: the difficulty each category was
   played at, when it was played (you improve over time), and how much.

   So fit one model over every attempt in scope:

       outcome ~ intercept + t + t^2 + difficulty + group

   Because difficulty and date enter the same way for every group, the group
   term is what is left once they are accounted for. Groups are effect-coded
   (the last one is minus the sum of the rest), which makes each term a
   deviation from YOUR average rather than from an arbitrary reference
   group, and that is the quantity being asked for.

   The reported number is an average marginal effect: swap the group in
   across your whole history and read the change in the metric's own units
   (accuracy points, or points per question). Groups too small to fit are
   pooled into one bucket that is estimated but not shown, so they still
   inform the difficulty and time terms.

   If Difficulty is one of the active breakdown dimensions it is dropped
   from the covariates, because the group term would otherwise be the same
   thing twice. */

const STRENGTH_MIN_GROUP = 25;   // below this a group is pooled, not shown
const STRENGTH_MIN_ROWS = 60;
const STRENGTH_MAX_GROUPS = 40;  // guard on the size of the linear system

const STRENGTH_METRIC = {
  acc: { label: 'Accuracy', unit: 'pp', binary: true, scale: 100,
         val: r => (r.res === 'C' ? 1 : 0), dec: 1 },
  ppq: { label: 'Points', unit: 'pts', binary: false, scale: 1,
         val: r => (r.res === 'C' ? (r.pow ? 15 : 10) : r.res === 'W' ? -5 : 0), dec: 2 },
};
const fmtDelta = (v, m) => (v >= 0 ? '+' : '−')
  + Math.abs(v * m.scale).toFixed(m.dec);

// Gauss-Jordan with partial pivoting. p is ~10-40 here, so a dense solve is
// the right tool and a decomposition would only add code.
function solveDense(A, b) {
  const n = A.length, m = A.map((r, i) => r.concat([b[i]]));
  for (let c = 0; c < n; c++) {
    let piv = c;
    for (let r = c + 1; r < n; r++) if (Math.abs(m[r][c]) > Math.abs(m[piv][c])) piv = r;
    const t = m[c]; m[c] = m[piv]; m[piv] = t;
    if (Math.abs(m[c][c]) < 1e-12) continue;
    for (let r = 0; r < n; r++) {
      if (r === c) continue;
      const f = m[r][c] / m[c][c];
      for (let k = c; k <= n; k++) m[r][k] -= f * m[c][k];
    }
  }
  return m.map((r, i) => r[n] / (m[i][i] || 1e-12));
}
function invDense(A) {
  const n = A.length, cols = [];
  for (let j = 0; j < n; j++) {
    const e = new Array(n).fill(0); e[j] = 1;
    cols.push(solveDense(A.map(r => r.slice()), e));
  }
  return A.map((_, i) => cols.map(col => col[i]));
}
const sigmoid = z => 1 / (1 + Math.exp(-z));

function fitOLS(X, y, ridge) {
  const n = X.length, p = X[0].length, H = [], g = new Array(p).fill(0);
  for (let j = 0; j < p; j++) { H.push(new Array(p).fill(0)); H[j][j] = ridge; }
  for (let i = 0; i < n; i++)
    for (let j = 0; j < p; j++) {
      g[j] += X[i][j] * y[i];
      for (let l = j; l < p; l++) H[j][l] += X[i][j] * X[i][l];
    }
  for (let j = 0; j < p; j++) for (let l = 0; l < j; l++) H[j][l] = H[l][j];
  const b = solveDense(H.map(r => r.slice()), g);
  let sse = 0;
  for (let i = 0; i < n; i++) {
    let e = 0;
    for (let j = 0; j < p; j++) e += X[i][j] * b[j];
    sse += (y[i] - e) * (y[i] - e);
  }
  const s2 = sse / Math.max(1, n - p), Hi = invDense(H);
  return { b, cov: Hi.map(r => r.map(v => v * s2)) };
}
function fitLogitIRLS(X, y, ridge) {
  const n = X.length, p = X[0].length, b = new Array(p).fill(0);
  let H = null;
  for (let it = 0; it < 40; it++) {
    const g = new Array(p).fill(0);
    H = [];
    for (let j = 0; j < p; j++) { H.push(new Array(p).fill(0)); H[j][j] = ridge; }
    for (let i = 0; i < n; i++) {
      let eta = 0;
      for (let j = 0; j < p; j++) eta += X[i][j] * b[j];
      const mu = sigmoid(eta), w = Math.max(mu * (1 - mu), 1e-6), res = y[i] - mu;
      for (let j = 0; j < p; j++) {
        g[j] += X[i][j] * res;
        for (let l = j; l < p; l++) H[j][l] += w * X[i][j] * X[i][l];
      }
    }
    for (let j = 0; j < p; j++) for (let l = 0; l < j; l++) H[j][l] = H[l][j];
    const d = solveDense(H.map(r => r.slice()), g);
    let step = 0;
    for (let j = 0; j < p; j++) { b[j] += d[j]; step += Math.abs(d[j]); }
    if (step < 1e-9) break;
  }
  return { b, cov: invDense(H) };
}

// The fit is the expensive part of the stats view, and the chart's window
// slider repaints on every drag, so it is cached against everything that
// could change it.
let strengthCache = { key: null, out: null };
function strengthKey(data, metric, dim) {
  return [metric, dim || DIM_ORDER.filter(d => statsDims.has(d)).join(','), data.length,
    data.length ? data[0].ts + '-' + data[data.length - 1].ts : '0',
    DIM_ORDER.map(f => [...statsScope[f]].sort().join('+')).join('/')].join('|');
}

/* Returns { effects, expect, adjDiff, pooled } or null when the scope is too
   thin to fit. `expect` is aligned 1:1 with `data`: the metric this attempt
   would be expected to yield from difficulty and date alone, with the group
   term removed. Rolling the residual gives a group's advantage over time. */
function strengthModel(data, metric, dim) {
  const key = strengthKey(data, metric, dim);
  if (strengthCache.key === key) return strengthCache.out;
  const out = buildStrength(data, metric, dim);
  strengthCache = { key, out };
  return out;
}
function buildStrength(data, metric, dim) {
  const M = STRENGTH_METRIC[metric];
  if (!M || data.length < STRENGTH_MIN_ROWS) return null;
  // group every row; anything too small to estimate joins one pooled bucket
  const keyOf = dim ? DIMS[dim] : compositeKey;
  const raw = data.map(keyOf);
  const counts = new Map();
  for (const k of raw) if (k != null) counts.set(k, (counts.get(k) || 0) + 1);
  const shown = [...counts.entries()].filter(([, n]) => n >= STRENGTH_MIN_GROUP)
    .map(([k]) => k).sort();
  if (shown.length < 2 || shown.length > STRENGTH_MAX_GROUPS) return null;
  // U+0001 cannot occur in a composite key, so the pooled bucket can never
  // collide with a real group name.
  const POOL = '\u0001other';
  const gOf = raw.map(k => (k != null && counts.get(k) >= STRENGTH_MIN_GROUP) ? k : POOL);
  // The pooled bucket only becomes a group if something landed in it. An
  // empty group would leave the effect-coding columns summing to the
  // intercept, which is exactly collinear and makes every variance garbage.
  const nPooled = gOf.reduce((a, g) => a + (g === POOL ? 1 : 0), 0);
  const groups = nPooled ? shown.concat([POOL]) : shown.slice();
  const gi = new Map(groups.map((g, i) => [g, i]));
  const k = groups.length;

  // difficulty is dropped when the breakdown already splits on it, or when
  // the scope has pinned it to a single value
  const diffs = [...new Set(data.map(r => r.diff).filter(d => d != null))].sort();
  const adjDiff = (dim ? dim !== 'diff' : !statsDims.has('diff')) && diffs.length > 1;
  const dCols = adjDiff ? diffs.slice(1) : [];

  const n = data.length;
  const X = new Array(n);
  for (let i = 0; i < n; i++) {
    // time is centred so the linear and quadratic terms are not nearly
    // the same column, which would leave the system badly conditioned
    const t = i / Math.max(1, n - 1) - 0.5;
    const row = [1, t, t * t - 1 / 12];
    for (const d of dCols) row.push(data[i].diff === d ? 1 : 0);
    const g = gi.get(gOf[i]);
    for (let j = 0; j < k - 1; j++) row.push(g === k - 1 ? -1 : (g === j ? 1 : 0));
    X[i] = row;
  }
  const nFixed = 3 + dCols.length;
  const y = data.map(M.val);
  const fit = M.binary ? fitLogitIRLS(X, y, 1e-3) : fitOLS(X, y, 1e-3);
  if (fit.b.some(v => !isFinite(v))) return null;

  // Effect coding gives the k-1 free terms; the last group is minus their
  // sum, and the variance of that sum is the sum of its covariance block.
  const nc = k - 1;
  const gam = [], Cf = [];
  for (let j = 0; j < nc; j++) gam.push(fit.b[nFixed + j]);
  let s = 0;
  for (let a = 0; a < nc; a++) s += gam[a];
  gam.push(-s);
  // full k x k covariance of the group terms, via L = [I ; -1]
  const Lrow = (a) => (i) => (a < nc ? (a === i ? 1 : 0) : -1);
  for (let a = 0; a < k; a++) {
    Cf.push([]);
    for (let b = 0; b < k; b++) {
      let acc = 0;
      const la = Lrow(a), lb = Lrow(b);
      for (let i = 0; i < nc; i++) {
        const lai = la(i); if (!lai) continue;
        for (let j = 0; j < nc; j++) {
          const lbj = lb(j); if (!lbj) continue;
          acc += lai * fit.cov[nFixed + i][nFixed + j] * lbj;
        }
      }
      Cf[a].push(acc);
    }
  }
  // Effect coding centres on the UNWEIGHTED mean across groups. Re-centre on
  // the size-weighted mean instead, so an estimate reads against the average
  // you actually posted, which is what the headline tile shows and what the
  // section claims. The contrast is (e_g - w), so its variance comes from
  // the same covariance block.
  const w = groups.map(g => (g === POOL ? nPooled : counts.get(g)) / n);
  let gbar = 0;
  for (let a = 0; a < k; a++) gbar += w[a] * gam[a];
  let wCw = 0;
  for (let a = 0; a < k; a++) for (let b = 0; b < k; b++) wCw += w[a] * w[b] * Cf[a][b];
  const varOf = idx => {
    let cross = 0;
    for (let b = 0; b < k; b++) cross += w[b] * Cf[idx][b];
    return Math.max(Cf[idx][idx] - 2 * cross + wCw, 0);
  };

  // prediction with the group term replaced by the weighted-average group,
  // i.e. what an average you would have been expected to score here
  const eta0 = new Float64Array(n);
  for (let i = 0; i < n; i++) {
    let e = gbar;
    for (let j = 0; j < nFixed; j++) e += X[i][j] * fit.b[j];
    eta0[i] = e;
  }
  const expect = new Float64Array(n);
  let baseSum = 0, densSum = 0;
  for (let i = 0; i < n; i++) {
    const p = M.binary ? sigmoid(eta0[i]) : eta0[i];
    expect[i] = p;
    baseSum += p;
    if (M.binary) densSum += p * (1 - p);
  }
  const base = baseSum / n, dens = M.binary ? densSum / n : 1;

  const effects = [];
  groups.forEach((g, idx) => {
    if (g === POOL) return;
    const d = gam[idx] - gbar;           // this group against the average you
    let est;
    if (M.binary) {
      let sum = 0;
      for (let i = 0; i < n; i++) sum += sigmoid(eta0[i] + d);
      est = sum / n - base;
    } else est = d;
    const se = Math.sqrt(varOf(idx)) * dens;
    effects.push({ k: g, n: counts.get(g), est, se,
      lo: est - 1.96 * se, hi: est + 1.96 * se, sig: Math.abs(est) > 1.96 * se });
  });
  effects.sort((a, b) => b.est - a.est);
  return { effects, expect, adjDiff, pooled: nPooled };
}

// sortable stat table. cols: seen, acc, cel, neg. weakest metric first.
function statTable(rows, opts) {
  opts = opts || {};
  rows = rows.filter(r => r.n >= (opts.min || 1));
  if (!rows.length) return '<div class="statnote">Not enough data yet.</div>';
  const col = statsSort.col, dir = statsSort.dir;
  rows.sort((a, b) => {
    let va = a[col], vb = b[col];
    if (va == null) va = dir > 0 ? Infinity : -Infinity;
    if (vb == null) vb = dir > 0 ? Infinity : -Infinity;
    return dir * (va - vb) || b.n - a.n;
  });
  const arrow = c => statsSort.col === c ? (statsSort.dir > 0 ? ' ▲' : ' ▼') : '';
  const th = (c, label) => '<th class="sorth" data-col="' + c + '">' + label + arrow(c) + '</th>';
  // The Trend column addresses rows by index: composite keys are NUL-joined
  // and can't ride in an attribute. opts.spark.keys is filled in row order.
  const spark = opts.spark || null;
  let h = '<table class="acc"><tr><th>' + (opts.label || '') + '</th>'
    + th('n', 'Seen') + th('acc', 'Accuracy') + th('cel', 'Celerity')
    + th('neg', 'Neg') + (spark ? '<th class="sparkhead">Trend</th>' : '') + '<th></th></tr>';
  for (const r of rows) {
    const pin = spark ? progState.pins.indexOf(r.k) : -1;
    h += '<tr' + (pin >= 0 ? ' class="pinned" style="--pincol:' + SERIES_COLS[pin % SERIES_COLS.length] + '"' : '')
      + '><td class="name">' + (opts.name ? opts.name(r.k) : esc(r.k)) + '</td>'
      + '<td>' + r.n + '</td>'
      + '<td><span class="bar"><i style="width:' + Math.round(r.acc * 100) + '%;background:' + pctCol(r.acc) + '"></i></span><span class="pct">' + fmtPct(r.acc) + '</span></td>'
      + '<td>' + (r.cel == null ? '<span class="pct" style="color:var(--faint)">—</span>'
          : '<span class="bar"><i style="width:' + Math.round(r.cel * 100) + '%;background:' + pctCol(r.cel) + '"></i></span><span class="pct">' + fmtPct(r.cel) + '</span>') + '</td>'
      + '<td class="pct">' + fmtPct(r.neg) + '</td>';
    if (spark) {
      h += '<td class="sparkcell"><canvas class="spark" title="Plot on the progress chart" data-si="'
        + spark.keys.length + '"></canvas></td>';
      spark.keys.push(r.k);
    }
    h += '<td>' + (opts.action ? opts.action(r.k) : '') + '</td></tr>';
  }
  return '<div class="tablewrap">' + h + '</table></div>';
}

function trendBlock(data) {
  // last 50 vs prior 50 attempts: accuracy + celerity deltas
  if (data.length < 20) return '';
  const N = Math.min(50, Math.floor(data.length / 2));
  const recent = data.slice(-N), prior = data.slice(-2 * N, -N);
  const acc = a => a.filter(r => r.res === 'C').length / a.length;
  const cel = a => { const b = a.filter(r => r.res === 'C' && r.bf != null); return b.length ? b.reduce((s, r) => s + (1 - r.bf), 0) / b.length : null; };
  const dAcc = acc(recent) - acc(prior), dCel = (cel(recent) || 0) - (cel(prior) || 0);
  const arrow = d => d > 0.01 ? '<span style="color:var(--good)">▲ ' + Math.round(d * 100) + '%</span>' : d < -0.01 ? '<span style="color:var(--bad)">▼ ' + Math.round(-d * 100) + '%</span>' : '<span style="color:var(--faint)">flat</span>';
  return '<div class="statnote">Trend (last ' + N + ' vs prior ' + N + '): accuracy ' + arrow(dAcc) + ' · celerity ' + arrow(dCel) + '</div>';
}

/* ---------- buzz calibration: accuracy vs buzz depth ----------
   A fitted logistic curve P(correct)=sigmoid(a+b·depth) with a 95% band,
   over the empirical per-band rates. Fitting (not interpolating) is what
   keeps it stable on the sparse per-user history; the band widens honestly
   where data is thin, and below ~40 buzzes we show nothing but a nudge. */
const CALIB_MIN = 40;
function calibrationBlock(data) {
  const buzzed = data.filter(r => r.bf != null);
  if (buzzed.length < 12) return '';
  let h = '<h2 class="sechead">Buzz calibration</h2><div class="statnote">Accuracy by buzz depth. Curve: fitted trend. Band: 95% uncertainty. Dots: your rate per depth band. Faint bars: <b style="color:var(--muted)">where you tend to buzz</b>.</div>';
  if (buzzed.length < CALIB_MIN)
    return h + '<div class="statnote" style="margin-top:-0.7rem">The curve needs about ' + CALIB_MIN + ' buzzes to fit; this scope has ' + buzzed.length + '.</div>';
  return h + '<canvas id="calibcanvas" width="720" height="280" class="calibcv"></canvas>';
}

// logistic regression P(correct)=sigmoid(a+b·depth) via IRLS, + covariance
function logisticFit(pts) {
  let a = 0, b = 0; const ridge = 1e-3;
  for (let it = 0; it < 30; it++) {
    let g0 = 0, g1 = 0, h00 = ridge, h01 = 0, h11 = ridge;
    for (const p of pts) { const eta = a + b * p.x, pr = 1 / (1 + Math.exp(-eta)), w = Math.max(pr * (1 - pr), 1e-6); g0 += p.y - pr; g1 += (p.y - pr) * p.x; h00 += w; h01 += w * p.x; h11 += w * p.x * p.x; }
    const det = h00 * h11 - h01 * h01; if (Math.abs(det) < 1e-12) break;
    const d0 = (h11 * g0 - h01 * g1) / det, d1 = (h00 * g1 - h01 * g0) / det; a += d0; b += d1;
    if (Math.abs(d0) + Math.abs(d1) < 1e-8) break;
  }
  let h00 = ridge, h01 = 0, h11 = ridge;
  for (const p of pts) { const eta = a + b * p.x, pr = 1 / (1 + Math.exp(-eta)), w = Math.max(pr * (1 - pr), 1e-6); h00 += w; h01 += w * p.x; h11 += w * p.x * p.x; }
  const det = h00 * h11 - h01 * h01;
  return { a, b, c00: h11 / det, c01: -h01 / det, c11: h00 / det };
}
function calibPred(fit, x) {
  const eta = fit.a + fit.b * x;
  const se = Math.sqrt(Math.max(fit.c00 + 2 * fit.c01 * x + fit.c11 * x * x, 0));
  const s = z => 1 / (1 + Math.exp(-z));
  return { p: s(eta), lo: s(eta - 1.96 * se), hi: s(eta + 1.96 * se) };
}
function drawCalib(data) {
  const cv = $('calibcanvas'); if (!cv || !cv.getContext) return;
  const buzzed = data.filter(r => r.bf != null); if (buzzed.length < CALIB_MIN) return;
  const ctx = cv.getContext('2d'); const W = cv.width, H = cv.height; ctx.clearRect(0, 0, W, H);
  const L = 46, R = 14, T = 16, B = 36, px0 = L, px1 = W - R, py0 = T, py1 = H - B;
  const X = x => px0 + x * (px1 - px0), Y = y => py1 - y * (py1 - py0);
  const cBorder = '#3a3f47', cFaint = '#808790', cAccent = '#e8b04a', cText = '#c8ccd1';
  const pcol = v => v < 0.5 ? '#e0655f' : v < 0.75 ? '#e8b04a' : '#5dbb7a';
  // histogram — where you buzz (own scale, subtle)
  const NB = 12, bins = new Array(NB).fill(0);
  for (const r of buzzed) bins[Math.min(NB - 1, Math.floor(r.bf * NB))]++;
  const maxB = Math.max(...bins, 1);
  ctx.fillStyle = 'rgba(128,135,144,0.16)';
  for (let i = 0; i < NB; i++) { const bw = (px1 - px0) / NB, bh = (bins[i] / maxB) * (py1 - py0) * 0.42; ctx.fillRect(px0 + i * bw + 1, py1 - bh, bw - 2, bh); }
  // grid + labels
  ctx.font = '11px -apple-system,Segoe UI,sans-serif'; ctx.lineWidth = 1;
  for (const gy of [0, 0.25, 0.5, 0.75, 1]) { ctx.strokeStyle = cBorder; ctx.globalAlpha = gy === 0 ? 0.6 : 0.28; ctx.beginPath(); ctx.moveTo(px0, Y(gy)); ctx.lineTo(px1, Y(gy)); ctx.stroke(); ctx.globalAlpha = 1; ctx.fillStyle = cFaint; ctx.textAlign = 'right'; ctx.fillText(Math.round(gy * 100) + '%', px0 - 6, Y(gy) + 3); }
  ctx.textAlign = 'center'; ctx.fillStyle = cFaint;
  for (const gx of [0, 0.25, 0.5, 0.75, 1]) ctx.fillText(Math.round(gx * 100) + '%', X(gx), py1 + 16);
  ctx.fillText('buzz depth  (0% = start of question → 100% = end)', (px0 + px1) / 2, py1 + 30);
  // fitted curve + band
  const fit = logisticFit(buzzed.map(r => ({ x: r.bf, y: r.res === 'C' ? 1 : 0 })));
  const xs = []; for (let i = 0; i <= 60; i++) xs.push(i / 60);
  const pr = xs.map(x => calibPred(fit, x));
  ctx.fillStyle = 'rgba(232,176,74,0.14)'; ctx.beginPath();
  xs.forEach((x, i) => { const p = ctx[i ? 'lineTo' : 'moveTo'](X(x), Y(pr[i].hi)); });
  for (let i = xs.length - 1; i >= 0; i--) ctx.lineTo(X(xs[i]), Y(pr[i].lo));
  ctx.closePath(); ctx.fill();
  ctx.strokeStyle = cAccent; ctx.lineWidth = 2.2; ctx.beginPath();
  xs.forEach((x, i) => { ctx[i ? 'lineTo' : 'moveTo'](X(x), Y(pr[i].p)); }); ctx.stroke();
  // empirical dots per depth band (skip bands with <3 buzzes)
  const NBK = 5;
  for (let bi = 0; bi < NBK; bi++) {
    const lo = bi / NBK, hi = (bi + 1) / NBK, cx = (lo + hi) / 2;
    const bd = buzzed.filter(r => r.bf >= lo && (bi === NBK - 1 ? r.bf <= hi : r.bf < hi));
    if (bd.length < 3) continue;
    const acc = bd.filter(r => r.res === 'C').length / bd.length, rad = Math.min(9, 3.5 + Math.sqrt(bd.length));
    ctx.beginPath(); ctx.arc(X(cx), Y(acc), rad, 0, 6.2832); ctx.fillStyle = pcol(acc); ctx.globalAlpha = 0.92; ctx.fill(); ctx.globalAlpha = 1;
    ctx.lineWidth = 1.5; ctx.strokeStyle = '#101418'; ctx.stroke();
    ctx.fillStyle = cText; ctx.textAlign = 'center'; ctx.font = '10px -apple-system,sans-serif'; ctx.fillText(bd.length + '', X(cx), Y(acc) - rad - 4);
  }
  // axis frame
  ctx.strokeStyle = cBorder; ctx.globalAlpha = 0.6; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(px0, py0); ctx.lineTo(px0, py1); ctx.lineTo(px1, py1); ctx.stroke(); ctx.globalAlpha = 1;
}

function exposureBlock(data) {
  const seenAns = new Set(data.map(r => r.ans).filter(Boolean)).size;
  const seenSec = new Set(data.map(r => r.section).filter(Boolean)).size;
  const totSec = (CAT.section_values || []).length;
  return '<div class="statnote">Exposure: <b>' + seenAns + '</b> distinct answerlines seen · <b>' + seenSec + '</b> of ' + totSec + ' groups touched.</div>';
}

function renderStats() {
  const g = $('statgrid'); const body = $('statbody');
  if (!CAT) {   // topic links need TOPICS; wait for boot
    body.innerHTML = '<div class="empty">Loading&hellip;</div>';
    return;
  }
  const n = LOG.length;
  if (!n) {
    g.innerHTML = '';
    body.innerHTML = '<div class="empty">No history yet.</div>';
    $('statnote').textContent = '';
    return;
  }
  const data = scopedLog();
  const dn = data.length;
  const c = data.filter(r => r.res === 'C').length;
  const negs = data.filter(r => r.res === 'W').length;
  const dead = data.filter(r => r.res === 'D').length;
  const celA = data.filter(r => r.res === 'C' && r.bf != null);
  const cel = celA.length ? celA.reduce((s, r) => s + (1 - r.bf), 0) / celA.length : null;
  const tile = (k, v) => '<div class="stat"><div class="k">' + k + '</div><div class="v">' + v + '</div></div>';
  g.innerHTML = tile('Questions', dn)
    + tile('Accuracy', dn ? Math.round(c / dn * 100) + '<small>%</small>' : '—')
    + tile('Celerity', cel == null ? '—' : Math.round(cel * 100) + '<small>%</small>')
    + tile('Neg rate', dn ? Math.round(negs / dn * 100) + '<small>%</small>' : '—')
    + tile('Dead', dn ? Math.round(dead / dn * 100) + '<small>%</small>' : '—');
  $('statnote').innerHTML = '<button class="btn primary" id="drillbtn">&#9654; Drill my weaknesses</button>'
    + ' <span style="color:var(--faint)">Celerity = how early you buzz on correct answers (higher = earlier).</span> '
    + '<button class="linkbtn" id="clearstats">Clear history</button>';
  $('drillbtn').onclick = () => {
    settings.drill = true; $('drill').checked = true; $('drillrow').style.display = '';
    rebuildQueue(); showView('play');
  };
  $('clearstats').onclick = () => { if (confirm('Clear all reader history?')) { LOG = []; saveLog(); renderStats(); rebuildQueue(); } };

  // Scope — one multi-select chip row per coarseness level; an empty row
  // means all. Subcategory/Group rows wait for a category pick so they
  // don't dump the whole taxonomy; their options follow the picked
  // categories (facetVals applies every OTHER facet).
  const escA = v => esc(v).replace(/"/g, '&quot;');
  let h = '<div class="scopebox">';
  for (const f of DIM_ORDER) {
    h += '<div class="facetrow"><span class="facetlbl">' + DIM_LABEL[f] + '</span>';
    if ((f === 'sub' || f === 'section') && !statsScope.cat.size && !statsScope[f].size) {
      h += '<span class="facetnote">pick a category first</span></div>';
      continue;
    }
    for (const v of facetVals(f))
      h += '<button class="chip' + (statsScope[f].has(v) ? ' on' : '') + '" data-facet="' + f
        + '" data-val="' + escA(v) + '">' + esc(f === 'diff' ? 'Diff ' + v : v) + '</button>';
    h += '</div>';
  }
  h += '<div class="scopefoot">' + dn + ' of ' + n + ' questions in scope'
    + (DIM_ORDER.some(f => statsScope[f].size)
        ? ' <button class="linkbtn" id="sc-reset">Reset</button>' : '')
    + '</div></div>';
  h += trendBlock(data) + exposureBlock(data);
  h += progressBlock(data);
  // dimension toggles — the breakdown groups by every active dimension
  h += '<h2 class="sechead">Breakdown</h2>'
    + '<div class="statnote">Toggle dimensions to combine them. Columns sort, weakest first.</div>'
    + '<div class="dimsel" id="dimsel">';
  for (const d of DIM_ORDER)
    h += '<button class="dimbtn' + (statsDims.has(d) ? ' on' : '') + '" data-dim="' + d + '">' + DIM_LABEL[d] + '</button>';
  h += '</div>';
  const activeDims = DIM_ORDER.filter(d => statsDims.has(d));
  // rows behind each breakdown key, for the Trend sparklines
  const byKey = new Map();
  for (const r of data) {
    const k = compositeKey(r); if (k == null) continue;
    let a = byKey.get(k); if (!a) { a = []; byKey.set(k, a); }
    a.push(r);
  }
  // a pin whose key no longer exists (scope narrowed, dimensions changed)
  // is dropped rather than silently drawn as an empty series
  progState.pins = progState.pins.filter(k => byKey.has(k));
  const spark = data.length >= PROG_MIN_ROWS ? { map: byKey, keys: [] } : null;
  h += statTable(aggBy(compositeKey, data), {
    label: activeDims.map(d => DIM_LABEL[d]).join(' · '),
    min: statsDims.has('section') || activeDims.length > 1 ? 2 : 1,
    name: k => k.split(KEY_SEP).map((p, i, a) =>
      i < a.length - 1 ? '<span class="kpart">' + esc(p) + ' · </span>' : esc(p)).join(''),
    spark,
  });
  h += calibrationBlock(data);
  // weakest answerlines — the drill seed
  h += '<h2 class="sechead">Weakest answerlines</h2>'
    + '<div class="statnote">Individual answers you miss most, weakest first. Only answerlines you have seen more than '
    + (ANS_MIN - 1) + ' times, so one bad night does not put an answer on the list.</div>'
    + statTable(aggBy(r => r.ans, data), {
        label: 'Answerline', min: ANS_MIN,
        name: k => {
          const rec = data.find(r => r.ans === k);
          const disp = rec ? rec.aDisp : k;
          return rec && rec.t ? '<a href="output/' + esc(rec.t) + '/stock.html">' + esc(disp) + '</a>' : esc(disp);
        },
      });
  body.innerHTML = h;
  body.querySelector('.scopebox').addEventListener('click', e => {
    const c = e.target.closest('.chip'); if (!c) return;
    const s = statsScope[c.dataset.facet], v = c.dataset.val;
    if (s.has(v)) s.delete(v); else s.add(v);
    if (c.dataset.facet !== 'diff') pruneScope();
    renderStats();
  });
  if ($('sc-reset')) $('sc-reset').onclick = () => { DIM_ORDER.forEach(f => statsScope[f].clear()); renderStats(); };
  $('dimsel').addEventListener('click', e => {
    const b = e.target.closest('.dimbtn'); if (!b) return;
    const d = b.dataset.dim;
    if (statsDims.has(d)) { if (statsDims.size > 1) statsDims.delete(d); }
    else statsDims.add(d);
    renderStats();
  });
  for (const th of body.querySelectorAll('.sorth')) {
    th.onclick = () => {
      const c2 = th.dataset.col;
      if (statsSort.col === c2) statsSort.dir *= -1;
      else statsSort = { col: c2, dir: c2 === 'n' ? -1 : 1 };
      renderStats();
    };
  }
  drawCalib(data);   // paint the calibration canvas once it's in the DOM
  wireProgress(data, spark);
}

/* ---------- history view: searchable log of played questions ----------
   Every attempt in LOG, newest first, filtered by a free-text query over
   the answer, taxonomy, and set name. A row expands to the full question
   text + answerline, resolved lazily from the set shard (same path the
   reader itself uses), with a wiki link when the answer has a page. */
const HIST_PAGE = 50;
let histShown = HIST_PAGE;
function histSetName(id) {
  const r = ID2ROW.get(id);
  return r != null ? CAT.sets[CAT.tossups.set[r]].name : '';
}
function histMatches() {
  const toks = $('histq').value.trim().toLowerCase().split(/\s+/).filter(Boolean);
  const out = [];
  for (let i = LOG.length - 1; i >= 0; i--) {
    const rec = LOG[i];
    if (toks.length) {
      const hay = [rec.aDisp, rec.ans, rec.cat, rec.sub, rec.section, histSetName(rec.id)]
        .filter(Boolean).join(' ').toLowerCase();
      if (!toks.every(t => hay.includes(t))) continue;
    }
    out.push(rec);
  }
  return out;
}
function renderHistory() {
  if (!CAT) { $('histlist').innerHTML = '<div class="empty">Loading&hellip;</div>'; return; }
  if (!LOG.length) {
    $('histnote').textContent = '';
    $('histlist').innerHTML = '<div class="empty">No history yet.</div>';
    return;
  }
  const m = histMatches();
  const searching = !!$('histq').value.trim();
  $('histnote').textContent = (searching ? m.length + ' of ' + LOG.length + ' played questions match'
    : LOG.length + ' played questions') + ', newest first. Click to re-read.';
  const RES = { C: ['c', 'Correct'], W: ['w', 'Neg'], D: ['d', 'Dead'] };
  let h = '';
  for (const rec of m.slice(0, histShown)) {
    const [cls, word] = RES[rec.res] || ['', rec.res || '?'];
    const depth = rec.bf != null ? ' · ' + Math.round(rec.bf * 100) + '%' : '';
    const when = rec.ts > 1e12 ? new Date(rec.ts).toLocaleDateString() : '';
    const meta = [rec.cat, rec.sub !== rec.cat ? rec.sub : null, when].filter(Boolean).join(' · ');
    h += '<div class="histrow" data-id="' + esc(rec.id) + '">'
      + '<div class="histhead"><span class="hist-ans">' + esc(rec.aDisp || rec.ans || '—') + '</span>'
      + '<span class="hist-meta">' + esc(meta) + '</span>'
      + '<span class="hist-res ' + cls + '">' + word + (rec.pow ? ' · power' : '') + depth + '</span></div>'
      + '<div class="histbody"></div></div>';
  }
  if (m.length > histShown)
    h += '<button class="btn" id="histmore">Show ' + Math.min(HIST_PAGE, m.length - histShown) + ' more</button>';
  $('histlist').innerHTML = h || '<div class="empty">Nothing matches.</div>';
  const more = $('histmore');
  if (more) more.onclick = () => { histShown += HIST_PAGE; renderHistory(); };
}
function histExpand(row) {
  const body = row.querySelector('.histbody');
  if (body.classList.contains('show')) { body.classList.remove('show'); return; }
  body.classList.add('show');
  if (body.dataset.loaded) return;
  body.dataset.loaded = '1';
  const id = row.dataset.id;
  const r = ID2ROW.get(id);
  if (r == null) {
    body.innerHTML = '<div class="statnote" style="margin:0">This question is no longer in the published catalog.</div>';
    return;
  }
  body.innerHTML = '<div class="statnote" style="margin:0">Loading&hellip;</div>';
  shardFor(CAT.tossups.set[r]).then(docs => {
    const doc = docs.get(id);
    if (!doc) { body.innerHTML = '<div class="statnote" style="margin:0">Question text unavailable.</div>'; return; }
    const slug = ID2SLUG.get(id);
    body.innerHTML = '<div class="qfull">' + esc(doc.question_sanitized || doc.question || '') + '</div>'
      + '<div class="answerline">' + answerHTML(doc.answer || doc.answer_sanitized || '') + '</div>'
      + (slug ? '<div style="margin-top:0.4rem"><a href="output/' + esc(slug) + '/stock.html">wiki page ↗</a></div>' : '');
  }).catch(() => {
    body.dataset.loaded = '';
    body.innerHTML = '<div class="statnote" style="margin:0">Could not load the question. Try again.</div>';
  });
}

/* ---------- preferences: reading settings + scope, persisted ----------
   So a refresh keeps what you just set (speed, trim, drill/focus, and the
   category/subcategory/subtype/group/difficulty scope). Sections persist as
   [unit, name] pairs, not catalog ids, so they survive a republish that
   reorders section_values. */
const PREFS = 'losReaderPrefsV1';
const FSIZE = 'losReaderFontSize';   // per-device, not part of the synced prefs
const focusLabel = v => v < 33 ? 'broad review' : v < 67 ? 'balanced' : 'targeted';
function savePrefs() {
  try {
    const sections = [...filters.sections].map(id =>
      id === -1 ? [-1] : (CAT && CAT.section_values[id]) || null).filter(Boolean);
    localStorage.setItem(PREFS, JSON.stringify({
      wpm: settings.wpm, sent: settings.sent, drill: settings.drill, focus: settings.focus,
      dist: settings.dist, distSub: settings.distSub, useDist: settings.useDist,
      voice: settings.voice, vrate: settings.vrate, multibuzz: settings.multibuzz,
      cats: [...filters.cats], subs: [...filters.subs], subsubs: [...filters.subsubs],
      diffs: [...filters.diffs], sections,
      up: Date.now(),   // last-write-wins stamp for cross-device sync
    }));
  } catch (e) {}
}
function loadPrefs() {
  let p; try { p = JSON.parse(localStorage.getItem(PREFS) || 'null'); } catch (e) {}
  if (!p) return;
  if (typeof p.wpm === 'number') settings.wpm = p.wpm;
  if (typeof p.sent === 'number') settings.sent = p.sent;
  if (typeof p.drill === 'boolean') settings.drill = p.drill;
  if (typeof p.focus === 'number') settings.focus = p.focus;
  if (p.dist && typeof p.dist === 'object') settings.dist = { ...DEFAULT_DIST, ...p.dist };
  if (p.distSub && typeof p.distSub === 'object') settings.distSub = p.distSub;
  if (typeof p.useDist === 'boolean') settings.useDist = p.useDist;
  if (typeof p.voice === 'boolean') settings.voice = p.voice;
  if (typeof p.vrate === 'number') settings.vrate = p.vrate;
  if (typeof p.multibuzz === 'boolean') settings.multibuzz = p.multibuzz;
  const setFrom = (set, arr) => { set.clear(); for (const v of (arr || [])) set.add(v); };
  setFrom(filters.cats, p.cats); setFrom(filters.subs, p.subs);
  setFrom(filters.subsubs, p.subsubs); setFrom(filters.diffs, p.diffs);
  filters.sections.clear();
  const SV = CAT.section_values || [];
  for (const pair of (p.sections || [])) {
    if (pair[0] === -1) { filters.sections.add(-1); continue; }
    const idx = SV.findIndex(s => s && s[0] === pair[0] && s[1] === pair[1]);
    if (idx >= 0) filters.sections.add(idx);
  }
}
// Sentences-read control: the Full button is "on" at sent=0, otherwise the
// manual last-n number field carries the value.
function syncSentUI() {
  $('sentmode').querySelector('button').classList.toggle('on', settings.sent === 0);
  const inp = $('sentn');
  inp.value = settings.sent > 0 ? settings.sent : '';
  inp.classList.toggle('on', settings.sent > 0);
}
// Push restored settings into the reading-panel controls (the filter chips
// re-render from state via renderFilters).
function syncControls() {
  $('wpm').value = settings.wpm;
  $('wpmval').textContent = settings.wpm;
  syncSentUI();
  $('voice').checked = settings.voice;
  $('voicerow').style.display = settings.voice ? '' : 'none';
  $('vrate').value = settings.vrate;
  $('vrateval').textContent = settings.vrate.toFixed(2) + 'x';
  $('multibuzz').checked = settings.multibuzz;
  $('multibuzzrow').style.display = settings.multibuzz ? '' : 'none';
  $('drill').checked = settings.drill;
  $('drillrow').style.display = settings.drill ? '' : 'none';
  $('focus').value = settings.focus;
  $('focusval').textContent = focusLabel(settings.focus);
  $('usedist').checked = settings.useDist;
  renderDistEditor();
}
/* ---------- distribution editor (nested weight tree) ---------- */
// Taxonomy tree for the editor, from the corpus maps: category ->
// subcategories -> subtypes (only for subtype-bearing subs). A category with
// a single subcategory collapses that level and hangs the subtypes, if any,
// directly under the category (Social Science). Alphabetical with Other/Misc
// names last, so the tree stays put as the corpus grows. Each child carries
// its split-group key in `parent` — the name whose distSub entry holds it.
let DIST_TREE = null;
function distTree() {
  if (DIST_TREE || !CAT) return DIST_TREE;
  if (!SUBTYPED_SUBS) computeSubtypedSubs();
  const subsOf = {};
  for (const s in CAT_OF_SUB) (subsOf[CAT_OF_SUB[s]] = subsOf[CAT_OF_SUB[s]] || []).push(s);
  const order = (a, b) => {
    const oa = /^(Other|Misc)\b/.test(a), ob = /^(Other|Misc)\b/.test(b);
    return oa !== ob ? (oa ? 1 : -1) : a.localeCompare(b);
  };
  const altKids = s => SUBTYPED_SUBS.has(s)
    ? [...(ALTS_OF_SUB[s] || [])].sort(order).map(a => ({ name: a, parent: s, kids: [] }))
    : [];
  DIST_TREE = Object.keys(DEFAULT_DIST).map(c => {
    const subs = (subsOf[c] || []).sort(order);
    const kids = subs.length > 1
      ? subs.map(s => ({ name: s, parent: c, kids: altKids(s) }))
      : altKids(subs[0] || '');
    return { name: c, parent: null, kids };
  });
  return DIST_TREE;
}
// Session-only expanded groups. Only category and subcategory nodes can
// expand, and those names are globally unique, so the bare name is a key.
const distOpen = new Set();
const distNodeW = n => {
  if (!n.parent) return Math.max(0, settings.dist[n.name] || 0);
  const g = settings.distSub[n.parent];
  return g && typeof g[n.name] === 'number' ? Math.max(0, g[n.name]) : 1;
};
function renderDistEditor() {
  const tree = $('disttree'); if (!tree) return;
  tree.innerHTML = '';
  tree.classList.toggle('disabled', !settings.useDist);
  const roots = distTree(); if (!roots) return;
  const den = roots.reduce((t, n) => t + distNodeW(n), 0);
  for (const n of roots) renderDistNode(tree, n, 0, den > 0 ? 100 * distNodeW(n) / den : 0, null);
  const tot = $('disttotal');
  if (tot) tot.textContent = den > 0 ? 'weights sum ' + (+den.toFixed(2)) : 'all zero';
}
function distEdited() { savePrefs(); if (!settings.drill) rebuildQueue(); renderDistEditor(); }
function renderDistNode(host, n, lv, sharePct, siblings) {
  const row = document.createElement('div');
  row.className = 'drow lv' + lv + (sharePct <= 0 ? ' zero' : '');
  const open = n.kids.length > 0 && distOpen.has(n.name);
  const tw = document.createElement('button');
  tw.type = 'button';
  tw.className = 'twist' + (n.kids.length ? (open ? ' open' : '') : ' leaf');
  tw.textContent = '▶';
  tw.setAttribute('aria-label', (open ? 'Collapse ' : 'Expand ') + n.name);
  if (n.kids.length) tw.onclick = () => {
    distOpen.has(n.name) ? distOpen.delete(n.name) : distOpen.add(n.name);
    renderDistEditor();
  };
  const nm = document.createElement('span');
  nm.className = 'dname'; nm.textContent = n.name; nm.title = n.name;
  const pct = document.createElement('span');
  pct.className = 'dpct';
  pct.textContent = sharePct > 0 ? (sharePct < 0.95 ? sharePct.toFixed(1) : Math.round(sharePct)) + '%' : '';
  const inp = document.createElement('input');
  inp.className = 'dw' + (n.parent && !settings.distSub[n.parent] ? ' untouched' : '');
  inp.type = 'number'; inp.min = '0'; inp.step = '0.5';
  inp.value = distNodeW(n);
  inp.disabled = !settings.useDist;
  inp.setAttribute('aria-label', n.name + ' weight');
  inp.onchange = () => {
    let v = parseFloat(inp.value); if (!(v >= 0)) v = 0;
    if (!n.parent) settings.dist[n.name] = v;
    else {
      // persist the whole group on first edit, so the split is explicit
      const g = settings.distSub[n.parent] = settings.distSub[n.parent] || {};
      for (const sib of siblings) if (typeof g[sib.name] !== 'number') g[sib.name] = 1;
      g[n.name] = v;
    }
    distEdited();
  };
  const bar = document.createElement('span');
  bar.className = 'bar';
  bar.style.width = Math.min(100, sharePct * (lv === 0 ? 2.2 : 4)) + '%';
  row.append(tw, nm, pct, inp, bar);
  host.appendChild(row);
  if (open) {
    const kids = document.createElement('div');
    kids.className = 'kids';
    const kden = n.kids.reduce((t, k) => t + distNodeW(k), 0);
    for (const k of n.kids)
      renderDistNode(kids, k, lv + 1, kden > 0 ? sharePct * distNodeW(k) / kden : 0, n.kids);
    const gk = n.kids[0].parent;
    if (settings.distSub[gk]) {
      const er = document.createElement('div'); er.className = 'evenrow';
      const eb = document.createElement('button');
      eb.type = 'button'; eb.className = 'linkbtn'; eb.textContent = 'Even';
      eb.setAttribute('aria-label', 'Even split for ' + gk);
      eb.onclick = () => { delete settings.distSub[gk]; distEdited(); };
      er.appendChild(eb); kids.appendChild(er);
    }
    host.appendChild(kids);
  }
}

/* ---------- controls ---------- */
function updateButtons() {
  const main = $('mainbtn');
  if (reviewing()) { main.textContent = 'Return to current'; main.disabled = false; }
  else if (phase === 'idle') { main.textContent = 'Start'; main.disabled = false; }
  else if (phase === 'loading') { main.textContent = 'Loading…'; main.disabled = true; }
  else if (phase === 'reading' || phase === 'paused') { main.textContent = 'Buzz'; main.disabled = false; }
  else if (phase === 'buzzed' || phase === 'selfgrade') { main.textContent = 'Buzz'; main.disabled = true; }
  else { main.textContent = 'Next question'; main.disabled = false; }
  $('pausebtn').disabled = reviewing() || !(phase === 'reading' || phase === 'paused');
  $('pausebtn').textContent = phase === 'paused' ? 'Resume' : 'Pause';
  $('skipbtn').disabled = reviewing() || !(phase === 'reading' || phase === 'paused' || phase === 'buzzed' || phase === 'selfgrade');
  // Prev enables once there's history and the live question is finished (or
  // while already reviewing, to step further back).
  const prev = $('prevbtn');
  if (prev) prev.disabled = !(history.length && (reviewing() || phase === 'done' || phase === 'idle'));
  syncMobileBar();
  if (typeof syncQPane === 'function') syncQPane();   // guard: test slices stub around this
}

function wireUp() {
  $('mainbtn').onclick = () => {
    if (reviewing()) { exitReview(); return; }
    if (phase === 'idle' || phase === 'done') loadNext();
    else if (phase === 'reading' || phase === 'paused') buzz();
  };
  $('prevbtn').onclick = openPrev;
  $('pausebtn').onclick = () => {
    if (phase === 'reading') {
      // Audio pauses natively (reliable) — don't tear down the element via
      // clearTimers; just pause it. Text mode stops its reveal timer.
      if (cur.voice && audioEl) { audioEl.pause(); wordIdx = audioWordIdx(); }
      else clearTimers();
      phase = 'paused'; setStatus('reading', 'Paused');
    } else if (phase === 'paused') {
      phase = 'reading'; setStatus('reading', cur.voice ? 'Reading aloud' : 'Reading');
      if (wordIdx >= cur.units.length) startGrace();
      else if (cur.voice && audioEl) audioEl.play().catch(() => {});   // native resume
      else scheduleStep();
    }
    updateButtons();
  };
  $('skipbtn').onclick = () => { // throw the question away — nothing recorded,
    // except a multibuzz neg already made: skipping must not erase it
    if (phase === 'reading' || phase === 'paused' || phase === 'buzzed' || phase === 'selfgrade') {
      if (cur && cur.negged && cur.res == null) { cur.res = 'W'; record('W'); }
      clearTimers(); loadNext();
    }
  };
  // Clicks on the text only toggle skipped clues or open a clue lookup —
  // buzzing and advancing are button-only (plus keys on desktop).
  $('qtext').addEventListener('click', e => {
    if (cur && e.target.closest('#skiptoggle')) { cur.showSkipped = !cur.showSkipped; renderQText(); return; }
    const cs = cur && phase === 'done' && e.target.closest('.clue-s');
    if (cs) openCluePanel(Number(cs.dataset.si));
  });
  $('answerinput').addEventListener('keydown', e => {
    if (e.key === 'Enter') { e.preventDefault(); submitAnswer(); }
    e.stopPropagation();
  });
  $('clueclose').onclick = closeCluePanel;
  $('sg-right').onclick = () => regrade('C');
  $('sg-wrong').onclick = () => regrade('W');

  document.addEventListener('keydown', e => {
    if (e.target.tagName === 'INPUT') return;
    if (e.key === 'k' || e.key === 'K') { e.preventDefault(); openPrev(); return; }
    if (e.key === 'j' || e.key === 'J') { e.preventDefault(); reviewForward(); return; }
    if (reviewing()) {   // in review, Space/N step forward (toward the live question)
      if (e.code === 'Space' || e.key === 'n' || e.key === 'N') { e.preventDefault(); reviewForward(); }
      return;
    }
    if (e.code === 'Space') { e.preventDefault();
      if (phase === 'reading' || phase === 'paused') buzz();
      else if (phase === 'done' || phase === 'idle') loadNext();
    }
    else if (e.key === 'n' || e.key === 'N') { if (phase === 'done' || phase === 'idle') loadNext(); }
    else if (e.key === 's' || e.key === 'S') { $('skipbtn').click(); }
    else if (e.key === 'p' || e.key === 'P') { $('pausebtn').click(); }
  });

  $('wpm').value = settings.wpm;
  $('wpmval').textContent = settings.wpm;
  $('wpm').oninput = e => {
    settings.wpm = Number(e.target.value);
    $('wpmval').textContent = settings.wpm;
    savePrefs();
  };
  // Question text size in px. Per-device (like the clue-search backend): a
  // size that reads well on a phone is wrong on a monitor, so it never
  // syncs. Defaults to the stylesheet's computed size for this layout.
  const applyFsize = px => {
    document.documentElement.style.setProperty('--qsize', px + 'px');
    $('fsize').value = px;
    $('fsizeval').textContent = px + 'px';
  };
  let fsize = Number(localStorage.getItem(FSIZE));
  if (!(fsize >= 13 && fsize <= 28))
    fsize = Math.round(parseFloat(getComputedStyle($('qtext')).fontSize)) || 17;
  applyFsize(fsize);
  $('fsize').oninput = e => {
    fsize = Number(e.target.value);
    applyFsize(fsize);
    try { localStorage.setItem(FSIZE, String(fsize)); } catch (err) {}
  };
  $('sentmode').querySelector('button').onclick = () => {
    settings.sent = 0;
    syncSentUI();
    savePrefs();
  };
  $('sentn').oninput = e => {
    const v = Math.floor(Number(e.target.value));
    settings.sent = Number.isFinite(v) && v > 0 ? Math.min(v, 99) : 0;
    // Don't rewrite the field mid-typing; just reflect the active state.
    $('sentmode').querySelector('button').classList.toggle('on', settings.sent === 0);
    $('sentn').classList.toggle('on', settings.sent > 0);
    savePrefs();
  };
  $('sentn').onblur = syncSentUI;
  // Voice reading — plays pre-generated audio; restricts the queue to
  // questions that have audio. Toggling loads the manifest, then rebuilds.
  if (!voiceAvailable()) {
    $('voicetoggle').style.display = 'none';
    $('voicerow').style.display = 'none';
  } else {
    $('voice').onchange = e => {
      settings.voice = e.target.checked;
      $('voicerow').style.display = settings.voice ? '' : 'none';
      savePrefs();
      if (settings.voice) enterAudioMode(); else { onFilter(); }
    };
    $('vrate').oninput = e => {
      settings.vrate = Number(e.target.value);
      $('vrateval').textContent = settings.vrate.toFixed(2) + 'x';
      if (audioEl) audioEl.playbackRate = voiceRate();   // live, mid-question
      savePrefs();
    };
  }
  // Multibuzz — a wrong answer resumes reading instead of ending the
  // question. Takes effect at the next wrong answer; no queue impact.
  $('multibuzz').onchange = e => {
    settings.multibuzz = e.target.checked;
    $('multibuzzrow').style.display = settings.multibuzz ? '' : 'none';
    savePrefs();
  };
  // Drill mode + focus slider. settings.focus holds the raw 0..100 value;
  // rebuildQueue derives the drive blend and sampling temperature from it
  // (0 -> broad/schedule-driven, 100 -> targeted/hammer-weaknesses).
  $('drill').onchange = e => {
    settings.drill = e.target.checked;
    $('drillrow').style.display = settings.drill ? '' : 'none';
    rebuildQueue();
    savePrefs();
  };
  $('focus').oninput = e => {
    settings.focus = Number(e.target.value);
    $('focusval').textContent = focusLabel(settings.focus);
    if (settings.drill) rebuildQueue();
    savePrefs();
  };
  $('usedist').onchange = e => {
    settings.useDist = e.target.checked;
    renderDistEditor();
    savePrefs();
    if (!settings.drill) rebuildQueue();
  };
  $('distreset').onclick = () => {
    settings.dist = { ...DEFAULT_DIST };
    settings.distSub = {};
    renderDistEditor();
    savePrefs();
    if (!settings.drill) rebuildQueue();
  };
  $('clearfilters').onclick = () => {
    for (const k of Object.keys(filters)) filters[k].clear();
    onFilter();
  };
  $('tab-play').onclick = () => showView('play');
  $('tab-stats').onclick = () => showView('stats');
  $('tab-missed').onclick = () => showView('missed');
  $('tab-history').onclick = () => showView('history');
  $('missedlist').addEventListener('click', e => {
    const t = e.target.closest('.simtoggle');
    if (!t) return;
    const open = t.nextElementSibling.classList.toggle('open');
    t.textContent = (open ? '▾' : '▸') + t.textContent.slice(1);
  });
  // Clue-search backend toggle (cloud / in-browser). Per-device — the
  // model download is a per-device commitment, so it never syncs.
  $('csmode').addEventListener('click', e => {
    const b = e.target.closest('button');
    if (!b) return;
    losClueSearch.setMode(b.dataset.m);
    syncCsUI();
    if (b.dataset.m === 'local') losClueSearch.prepare().catch(() => {});
  });
  losClueSearch.onProgress(p => {
    const el = $('csprog');
    if (!el) return;
    if (p.phase === 'model') {
      el.style.display = '';
      el.textContent = 'Downloading model · ' + p.loadedMB + ' / ' + p.totalMB + ' MB';
    } else if (p.phase === 'ready') {
      el.style.display = '';
      el.textContent = 'Model ready.';
    }
  });
  syncCsUI();
  $('histq').addEventListener('input', () => { histShown = HIST_PAGE; renderHistory(); });
  $('histq').addEventListener('keydown', e => e.stopPropagation());   // typing must not buzz/skip
  $('histlist').addEventListener('click', e => {
    if (e.target.closest('a')) return;
    const row = e.target.closest('.histrow');
    if (row) histExpand(row);
  });
  initMobile();
}

function showView(v) {
  for (const name of ['play', 'stats', 'missed', 'history']) {
    $('view-' + name).style.display = name === v ? '' : 'none';
    $('tab-' + name).classList.toggle('on', name === v);
  }
  if (v === 'stats') renderStats();
  if (v === 'missed') renderMissed();
  if (v === 'history') renderHistory();
  syncQPane();   // leaving the play view mid-question must retract the run-out
}

/* ---------- mobile chrome ----------
 * Everything mobile-specific lives here. The layout mode is the
 * html[data-layout] attribute maintained by the inline head script
 * (theme.layout_switch_script); components come from lib/js/mobile.js
 * (losSheet/losSwipe/losLayout). The rail panels are single DOM nodes that
 * are REPARENTED between the desktop rail and the bottom sheets — never
 * cloned — so ids, listeners, and state travel with them. The bottom
 * action bar's buttons are pure proxies for the desktop buttons; all game
 * logic stays in the handlers above. */

let sheetScope = null, sheetReading = null;

function updateScopeBadge() {
  const b = $('scopebadge'); if (!b) return;
  const n = Object.values(filters).reduce((a, s) => a + s.size, 0);
  b.textContent = n;
  b.style.display = n ? '' : 'none';
}

// Mirror label + disabled state from the desktop controls onto the bottom
// bar. Called from updateButtons(), so the bar can never drift.
function syncMobileBar() {
  const m = $('m-main'); if (!m) return;
  m.textContent = $('mainbtn').textContent;
  m.disabled = $('mainbtn').disabled;
  $('m-pause').textContent = $('pausebtn').textContent;
  $('m-pause').disabled = $('pausebtn').disabled;
  $('m-skip').disabled = $('skipbtn').disabled;
  $('m-prev').disabled = $('prevbtn').disabled;
}

/* Live-question run-out: while a question is live on a phone, #qspacer
 * extends the page so the revealing text can be scrolled to sit anywhere on
 * screen — plain manual scrolling, no auto-follow. The spacer holds through
 * buzz/grading so the page doesn't jump under the answer input. A new
 * question resets the view to the top. Voice mode is exempt — its text is
 * hidden until done anyway. */
let qpaneQid = null;

function qpaneLive() {
  return !!(window.losLayout && losLayout() === 'mobile' && cur && !cur.voice
    && (phase === 'reading' || phase === 'paused' || phase === 'buzzed' || phase === 'selfgrade')
    && $('view-play').style.display !== 'none');
}

// Called on phase changes and layout flips (via updateButtons).
function syncQPane() {
  const sp = $('qspacer'); if (!sp) return;
  const live = qpaneLive();
  sp.hidden = !live;
  if (live && qpaneQid !== cur.q.id) {
    qpaneQid = cur.q.id;
    window.scrollTo({ top: 0 });
  }
}

// Move the rail panels (and the timerbar) into the mobile chrome and back.
// appendChild moves the live nodes; the desktop homes (scope in the left
// rail, reading + sync in the right) and the timerbar's place at the end of
// .controls are restored on the way out.
function placePanels(mode) {
  const timerbar = document.querySelector('.timerbar');
  if (mode === 'mobile') {
    $('sheet-scope-body').appendChild($('panel-scope'));
    $('sheet-reading-body').appendChild($('panel-reading'));
    $('sheet-reading-body').appendChild($('panel-cluesearch'));
    $('sheet-reading-body').appendChild($('syncpanel'));
    $('mbar-timer').appendChild(timerbar);
  } else {
    if (sheetScope) sheetScope.close();
    if (sheetReading) sheetReading.close();
    $('rail').appendChild($('panel-scope'));
    $('rail2').appendChild($('panel-reading'));
    $('rail2').appendChild($('panel-cluesearch'));
    $('rail2').appendChild($('syncpanel'));
    document.querySelector('.controls').appendChild(timerbar);
  }
}

// While the answer input is focused on mobile, ride the bottom bar above
// the on-screen keyboard (visualViewport gives the visible height) and
// center the input. Listeners detach on blur so no stale offsets stick.
function initKbInset() {
  const vv = window.visualViewport; if (!vv) return;
  const input = $('answerinput'), mbar = $('mbar');
  let active = false;
  const onVV = () => {
    if (!active) return;
    mbar.style.bottom = Math.max(0, window.innerHeight - vv.height - vv.offsetTop) + 'px';
  };
  input.addEventListener('focus', () => {
    if (losLayout() !== 'mobile') return;
    active = true;
    vv.addEventListener('resize', onVV);
    vv.addEventListener('scroll', onVV);
    setTimeout(() => {
      onVV();
      input.scrollIntoView({ block: 'center', behavior: 'smooth' });
    }, 300);
  });
  input.addEventListener('blur', () => {
    active = false;
    mbar.style.bottom = '';
    vv.removeEventListener('resize', onVV);
    vv.removeEventListener('scroll', onVV);
  });
}

function initMobile() {
  if (!window.losSheet || !$('mbar')) return;  // mobile.js not loaded
  sheetScope = losSheet($('sheet-scope'));
  sheetReading = losSheet($('sheet-reading'));
  $('open-scope').onclick = () => sheetScope.toggle();
  $('open-reading').onclick = () => sheetReading.toggle();

  // Bottom-bar proxies — route through the real buttons.
  $('m-main').onclick = () => $('mainbtn').click();
  $('m-pause').onclick = () => $('pausebtn').click();
  $('m-skip').onclick = () => $('skipbtn').click();
  $('m-prev').onclick = () => $('prevbtn').click();

  // Swipes on the question card. Left = advance (Next when finished, Skip
  // — uncounted — while live, step forward in review); right = previous.
  // Buzzing stays button-only: a timing-critical action must not wait on
  // gesture disambiguation.
  losSwipe($('qcard'), {
    left: () => {
      if (reviewing()) { reviewForward(); return; }
      if (phase === 'done' || phase === 'idle') loadNext();
      else if (phase === 'reading' || phase === 'paused' || phase === 'buzzed' || phase === 'selfgrade') $('skipbtn').click();
    },
    right: openPrev,
  });


  window.addEventListener('loslayout', e => { placePanels(e.detail); syncQPane(); });
  placePanels(losLayout());
  initKbInset();
  updateScopeBadge();
}

/* ---------- boot ---------- */
wireUp();
$('qtext').innerHTML = '<div class="empty">Loading the question catalog&hellip;</div>';
Promise.all([qdataFetch('catalog.json'), qdataFetch('topics.json')]).then(([cat, top]) => {
  CAT = cat;
  TOPICS = top.topics;
  // Deduped answerline keys for the drill scheduler; normAns is idempotent,
  // so re-normalizing the published (already-normalized) values is a no-op
  // safety net that keeps the JS normalizer authoritative vs LOG entries.
  AKEYS = (CAT.answerline_values || []).map(normAns);
  ID2SLUG = new Map();
  for (const slug in TOPICS) for (const id of (TOPICS[slug].tossups || [])) ID2SLUG.set(id, slug);
  ID2ROW = new Map();
  for (let r = 0; r < CAT.tossups.id.length; r++) ID2ROW.set(CAT.tossups.id[r], r);
  loadPrefs();       // restore reading settings + scope from a prior session
  syncControls();
  // If read-aloud was left on, load the audio manifest before building the
  // queue so the audio restriction applies from the first render.
  const ready = (settings.voice ? loadAudioIndex().catch(() => { settings.voice = false; if ($('voice')) $('voice').checked = false; $('voicerow').style.display = 'none'; }) : Promise.resolve());
  ready.then(() => {
    renderFilters();
    rebuildQueue();
    $('qtext').innerHTML = '<div class="empty">' + queueSize().toLocaleString()
      + ' questions in scope. Press <b>Start</b> (or tap here).</div>';
    phase = 'idle';
    updateButtons();
  });
}).catch(err => {
  $('qtext').innerHTML = qdataErrorHtml(err);
});
})();
