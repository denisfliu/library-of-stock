...
const settings = { wpm: 380, sent: 0, drill: false, focus: 55,  // focus = raw 0..100 (broad..targeted)
  dist: { ...DEFAULT_DIST }, useDist: true, voice: false, voiceName: '', vrate: 0.95 };
let queue = [], qpos = -1;
let cur = null;
...
function startQuestion(r, doc) {
  ...
  updateButtons();
  if (cur.voice) speakFrom(0); else scheduleStep();
}
...
/* ---------- voice reading (Web Speech API) ----------
   When the Voice toggle is on, the question is spoken instead of shown —
   the text stays hidden until the question resolves (guard at the top of
   renderQText), like a moderator reading to you. Buzz position comes from
   the utterance's word-boundary events mapped back to reveal units, so
   buzz depth, power detection, and stats keep working; engines that never
   fire boundary events (some mobile) fall back to a rate-based time
   estimate at buzz/pause time. Pause is cancel + re-speak from the current
   word — native pause/resume is unreliable across engines. */
const TTS = window.speechSynthesis || null;
let speechRun = null;   // live utterance bookkeeping; null when not speaking

function voiceAvailable() { return !!TTS; }
function listVoices() {
  if (!TTS) return [];
  const vs = TTS.getVoices();
  const en = vs.filter(v => /^en/i.test(v.lang));
  // local voices first — they start faster and never hit Chrome's
  // remote-voice long-utterance stall
  return (en.length ? en : vs).slice().sort((a, b) =>
    ((b.localService === true) - (a.localService === true)) || a.name.localeCompare(b.name));
}
function pickVoice() {
  const vs = listVoices();
  return vs.find(v => v.name === settings.voiceName) || vs[0] || null;
}
// Voice pace is its own setting (settings.vrate), NOT the text-reveal wpm
// slider — reveal speeds (380 wpm default) map to absurdly fast speech.
// Rate 1 is ~180 wpm for most voices; ~0.95 approximates a moderator.
function voiceRate() { return Math.min(2, Math.max(0.5, settings.vrate || 0.95)); }

function stopSpeech() {
  if (speechRun) {
    speechRun.dead = true;
    if (speechRun.keep) clearInterval(speechRun.keep);
    speechRun = null;
  }
  if (TTS) TTS.cancel();
}

// This engine never fired a boundary event: estimate progress from elapsed
// time at the voice's approximate wpm.
function voiceProgressEstimate() {
  const run = speechRun; if (!run || !run.t0) return wordIdx;
  const est = Math.floor((performance.now() - run.t0) / 60000 * voiceRate() * 180);
  return Math.min(cur.units.length, run.base + est);
}
...
function speakFrom(startIdx) {
  ...
  for (let i = startIdx; i < units.length; i++) {
    ...
    if (units[i].t !== '(*)') text += units[i].t + ' ';   // power mark is silent
  }
  const run = { base: startIdx, offs, ptr: 0, bounds: false, dead: false, t0: 0, keep: null };
  speechRun = run;
  ...
  setTimeout(() => {
    ...
    u.rate = voiceRate();
    u.onboundary = e => {
      if (run.dead || typeof e.charIndex !== 'number') return;
      run.bounds = true;
      while (run.ptr + 1 < run.offs.length && run.offs[run.ptr + 1] <= e.charIndex) run.ptr++;
      wordIdx = run.base + run.ptr + 1;   // the word being spoken counts as heard
    };
    u.onend = () => {
      if (run.dead) return;
      stopSpeech();
      if (phase === 'reading') { wordIdx = units.length; startGrace(); }
    };
    u.onerror = () => {   // engine failed (not our own cancel): degrade to text
      if (run.dead) return;
      stopSpeech();
      if (phase === 'reading' && cur && cur.voice) {
        cur.voice = false;
        renderQText();
        scheduleStep();
      }
    ...
    // some engines stall on long utterances unless nudged; resume() while
    // speaking (not paused) is a no-op everywhere else
    run.keep = setInterval(() => { try { TTS.resume(); } catch (e) {} }, 10000);
    TTS.speak(u);
  }, 30);
}

// Voice picker options (device-dependent; refreshed on voiceschanged).
function renderVoiceSel() {
  const sel = $('voicesel'); if (!sel || !voiceAvailable()) return;
  $('voicerow').style.display = settings.voice ? '' : 'none';
  sel.innerHTML = '';
  for (const v of listVoices()) {
    const o = document.createElement('option');
    o.value = v.name;
    o.textContent = v.name.replace(/^(Microsoft|Google|Apple) /, '') + (v.localService ? '' : ' (online)');
    sel.appendChild(o);
  }
...
function buzz() {
  ...
  if (cur.voice && speechRun && !speechRun.bounds) wordIdx = voiceProgressEstimate();
  clearTimers();
  buzzAt = wordIdx;
  phase = 'buzzed';
...
function savePrefs() {
  try {
    ...
    localStorage.setItem(PREFS, JSON.stringify({
      ...
      voice: settings.voice, voiceName: settings.voiceName, vrate: settings.vrate,
...
function loadPrefs() {
  ...
  // voiceName is device-dependent; a name synced from another device simply
  // won't match and pickVoice falls back to the best local default
  if (typeof p.voiceName === 'string') settings.voiceName = p.voiceName;
...
function wireUp() {
  ...
  $('pausebtn').onclick = () => {
    if (phase === 'reading') {
      if (cur.voice && speechRun && !speechRun.bounds) wordIdx = voiceProgressEstimate();
      clearTimers(); phase = 'paused'; setStatus('reading', 'Paused');
    } else if (phase === 'paused') {
      phase = 'reading'; setStatus('reading', cur.voice ? 'Reading aloud' : 'Reading');
      if (wordIdx >= cur.units.length) startGrace();
      else if (cur.voice) speakFrom(Math.max(0, wordIdx - 1));   // re-speak the interrupted word
      else scheduleStep();
    }
    updateButtons();
  };
  ...
  // Voice reading — the toggle takes effect at the next question.
  if (!voiceAvailable()) {
    $('voicetoggle').style.display = 'none';
    $('voicerow').style.display = 'none';
  } else {
    $('voice').onchange = e => { settings.voice = e.target.checked; renderVoiceSel(); savePrefs(); };
    $('voicesel').onchange = e => { settings.voiceName = e.target.value; savePrefs(); };
    $('vrate').oninput = e => {
      settings.vrate = Number(e.target.value);
      $('vrateval').textContent = settings.vrate.toFixed(2) + 'x';
      savePrefs();
    };
    TTS.onvoiceschanged = renderVoiceSel;   // voices load async on most engines
  }
...
// appendChild moves the live nodes; rail order (scope, reading, sync) and
// the timerbar's place at the end of .controls are restored on the way out.
function placePanels(mode) {
  const rail = $('rail'), timerbar = document.querySelector('.timerbar');
  if (mode === 'mobile') {
    ...
    rail.appendChild($('panel-reading'));
    rail.appendChild($('syncpanel'));
