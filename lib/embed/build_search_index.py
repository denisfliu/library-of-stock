...
produces mirror/search_index/:
    ...
    bundle_{b:02d}.bin   partitions packed back-to-back; each row is
                         16B meta + 128B binary code (sign bits of the
                         1024-dim vector):
    ...
    search_manifest.json {version, dims, k, rows, parts: [bundle, offset,
                          length, count], set_slugs}

The Worker (sync/worker.js /search) picks nprobe partitions by centroid
dot product, range-reads them from the bundles, and scans Hamming
distance. Clients resolve ids to text via the published sets/{slug}.json
shards — set_slugs here uses the SAME slug derivation as publish.py.

--eval measures recall of the quantized pipeline against exact float
search on sample queries before anything is uploaded.
...
def _set_ordinals(meta):
    """Per-row set ordinal + the ordinal->slug list, publish-consistent."""
    ...
    for table in ("tossups", "bonuses"):
        id2set.update(qb.execute(f"select id, set_name from {table}"))
    ...
    rows = np.array([ordinals.get(id2set.get(id_), 0xFFFF)
                     for id_, _k, _s in meta], np.uint16)
    return rows, set_slugs
...
def stage(meta, codes, centroids, assign, set_rows, set_slugs, k,
          rq, rscale):
    ...
    manifest = {
        "version": 2, "model": MODEL_ID, "dims": DIMS, "k": k,
        "rows": len(meta), "row_bytes": ROW_BYTES, "bundles": BUNDLES,
        "rerank_dims": RERANK_DIMS, "rerank_top": RERANK_TOP,
        "parts": parts_meta, "set_slugs": set_slugs,
    }
...
def main():
    ...
    set_rows, set_slugs = _set_ordinals(meta)
    stage(meta, codes, centroids, assign, set_rows, set_slugs, args.k,
          rq, rscale)
