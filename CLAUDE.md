...
- **Semantic search** — the page moved to **qbsuite/qb-semantic-search** (Aug 2026; app at `qbsuite.github.io/qb-semantic-search/app/`, dev checkout `../qb-semantic-search`; the root `search.html` is a committed redirect stub). This repo keeps the supply side: `lib/embed/build_search_index.py` builds the R2 IVF-binary index (manifest v3 carries per-row taxonomy/difficulty + set year/name tables; filters prune rows mid-scan) and the sync Worker's `/search` route is the cloud backend (sign-in gated, embeds queries at the edge). `lib/js/clue_search.js` (the shared two-backend engine, used by the reader's clue panel + missed-clues report) is **vendored from qb-semantic-search** — canonical copy in `../qb-semantic-search/app/js/clue_search.js`, synced by `build.py`'s vendor step; its scan is a port of worker.js — keep in lockstep. The taxonomy ordinal contract is `qbmirror.query`'s list order (frozen by qb-mirror's tests) — shared by the index builder, Worker, and the qb-semantic-search app. `tests/semsearch/` is where Worker and engine meet in CI (parity + scan-vs-reference); the page's own tests live in qb-semantic-search.
- **Reader missed-clues report** (Missed clues tab in `reader.html`): session-scoped, worst buzz first; each attempt queries the up-to-3 sentences before the last-heard one (late buzzes need to reach back a couple of clues), re-derived from the LOG row alone (`missedQueryFor` in reader.js — bf + the stored `sent` setting recover the buzz unit exactly; nothing extra is stored) and run through clue search. Tested by `tests/missed_clues/`.
...
node tests/reader_audio/run_tests.js        # reader read-aloud: queue restriction + sidecar audio-time->text mapping
python lib/audio/ttsclean.py               # TTS text-cleaning contract self-test (57 cases)
...
- **In-browser embed recall eval**: the local clue-search mode embeds with the ONNX q8 build of Qwen3-Embedding-0.6B on the WASM backend (cosine ~0.90 / sign agreement ~86% vs the fp16 corpus pipeline; top-10 overlap 9/10 with identical top hit on a spot query, July 2026). **WebGPU is deliberately disabled**: q8 on WebGPU runs with fp16 accumulation and produced completely wrong searches in the field — re-enable only with an in-page parity check (embed a known query, compare against stored reference signs) and try dtype `q4f16`/`fp16` there. Also worth a proper recall sweep (`build_search_index.py --eval-only` with ONNX query vectors).
