...
- **Semantic search** — the page moved to **qbsuite/qb-search** (Aug 2026; app at `qbsuite.github.io/qb-search/app/`, dev checkout `../qb-search`; the root `search.html` is a committed redirect stub). This repo keeps the supply side: `lib/embed/build_search_index.py` builds the R2 IVF-binary index (manifest v3 carries per-row taxonomy/difficulty + set year/name tables; filters prune rows mid-scan) and the sync Worker's `/search` route is the cloud backend (sign-in gated, embeds queries at the edge). `lib/js/clue_search.js` (the shared two-backend engine, used by the reader's clue panel + missed-clues report) is **vendored from qb-search** — canonical copy in `../qb-search/app/js/clue_search.js`, synced by `build.py`'s vendor step; its scan is a port of worker.js — keep in lockstep. The taxonomy ordinal contract is `qbmirror.query`'s list order (frozen by qb-mirror's tests) — shared by the index builder, Worker, and the qb-search app. `tests/semsearch/` is where Worker and engine meet in CI (parity + scan-vs-reference); the page's own tests live in qb-search.
