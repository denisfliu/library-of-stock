...
- **Semantic search page** (`search.html`, `lib/render/build_search.py` + `lib/js/semsearch.js`): free-text semantic search over all embedded sentences/bonus parts with qbreader-/db filters. Two backends behind `lib/js/clue_search.js` (shared engine, per-device toggle on reader + search pages): **cloud** — the sync Worker's `/search` (sign-in gated) embeds queries at the edge and scans the R2 IVF-binary index; **in browser** — transformers.js embeds with the ONNX q8 build of the same model (~600 MB, browser-cached) and the same scan runs client-side via R2 range reads, no sign-in. The scan in clue_search.js is a port of worker.js — keep in lockstep. Index: `lib/embed/build_search_index.py` (manifest v3 carries per-row taxonomy/difficulty + set year/name tables); filters prune rows mid-scan. The taxonomy ordinal contract is `qbmirror.query`'s list order (frozen by qb-mirror's tests) — shared by the index builder, Worker, and pages. Tested by `tests/semsearch/`.
