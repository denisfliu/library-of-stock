...
- **In-browser embed recall eval**: the local clue-search mode embeds with the ONNX q8 build of Qwen3-Embedding-0.6B (cosine ~0.90 / sign agreement ~86% vs the fp16 corpus pipeline; top-10 overlap 9/10 with identical top hit on a spot query, July 2026). Worth a proper sweep: run `build_search_index.py --eval-only`-style recall with ONNX query vectors, and try dtype `q4f16`/`fp16` if q8 recall lags.
